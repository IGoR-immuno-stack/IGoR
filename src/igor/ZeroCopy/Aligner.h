 ...
 * \brief Stores information on the alignment of one genomic template against
 * the target
 * \author Q.Marcou
 * \version 1.0
 *
 * Stores information on the alignment of one genomic template against the
 * target. It contains:
 * - the gene name
 * - the offset of the alignment (index on the target sequence on which the
 * first letter of the FULL genomic template aligns (can be negative or lie
 * outside the target)
 * - 5' and 3' offset positions of the best alignment first and last aligned
 * nucleotide
 * - insertions : indices on the TARGET of inserted nucleotides
 * - deletions : indices on the GENOMIC TEMPLATE of deleted nucleotides
 * - alignment length
 * - list of mismatches (that lie event outside the best alignment to allow IGoR
 * to know mismatch positions in advance while exploring different deletions
 * numbers)
 * - the alignment score
 */
struct Alignment_data {
    ...
    Alignment_data(std::string gene, int off)
        : gene_name(gene),
          ...
          score(0) {}
    Alignment_data(int off, size_t five_p_off, size_t three_p_off, size_t align_len,
                   std::forward_list<int> ins, std::forward_list<int> del, std::vector<int> mis,
    ...
    Alignment_data(std::string gene, int off, size_t align_len, std::forward_list<int> ins,
        ...
        : gene_name(gene),
          ...
          mismatches(mis),
          score(alignment_score) {}
    Alignment_data(std::string gene, int off, size_t five_p_off, size_t three_p_off,
                   size_t align_len, std::forward_list<int> ins, std::forward_list<int> del,
    ...
    /*	bool operator<(const Alignment_data& align){
                                                                    //Hardcode to get the alignments
 in descending order using sort() return this->score > align.score;
                                    }*/
};
 ...
 * \version 1.0
 *
 * The Aligner class allows to perform SW alignments according to the parameters
 * (substitution matrix,gap penalty) supplied upon construction of the object.
 * The SW alignments matrix has been altered for V and J in order to allow for
 * deletions on the deleted side only. Alignments can be made in parallel using
 * openMP
 *
 */
class Aligner {
public:
    ...
    std::forward_list<Alignment_data> align_seq(std::string, double, bool, bool, int, int,
                                                bool = false);
    std::forward_list<Alignment_data> align_seq(std::string, double, bool, bool, int, int,
                                                std::set<std::string>, bool = false);
    std::forward_list<Alignment_data> align_seq(
        std::string, double, bool, std::unordered_map<std::string, std::pair<int, int>>,
        bool = false);
    std::forward_list<Alignment_data> align_seq(
        std::string, double, bool, std::unordered_map<std::string, std::pair<int, int>>,
        std::set<std::string>, bool = false);
    std::forward_list<Alignment_data> align_seq(
        std::string, double, bool, bool, std::unordered_map<std::string, std::pair<int, int>>,
        bool = false);
    std::forward_list<Alignment_data> align_seq(
        std::string, double, bool, bool, std::unordered_map<std::string, std::pair<int, int>>,
        std::set<std::string>, bool = false);
    ...
    std::unordered_map<int, std::forward_list<Alignment_data>> align_seqs(
        std::vector<std::pair<const int, const std::string>>, double, bool);
    std::unordered_map<int, std::forward_list<Alignment_data>> align_seqs(
        std::vector<std::pair<const int, const std::string>>, double, bool, bool);
    std::unordered_map<int, std::forward_list<Alignment_data>> align_seqs(
        std::vector<std::pair<const int, const std::string>>, double, bool, int, int, bool = false);
    std::unordered_map<int, std::forward_list<Alignment_data>> align_seqs(
        std::vector<std::pair<const int, const std::string>>, double, bool, bool, int, int,
        bool = false);
    std::unordered_map<int, std::forward_list<Alignment_data>> align_seqs(
        std::vector<std::pair<const int, const std::string>>, double, bool,
    ...
    std::unordered_map<int, std::forward_list<Alignment_data>> align_seqs(
        std::vector<std::pair<const int, const std::string>>, double, bool, bool,
        std::unordered_map<std::string, std::pair<int, int>>, bool = false);
    void align_seqs(std::string, std::vector<std::pair<const int, const std::string>>, double,
                    bool);
    ...
    void align_seqs(std::string, std::vector<std::pair<const int, const std::string>>, double, bool,
                    std::unordered_map<std::string, std::pair<int, int>>, bool = false);
    void align_seqs(std::string, std::vector<std::pair<const int, const std::string>>, double, bool,
                    bool, std::unordered_map<std::string, std::pair<int, int>>, bool = false);

    // I/O related methods
    void write_alignments_seq_csv(std::string,
                                  std::unordered_map<int, std::forward_list<Alignment_data>>);
    std::unordered_map<int, std::forward_list<Alignment_data>> read_alignments_seq_csv(std::string,
                                                                                       double,
                                                                                       bool);

    void set_genomic_sequences(std::vector<std::pair<std::string, std::string>>);
    int incorporate_in_dels(std::string &, std::string &, const std::forward_list<int>,
                            const std::forward_list<int>, int);
...
private:
    ...
    using DoubleMatrixView =
        std::experimental::mdspan<double, std::experimental::dextents<size_t, 2>>;

    void sw_align_common(const Int_Str &, const Int_Str &, const int, const int, DoubleMatrixView,
                         IntMatrixView, IntMatrixView, IntMatrixView, std::vector<int> &,
                         std::vector<int> &, std::vector<int> &);
    std::list<std::pair<int, Alignment_data>> sw_align(const Int_Str &, const Int_Str &, double,
                                                       bool, int, int);
...
read_alignments_seq_csv(std::string, Gene_class, double, bool,
...
std::vector<
    ...
    map2vect(std::unordered_map<
             int,
             std::pair<std::string, std::unordered_map<Gene_class, std::vector<Alignment_data>>>>);
...
std::unordered_map<std::string, size_t> read_gene_anchors_csv(std::string,
                                                              std::string separator = ";");
std::unordered_map<std::string, std::pair<int, int>> read_template_specific_offset_csv(
    std::string, std::string separator = ";");
...
std::vector<std::pair<const int, const std::string>> sample_indexed_seq(
    std::vector<std::pair<const int, const std::string>>, const size_t);
...
std::forward_list<Alignment_data> extract_best_gene_alignments(
    const std::forward_list<Alignment_data> &);

/*
                                namespace substitution_matrices{
                                                                //from:
   ftp://ftp.ncbi.nih.gov/blast/matrices/NUC.4.4 static Matrix<int> nuc44_sub_matrix(4,4,{5,-4,-4,-4
