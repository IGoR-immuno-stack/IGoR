...
#include <igor/ZeroCopy/Aligner.h>
 ...
 * @gene : Gene class of the gene aligned. V gene allows for deletions on the 3'
 * side of the genomic template, J gene on the 5' , D gene and undefined allow
...
/*
 * This method reads sequences in a fasta file and return a vector of indexed
 * sequences
 */
...
/*
 * This method reads a file with one sequence per line and returns a vector of
 * indexed sequences
 */
...
/*
 * This methods reads a file containing at each line the index of the sequence a
 * semicolon and the actual sequence
...
forward_list<Alignment_data> Aligner::align_seq(string nt_seq, double score_threshold,
    ...
    // This is not very elegant, however this function will probably not be called
    // anymore except for aligning a single sequence.
 ...
 *  \overload
 */
forward_list<Alignment_data> Aligner::align_seq(
    string nt_seq, double score_threshold, bool best_align_only,
...
forward_list<Alignment_data> Aligner::align_seq(
    string nt_seq, double score_threshold, bool best_align_only, bool best_gene_only,
    unordered_map<string, pair<int, int>> genomic_offset_bounds, bool rev_offset_frame /*=false*/) {
...
/**
 * \brief A function performing alignment of all genomic templates against a
 * given sequence
 * \author Q.Marcou, M.Puelma Touzel
 * \version 1.2.1
 *
 * \param [in] nt_seq the nucleotide sequence to study
 * \param [in] score_threshold The SW alignment score threshold to record an
 * alignment
 * \param [in] best_align_only Only retain the best alignment for each genomic
 * template.
 * \param [in] best_gene_only Only retain the best gene/allele candidate (or
 * best candidates if several have the same highest score).
 * \param [in] genomic_offset_bounds A hash map containing offsets lower and
 * upper bounds for each genomic template. Keys of the map are the genomic
 * templates names.
 * \param [in] restricted_genomic_list A set containing the names of the genes
 * that should be aligned to the sequence.
 * \param [in] rev_offset_frame Are offsets bounds given reversed? (offset
 * defined based on the last sequence nt instead of the first). Default is
 * false.
 *
 * Call the SW alignment function for every genomic template aligning them
 * against one target sequence. There is possibility to pass different offset
 * bounds for different genomic templates. There is also a possibility to pass
 * these offsets reversed (i.e defined from the last nucleotide of the target)
 * in case it is more handy (e.g alignement of CDR3 sequences or J/C primer
 * sequencing).
 *
 */
forward_list<Alignment_data> Aligner::align_seq(
    ...
    for (forward_list<pair<string, Int_Str>>::const_iterator iter = int_genomic_sequences.begin();
        ...
        if (restricted_genomic_list.count((*iter).first) > 0) {
            ...
            try {
                ...
                cerr << "Exception caught trying to fetch template specific offset "
                        "bounds in Aligner::align_seq"
                     << endl;
                throw runtime_error("Missing genomic offset bounds for genomic template \"" +
                                    iter->first + "\"");
            }

            // Reverse the offset if necessary (e.g for J CDR3 alignment or sequencing
            // from J primer)
            min_offset += (rev_offset_frame) ? seqlen - 1 : 0;  // seqlen-1 correspond to the index
            // of the last nt of the sequence
 ...
 */
unordered_map<int, forward_list<Alignment_data>> Aligner::align_seqs(
    vector<pair<const int, const string>> sequence_list, double score_threshold,
...
/*
 * \brief A function performing alignment of all genomic templates against all
 * provided sequences. Alignments are stored in memory.
 */
unordered_map<int, forward_list<Alignment_data>> Aligner::align_seqs(
    vector<pair<const int, const string>> sequence_list, double score_threshold,
...
unordered_map<int, forward_list<Alignment_data>> Aligner::align_seqs(
    ...
    bool rev_offset_frame /*=false*/) {
    unordered_map<int, forward_list<Alignment_data>> alignment_map =
        align_seqs(sequence_list, score_threshold, best_align_only, false, genomic_offset_bounds,
                   rev_offset_frame);
    return alignment_map;
}
...
(merge:unordered_map<int,forward_list<Alignment_data>>:omp_out.insert(omp_in.begin(),omp_in.end()))
#pragma omp parallel for schedule(dynamic)
reduction(merge:alignment_map) shared(processed_seq_number)
*/
...
/**
 * \brief A function performing alignment of all genomic templates against all
 * provided sequences. Output on file.
 * \author Q.Marcou, M.Puelma Touzel
 * \version 1.2.0
 *
 * \param [in] filename Path and filename for the ouput alignment file
 * \param [in] sequence_list A forward list containing pairs of nt sequence and
 ...
 *
 * Call the SW alignment function for every genomic template aligning them
 * against all target sequences. Alignments are all written on disk on the fly
 * to avoid memory issues. A summary file containing all alignments parameters
 * and relevant information is created/appended in the directory. There is
 * possibility to pass different offset bounds for different genomic templates.
 * There is also a possibility to pass these offsets reversed (i.e defined from
 * the last nucleotide of the target) in case it is more handy (e.g alignement
 * of CDR3 sequences or J/C primer sequencing).
 *
...
void Aligner::align_seqs(string filename, vector<pair<const int, const string>> sequence_list,
    ...
    // Check if all templates have the same min and max offsets and compute min
    // and max over all of them.
    ...
    outfile << "seq_index" << ";" << "gene_name" << ";" << "score" << ";"
...
/**
 * \brief A small function to automatically build a hashmap containing genomic
 * offset bounds from fixed bounds over genomic templates.
 */
std::unordered_map<std::string, std::pair<int, int>> Aligner::build_genomic_bounds_map(
    int min_offset, int max_offset) const {
    unordered_map<string, pair<int, int>> genomic_offset_bounds;
 ...
 * @sequence
 */
void write_indexed_seq_csv(string filename,
                           vector<pair<const int, const string>> indexed_seq_list) {
    ofstream outfile(filename);
 ...
 * The structure of the file is assumed to be the same as the one created by the
 * Aligner::write_indexed_seq_csv method
 */
 ...
 * The structure of the file is assumed to be the same as the one created by the
 * Aligner::write_alignments_seq_csv method
 */
unordered_map<int, vector<Alignment_data>> read_alignments_seq_csv(string filename,
    ...
    while (getline(infile, line_str)) {
        ...
        // Define a scope
        {
            // Get the index of insertions from comma separated integers surrounded by
            // curly braces
...
read_alignments_seq_csv_score_range(string filename, Gene_class aligned_gene, double score_range,
    ...
    sorted_alignments = read_alignments_seq_csv_score_range(
        filename, aligned_gene, score_range, allow_in_dels, indexed_sequences, sorted_alignments);
    return sorted_alignments;
}
...
read_alignments_seq_csv_score_range(
    ...
    for (vector<pair<const int, const string>>::const_iterator seq_it = indexed_sequences.begin();
        ...
        for (vector<Alignment_data>::iterator align_it = (seq_alignments.end() - 1);
             align_it != (seq_alignments.begin() - 1); --align_it) {
            if ((*align_it).score < (max_score - score_range)) {
                seq_alignments.erase(align_it);
            }
        }
...
vector<tuple<int, string, unordered_map<Gene_class, vector<Alignment_data>>>> map2vect(
    unordered_map<int, pair<string, unordered_map<Gene_class, vector<Alignment_data>>>>
        alignments_map) {
...
/*
 * This method will incorporate gaps('-') at the places where insertion or
 * deletion occured both in the data and genomic sequence A deletion will
 * correspond to a gap introduced in the data sequence An insertion to a gap in
 * the genomic sequence
 * <\code>prev_dels<code> will indicate the shift in the data_seq offset induced
 * by the introduction of previous deletions
 *
 * The method returns the shift induced by introducing the deletions of this
 * alignment
 */
...
/**
 * This function compares nucleotides and output a boolean if they do not
 * necessarily imply an error (ambiguous nucleotides are thus treated in a loose
 * sense).
 */
        ...
        if ((nt_1 < 4) & (nt_2 < 4)) {
            ...
            switch (nt_1) {
                ...
                case int_W:
                    ...
                    break;
                case int_B:
                    if (nt_2 != int_A) {  // Of course this is in the hope that nt_2 is in
                        // the correct range of int
                        return true;
                    }
                    break;
                case int_D:
                    if (nt_2 != int_C) {  // Of course this is in the hope that nt_2 is in
                        // the correct range of int
                        return true;
                    }
                    break;
                case int_H:
                    if (nt_2 != int_G) {  // Of course this is in the hope that nt_2 is in
                        // the correct range of int
                        return true;
                    }
                    break;
                case int_V:
                    if (nt_2 != int_T) {  // Of course this is in the hope that nt_2 is in
                        // the correct range of int
                        return true;
                    }
                    break;
...
list<Int_nt> get_ambiguous_nt_list(const Int_nt &ambiguous_nt) {
    ...
    if ((ambiguous_nt == int_A) or (ambiguous_nt == int_C) or (ambiguous_nt == int_G) or
        (ambiguous_nt == int_T)) {
        nt_list.emplace_back(ambiguous_nt);
    } else {
        bool any_true = false;
        // Add an A for all cases implying an A	(do not add a break to allow for
        // execution of other cases and possibly add more letters to the list)
        if ((ambiguous_nt == int_R) or (ambiguous_nt == int_W) or (ambiguous_nt == int_M) or
            (ambiguous_nt == int_D) or (ambiguous_nt == int_H) or (ambiguous_nt == int_V) or
            (ambiguous_nt == int_N)) {
            nt_list.emplace_back(int_A);
            any_true = true;
        }
        // Same for C
        if ((ambiguous_nt == int_Y) or (ambiguous_nt == int_S) or (ambiguous_nt == int_M) or
            (ambiguous_nt == int_B) or (ambiguous_nt == int_H) or (ambiguous_nt == int_V) or
            (ambiguous_nt == int_N)) {
            nt_list.emplace_back(int_C);
            any_true = true;
        }
        // Same for G
        if ((ambiguous_nt == int_R) or (ambiguous_nt == int_S) or (ambiguous_nt == int_K) or
            (ambiguous_nt == int_B) or (ambiguous_nt == int_D) or (ambiguous_nt == int_V) or
            (ambiguous_nt == int_N)) {
            nt_list.emplace_back(int_G);
            any_true = true;
        }
        // Same for T
        if ((ambiguous_nt == int_Y) or (ambiguous_nt == int_W) or (ambiguous_nt == int_K) or
            (ambiguous_nt == int_B) or (ambiguous_nt == int_D) or (ambiguous_nt == int_H) or
            (ambiguous_nt == int_N)) {
 ...
 */
vector<pair<const int, const string>> sample_indexed_seq(
    vector<pair<const int, const string>> indexed_seqs, const size_t sample_size) {
    // Return an error if trying to sample more than the number of available
    // sequences
...
void Aligner::sw_align_common(const Int_Str &int_data_sequence, const Int_Str &int_genomic_sequence,
                              const int i, const int j, DoubleMatrixView score_matrix,
                              IntMatrixView row_memory_matrix, IntMatrixView col_memory_matrix,
                              IntMatrixView alignment_numb_tracker, vector<int> &max_score,
    ...
    if ((subs_score >= data_gap_score) & (subs_score >= genomic_gap_score) &
        ...
        // No real rational behind it just to avoid undefined behavior of the
        // aligner
...
/**
 *\brief Performs Smith-Waterman alignment between two sequences (translated to
 *int sequence as a prior) Output: int:Alignment score Alignment_data: comprises
 *offset, insertions and deletions locations. Note: the gene_name field of the
 *Alignment_data object is left blank and should be completed in a higher level
 *method
 */
list<pair<int, Alignment_data>> Aligner::sw_align(const Int_Str &int_data_sequence,
    ...
    if(int_genomic_sequence.size()>(-min_offset +
    int_data_sequence.size())){ short_int_genomic_sequence.erase((-min_offset +
    int_data_sequence.size()),string::npos);
    }
    }
    else{
    ...
    */
    ...
    std::vector<double> score_vec(n_rows * n_cols, 0.0);
    DoubleMatrixView score_matrix(score_vec.data(), static_cast<size_t>(n_rows),
                                  static_cast<size_t>(n_cols));

    std::vector<int> col_memory_vec(n_rows * n_cols, 0);
    IntMatrixView col_memory_matrix(col_memory_vec.data(), static_cast<size_t>(n_rows),
                                    static_cast<size_t>(n_cols));

    std::vector<int> row_memory_vec(n_rows * n_cols, 0);
    IntMatrixView row_memory_matrix(row_memory_vec.data(), static_cast<size_t>(n_rows),
                                    static_cast<size_t>(n_cols));

    std::vector<int> alignment_numb_tracker_vec(n_rows * n_cols, -1);
    IntMatrixView alignment_numb_tracker(alignment_numb_tracker_vec.data(),
    ...
    while (!matrix_complete) {
        ...
        // TODO test first if the whole genomic seq has been spanned (usually
        // shorter than the data seq??)

        // Always start at index 1 since first column and first raw are filled with
        // 0
    ...
    for (size_t align = 0; align != max_score.size(); ++align) {
        if (max_score[align] >= score_threshold) {
            ...
            // If sequence has been flip compute how the offset and insertion/deletion
            // should be changed
            int flip_factor;
            int flip_offset;
            int flip_mis;
            ...
            ofstream
            temp1(string("/home/quentin/Desktop/output_test/seq_gen_check/alignment_check/output_matrices/score_matrix.csv"));
            ...
            ofstream
            temp2(string("/home/quentin/Desktop/output_test/seq_gen_check/alignment_check/output_matrices/row_memory_matrix.csv"));
            for(int a = 0 ; a!=n_rows ; a++){
            for(int b =0 ; b!=n_cols ; b++){
            ...
            ofstream
            temp3(string("/home/quentin/Desktop/output_test/seq_gen_check/alignment_check/output_matrices/col_memory_matrix.csv"));
            for(int a = 0 ; a!=n_rows ; a++){
            for(int b =0 ; b!=n_cols ; b++){
            ...
            */
            // TODO correct this to get the alignment until the end (not just until
            // the best scoring nucl)
            ...
            // Offset is the place where the first letter of the genomic sequence
            // aligns if the alignment does not start from the beginning need to
            // extrapolate
            int offset = flip_factor * (i - j) + flip_offset + offset_change;

            if ((offset >= min_offset) &
                (offset <= max_offset)) {  // TODO reduce computation time by truncating the
                // alignment from the beginning?
                // TODO change this and use incorporate_in_dels(), should probably
                // change the list inside alignment data also to have the actual
                // corresponding indices
                // TODO return the actual inserted/deleted sequences in the alignment
                // data??
                Int_Str dat_seq;
                Int_Str gen_seq;
    ...
    while (getline(infile, temp_str)) {
        ...
        if (first_line) {
            if (separated_strings.size() < 2) {
                throw runtime_error(
                    "Expected at least two fields in read_gene_anchors_csv(). Make "
                    "sure file is separated by:\'" +
                    sep + "\'.");
            }
            ...
            try {
                ...
                throw runtime_error(
                    "Expected an integer for the second field in "
                    "read_gene_anchors_csv(), received:" +
...
unordered_map<string, pair<int, int>> read_template_specific_offset_csv(string filename,
    ...
    while (getline(infile, temp_str)) {
        ...
        if (first_line) {
            if (separated_strings.size() < 2) {
                throw runtime_error(
                    "Expected at least three fields in read_gene_anchors_csv(). Make "
                    "sure file is separated by:\'" +
                    sep + "\'.");
            }
            ...
            try {
                ...
                throw runtime_error(
                    "Expected an integer for the min offset (second "
                    "field) in read_gene_anchors_csv(), received:" +
                    separated_strings.at(1));
            }
            ...
            try {
                ...
                throw runtime_error(
                    "Expected an integer for the max offset (third "
                    "field) in read_gene_anchors_csv(), received:" +
...
Matrix<double> read_substitution_matrix(const string &filename, string sep /*=","*/) {
    ...
    while (getline(infile, temp_str)) {
        ...
        if (line_vect.size() != line_size) {
            throw runtime_error(
                "Substitution matrix' rows length are inconsistent "
                "in input matrix for read_substitution_matrix()");
        }
    ...
    if (tmp_vect.size() / line_size != line_size) {
        throw runtime_error(
            "Substitution matrix' number of rows and columns are inconsistent in "
            "input matrix for read_substitution_matrix()");
    }
    return Matrix<double>(line_size, line_size, tmp_vect);
}

/**
 * \brief Compute min and max offsets over all genomic templates and check if
 * they are constant.
 * \author Q.Marcou
 * \version 1.2.0
 *
 ...
 * upper bounds for each genomic template. Keys of the map are the genomic
 * templates names.
 * \return A three component tuple: first: boolean true if offsets are template
 * specific (not all the same for all templates, second and third the min and
 * max offsets over all templates.
 */
tuple<bool, int, int> extract_min_max_genomic_templates_offsets(
    ...
    // Compute min and max offsets over all genomic templates and check if they
    // are constant.
    int min_offset = INT32_MAX;
    int max_offset = INT32_MIN;
    bool template_specific_offsets =
        false;  // Although silly we have to recover the fact that not all
    // templates have the same offset.
    for (const pair<string, pair<int, int>> template_bounds : genomic_offset_bounds) {
        if (min_offset > template_bounds.second.first) {
            if (not template_specific_offsets and min_offset != INT32_MAX) {
                // If min_offset's values is no longer UINT64_MAX, it has been updated
                // once and this is the second => templates have different bounds
        ...
        if (max_offset < template_bounds.second.second) {
            if (not template_specific_offsets and max_offset != INT32_MIN) {
                // If max_offset's values is no longer -UINT64_MAX, it has been updated
                // once and this is the second => templates have different bounds
                template_specific_offsets = true;
            }
 ...
 * \version 1.2.1
 *
 * \param [in] all_aligns A forward list containing alignments for several
 * genes/alleles.
 * \return A forward list containing only alignments of the best gene/allele
 * candidate.
 */
forward_list<Alignment_data> extract_best_gene_alignments(
    const forward_list<Alignment_data> &all_aligns) {
    ...
    for (Alignment_data alignment : all_aligns) {
        if (alignment.score > best_align_score) {
            ...
            if (best_genes_names.count(alignment.gene_name) == 0) {
                // Make sure gene name is not already contained in the set before adding
                // it
                best_genes_names.emplace(alignment.gene_name);
            }
        }
