...
Model_marginals::Model_marginals(const Model_Parms &model_parms) {
    ...
    if (marginal_arr_size == 0) {
        throw runtime_error(
            "provided Model_parms imply empty marginals in "
            "Model_marginals::Model_marginals(const Model_Parms& model_parms)");
    }
 ...
 */
Model_marginals::Model_marginals(const Model_marginals &other)
    : marginal_array_smart_p(Marginal_array_p(new long double[other.marginal_arr_size])),
      marginal_arr_size(other.marginal_arr_size) {
...
Model_marginals &Model_marginals::invert_edge(Rec_Event_name ev1_name, Rec_Event_name ev2_name,
     ...
     * There is no good way of checking that those marginals and those parms are
     * related...
    ...
    if (model_parms.has_edge(ev1_name, ev2_name)) {
        ...
        throw runtime_error("Model_marginals::invert_edge() : the edge between \"" + ev1_name +
                            "\" and \"" + ev2_name + "\" does not exist");
    }

    const unordered_map<Rec_Event_name, int> orig_marginals_index_map =
        this->get_index_map(model_parms);
    const unordered_map<Rec_Event_name, list<pair<shared_ptr<const Rec_Event>, int>>>
        orig_marginals_inverse_offset_map = this->get_inverse_offset_map(model_parms);
    ...
    try {
        ...
        cerr << "Exception caught trying to invert an edge on a test model parms "
                "in Model_marginals::invert_edge, the operation is most likely "
                "creating a cycle, throwing exception now..."
             << endl;
        throw(e);
    }
     ...
     * To do so we marginalize the joint probability of this event and the former
     * parent over the former parent realizations This simply requires to
     * marginalize the probability of the former parent over all events that are
     * not parents of the former child
     */
    ...
    child_kept_dependencies.emplace(child_ptr->get_name());  // Add the child in order to get an
                                                             // array of the correct size directly

    pair<list<pair<Rec_Event_name, size_t>>, shared_ptr<long double>>
        marginalized_parent_probabilities = compute_event_marginal_probability(
            parent_ptr->get_name(), child_kept_dependencies, model_parms);

    /*cout<<"Model_marginals::invert_edge()"<<endl;
    size_t tmp_size = 1;
    for(pair<Rec_Event_name,size_t> tmp :
    marginalized_parent_probabilities.first){ cout<<tmp.first<<",";
                                    tmp_size*=tmp.second;
    }
    ...
    list<pair<shared_ptr<const Rec_Event>, int>> sorted_inv_offset_list =
        orig_marginals_inverse_offset_map.at(child_ptr->get_name());
    ...
    for (const pair<shared_ptr<const Rec_Event>, int> &inv_offset : sorted_inv_offset_list) {
        child_dependencies_order_list.emplace_back(inv_offset.first->get_name(),
                                                   inv_offset.first->size());
    }
    ...
    pair<list<pair<Rec_Event_name, size_t>>, shared_ptr<long double>> tmp_pair =
        make_pair(child_dependencies_order_list, joint_child_array);
     ...
     * To do so we need to marginalize the former child distribution over all
     * unshared parents except the former parent Multiply by the the former parent
     * proba to get the joint Divide by the former child distribution marginalized
     * over all unshared parents (including the former parent) for the bayesian
     * inversion
     */
    ...
    pair<list<pair<Rec_Event_name, size_t>>, shared_ptr<long double>>
        marginalized_child_probabilities = compute_event_marginal_probability(
            child_ptr->get_name(), parent_kept_dependencies, model_parms);
    // By removing the dependence on the former parent we lose one dimension and
    // we thus add it by hand Compute the total size
    ...
    shared_ptr<long double> marginalized_child_proba_expanded_arr(
        new long double[new_child_array_size]);
    for (size_t i = 0; i != new_child_array_size; ++i) {
        marginalized_child_proba_expanded_arr.get()[i] =
            marginalized_child_probabilities.second
                .get()[i % (new_child_array_size / parent_ptr->size())];
    }
    ...
    cout<<endl;
    cout<<"Sanity check on the marginalized child expanded pointer
    transfer:"<<endl; for(size_t i=0 ; i!=new_child_array_size ; ++i){
    ...
    pair<list<pair<Rec_Event_name, size_t>>, shared_ptr<long double>>
        marginalized_child_g_parent_probabilities = compute_event_marginal_probability(
            child_ptr->get_name(), parent_kept_dependencies, model_parms);
    ...
    // Now extract the former marginal values and expand the dimension with the
    // former child
    ...
    for (size_t i = 0; i != new_child_array_size; ++i) {
        new_parent_array.get()[i] =
            this->marginal_array_smart_p
                .get()[parent_index + i % (new_child_array_size / child_ptr->size())];
    ...
    // Now add dimensions in the correct order
    for (const pair<shared_ptr<const Rec_Event>, int> &inv_offset : sorted_inv_offset_list) {
        parents_dependencies_order_list.emplace_back(inv_offset.first->get_name(),
                                                     inv_offset.first->size());
    }
    ...
    const unordered_map<Rec_Event_name, int> new_marginals_index_map =
        this->get_index_map(model_parms);
    const unordered_map<Rec_Event_name, list<pair<shared_ptr<const Rec_Event>, int>>>
        new_marginals_inverse_offset_map = this->get_inverse_offset_map(model_parms);
    ...
    // Now add dimensions in the correct order
    for (const pair<shared_ptr<const Rec_Event>, int> &inv_offset : sorted_inv_offset_list) {
        final_new_child_ordering_list.emplace_back(inv_offset.first->get_name(),
                                                   inv_offset.first->size());
    }
    ...
    pair<list<pair<Rec_Event_name, size_t>>, shared_ptr<long double>> tmp_pair_parent =
        make_pair(parents_dependencies_order_list, new_parent_array);
    ...
    */
    ...
    for (size_t i = 0; i != new_child_array_size; ++i) {
        if (marginalized_child_probabilities.second.get()[i] != 0) {
            new_parent_array.get()[i] *= marginalized_child_g_parent_probabilities.second.get()[i] /
                                         marginalized_child_probabilities.second.get()[i];
        } else {
    ...
    for (const shared_ptr<Rec_Event> event_ptr : model_parms.get_event_list()) {
        if ((event_ptr->get_name() != ev1_name) and (event_ptr->get_name() != ev2_name)) {
            ...
            for (size_t i = 0; i != tmp_event_size; ++i) {
                new_marginal_array[new_event_index + i] =
                    this->marginal_array_smart_p[former_event_index + i];
            }
        }
    }
 ...
 *
 */
size_t Model_marginals::get_event_size(shared_ptr<const Rec_Event> event_p,
 ...
 * FIXME make sure the marginals are normalized? otherwise defined up to a
 * multiplicative constant??
 *
 * Compute the event marginal probability by recursion
 *
 * should return a list of offsets (maybe it would be better to return the event
 * sizes) and corresponding events
 */
pair<list<pair<Rec_Event_name, size_t>>, shared_ptr<long double>>
Model_marginals::compute_event_marginal_probability(Rec_Event_name event_name,
                                                    const Model_Parms &model_parms) const {
    return this->compute_event_marginal_probability(event_name, set<Rec_Event_name>(), model_parms);
}

pair<list<pair<Rec_Event_name, size_t>>, shared_ptr<long double>>
Model_marginals::compute_event_marginal_probability(
    Rec_Event_name event_name, const set<Rec_Event_name> &kept_dependencies_list,
    const Model_Parms &model_parms) const {
    ...
    const unordered_map<Rec_Event_name, vector<pair<shared_ptr<const Rec_Event>, int>>> offset_map =
        this->get_offsets_map(model_parms);
    const unordered_map<Rec_Event_name, list<pair<shared_ptr<const Rec_Event>, int>>>
        inverse_offset_map = this->get_inverse_offset_map(model_parms);
    return this->compute_event_marginal_probability(event_name, kept_dependencies_list, model_parms,
                                                    index_map, offset_map, inverse_offset_map);
}
...
Model_marginals::compute_event_marginal_probability(
    ...
    /*	cout<<"Model_marginals::compute_event_marginal_probability()"<<endl;
                                    cout<<"Marginalized event: "<<event_name<<endl<<"Kept
       dependencies: "; for(Rec_Event_name ev_name : kept_dependencies_list){ cout<<ev_name<<",";
                                    }
                                    cout<<endl;*/
    ...
    if (parents_list.empty()) {
        /*
         * If the event has no parents then the probabilities contained on the array
         * are already the marginal probabilities This condition must be met
         * recursively by reaching a root of the graph (this is ensured by the
         * acyclicity of the graph) However the array still needs to be copied
         * several times to match the kept dependencies format
         */

        // Simply copy the values in the array (the number of times required by the
        // kept dependencies)
        for (size_t i = 0; i != new_array_size; ++i) {
            marginal_proba_ptr.get()[i] =
                this->marginal_array_smart_p[event_index + (i % event_size)];
        }
        // Emplace Event order
        dependencies_order_list.emplace_back(event_name, event_size);
        for (Rec_Event_name ev_name : kept_dependencies_list) {
            if (ev_name != event_name) {
                dependencies_order_list.emplace_back(
                    ev_name, model_parms.get_event_pointer(ev_name)->size());
            }
        }
    } else {
        // Compute event marginal array size(including kept dependencies) and create
        // an array on which we compute the joint probabilities
        size_t marginal_event_size = event_size;

        // Make a copy of the kept dependencies and remove the parents and the event
        // itself from it in order to avoid duplicate dimensions
        set<Rec_Event_name> tmp_kept_dependencies_utility = kept_dependencies_list;

        /*
         * Here we get the list of dimensions in the correct order with respect to
         * the existing marginals We use the inverse offset map for it and the fact
         * that the event itself is always the first dimension
         */
        // Add the event itself as a first array dimension (since it always is the
        // first dimension according to the offset map construction)
        dependencies_order_list.emplace_back(event_name, event_size);
        ...
        // Create a list containing all parents if they are not already in the kept
        // dependencies (avoid duplicates) Use the inverse offset map to know the
        // correct dimension ordering Disclaimer: this is going to be quite ugly and
        // should be rewritten //TODO

        // First copy a list containing the inverse offsets and sort it
        list<pair<shared_ptr<const Rec_Event>, int>> sorted_inv_offset_list =
            inverse_offset_map.at(event_name);
        sorted_inv_offset_list.sort(inverse_offset_comparator());

        /*cout<<"Inverse offset list: ";
        for(const pair<shared_ptr<const Rec_Event>,int>& inv_offset :
        sorted_inv_offset_list){
        ...
        // Now add dimensions in the correct order
        for (const pair<shared_ptr<const Rec_Event>, int> &inv_offset : sorted_inv_offset_list) {
            dependencies_order_list.emplace_back(inv_offset.first->get_name(),
                                                 inv_offset.first->size());
        ...
        // Now append kept dependencies that are not parents or the event itself as
        // extra dimensions
        for (Rec_Event_name kept_dep_name : tmp_kept_dependencies_utility) {
            dependencies_order_list.emplace_back(
                kept_dep_name, model_parms.get_event_pointer(kept_dep_name)->size());
            marginal_event_size *= model_parms.get_event_pointer(kept_dep_name)->size();
        }

        // Instantiate a large array to record the full joint proba with kept
        // dependencies
        shared_ptr<long double> joint_proba_array(new long double[marginal_event_size]);

        // Compute the joint probabilities
        // First copy the conditionals from the marginals (again this assumes we
        // have the dimensions in the right order, extra kept dependencies are just
        // copies)
        size_t event_original_marginal_array_size =
            this->get_event_size(model_parms.get_event_pointer(event_name), model_parms);
        for (size_t i = 0; i != marginal_event_size; ++i) {
            joint_proba_array.get()[i] =
                this->marginal_array_smart_p
                    [event_index + i % event_original_marginal_array_size];  // THIS IS ASSUMING AN
                                                                             // ORDER FOR THE
                                                                             // MARGINALS, NEED TO
                                                                             // MAKE SURE IT IS
                                                                             // CORRECT
        ...
        /*
         * Here we call recursively the compute_event_marginal_probability function
         * on every parent We append the full list of parents (necessary if some
         * joint ancestors exist, or if one parent is ancestor of the other) to the
         * ket dependencies We also append the event itself: although it cannot be
         * an ancestor of the parents (acyclic graph) it will force to output an
         * array of the correct size Note that this might result in very big arrays
         * if the graph is big, and might turn out to be inefficient //TODO compute
         * overlap with ancestors and reexpand after marginalization
         */
        ...
        for (shared_ptr<Rec_Event> parent_event : parents_list) {
            ...
            pair<list<pair<Rec_Event_name, size_t>>, shared_ptr<long double>>
                parent_marginal_proba = compute_event_marginal_probability(
                    parent_event->get_name(), new_kept_dependencies_list, model_parms, index_map,
                    offset_map, inverse_offset_map);
            ...
            for (size_t i = 0; i != marginal_event_size; ++i) {
                joint_proba_array.get()[i] *=
                    parent_marginal_proba.second
                        .get()[i];  // THIS IS ASSUMING AN ORDER FOR THE MARGINALS? NEED
        ...
        // Now re-organize the joint in order to have all necessary dimensions
        // before marginalized ones
        list<pair<Rec_Event_name, size_t>> final_dimensions_order_list;
        final_dimensions_order_list.emplace_back(
            event_name, event_size);  // Put the considered event in first position
        for (Rec_Event_name ev_name : kept_dependencies_list) {
            if (ev_name != event_name) {
                final_dimensions_order_list.emplace_back(
                    ev_name, model_parms.get_event_pointer(ev_name)->size());
        ...
        pair<list<pair<Rec_Event_name, size_t>>, shared_ptr<long double>> tmp_pair =
            make_pair(dependencies_order_list, joint_proba_array);
        align_marginal_array(final_dimensions_order_list, tmp_pair);
        // Now reassign the correct pointer since i has been changed in the function
        // and make_pair made a copy of teh smart pointer
        ...
        // Now from the joint compute the marginal joint probability (sum all extra
        // dimensions)
        for (size_t i = 0; i != marginal_event_size; ++i) {
...
Model_marginals &Model_marginals::operator+=(Model_marginals marginals) {
    if (this->marginal_arr_size != marginals.marginal_arr_size) {
        throw invalid_argument(
            "Model_marginals must have the same size in : "
            "Model_marginals::operator+=");
    } else {
...
Model_marginals::get_inverse_offset_map(const Model_Parms &model_parms) const {
    queue<shared_ptr<Rec_Event>> model_queue = model_parms.get_model_queue();
    return get_inverse_offset_map(model_parms, model_queue);
}

unordered_map<Rec_Event_name, list<pair<shared_ptr<const Rec_Event>, int>>>
Model_marginals::get_inverse_offset_map(const Model_Parms &model_parms,
    ...
    unordered_map<Rec_Event_name, list<pair<shared_ptr<const Rec_Event>, int>>> invert_offset_map =
        unordered_map<Rec_Event_name, list<pair<shared_ptr<const Rec_Event>, int>>>();
        ...
        if (!model_stack.empty()) {
            ...
            unordered_map<Rec_Event_name, shared_ptr<Rec_Event>> *related_events_map_p =
                new unordered_map<Rec_Event_name,
                                  shared_ptr<Rec_Event>>;  // stores all the
                                                           // related events
            unordered_map<Rec_Event_name, shared_ptr<Rec_Event>> related_events_map =
                *related_events_map_p;

            if (!model_parms.get_parents(current_event_point).empty()) {
                list<shared_ptr<Rec_Event>> event_parents =
                    model_parms.get_parents(current_event_point);

                for (list<shared_ptr<Rec_Event>>::const_iterator iter = event_parents.begin();
                     iter != event_parents.end(); ++iter) {
 ...
 * E.g: the choice of a V (say V3) gene will influence the position on the array
 * where p(ins|V) must be written regardless of the number of insertions.
 */
unordered_map<Rec_Event_name, vector<pair<shared_ptr<const Rec_Event>, int>>>
Model_marginals::get_offsets_map(const Model_Parms &model_parms) const {
    queue<shared_ptr<Rec_Event>> model_queue = model_parms.get_model_queue();
    return get_offsets_map(model_parms, model_queue);
}

unordered_map<Rec_Event_name, vector<pair<shared_ptr<const Rec_Event>, int>>>
Model_marginals::get_offsets_map(const Model_Parms &model_parms,
                                 queue<shared_ptr<Rec_Event>> model_queue) const {
    unordered_map<Rec_Event_name, list<pair<shared_ptr<const Rec_Event>, int>>> invert_offset_map =
        get_inverse_offset_map(model_parms, model_queue);

    // Inversion of the map
    unordered_map<Rec_Event_name, vector<pair<shared_ptr<const Rec_Event>, int>>>
        offset_map;  // = *(new
                     // unordered_map<Rec_Event_name,list<pair<shared_ptr<const
                     // Rec_Event>,int>>>);

    for (unordered_map<Rec_Event_name, list<pair<shared_ptr<const Rec_Event>, int>>>::const_iterator
             iter = invert_offset_map.begin();
         iter != invert_offset_map.end(); ++iter) {
        for (list<pair<shared_ptr<const Rec_Event>, int>>::const_iterator jiter =
                 (*iter).second.begin();
             jiter != (*iter).second.end(); ++jiter) {
            /*if(offset_map.count( (*jiter).first->get_name() ) == 0){
                                            offset_map.emplace( (*jiter).first->get_name() , *(new
            list<pair<shared_ptr<const Rec_Event>,int>>) );
            }*/

            offset_map[(*jiter).first->get_name()].push_back(
                make_pair(model_parms.get_event_pointer((*iter).first), (*jiter).second));
        }
    }
    return offset_map;
}

unordered_map<Rec_Event_name, int> Model_marginals::get_index_map(
    const Model_Parms &model_parms) const {
    queue<shared_ptr<Rec_Event>> model_queue = model_parms.get_model_queue();
...
unordered_map<Rec_Event_name, int> Model_marginals::get_index_map(
    const Model_Parms &model_parms, queue<shared_ptr<Rec_Event>> model_queue) const {
    int index = 0;
    stack<shared_ptr<Rec_Event>> *model_stack_p = new stack<shared_ptr<Rec_Event>>;
    stack<shared_ptr<Rec_Event>> model_stack = *model_stack_p;
    ...
    while (!model_queue.empty()) {
        // The index added in the map have been computed at the previous iteration
        // (is 0 if first event in the queue)
        ...
        //We look for all events upstream (parents)
        unordered_map<Rec_Event_name,shared_ptr<Rec_Event>>*
        related_events_map_p = new
        unordered_map<Rec_Event_name,shared_ptr<Rec_Event>>; //stores all the
        related events unordered_map<Rec_Event_name,shared_ptr<Rec_Event>>
        related_events_map = *related_events_map_p;

        if(!model_parms.get_parents(
        current_event_point ).empty()){ list<shared_ptr<Rec_Event>> event_parents =
        model_parms.get_parents(current_event_point);

        for(list<shared_ptr<Rec_Event>>::const_iterator
        iter=event_parents.end() ; iter!= event_parents.end() ; iter++){
        related_events_map.insert(make_pair((*iter)->get_name(),*iter));
        }

        }
        //Copy model stack to have a modifiable copy
        //TODO model stack might be more complicated
        than a simple list stack<shared_ptr<Rec_Event>> model_stack_copy =
        model_stack;


        //Compute the event size:
        total size needed on the array to store informations on an event and the ones that it
        depends on while(!model_stack_copy.empty()){
        if(related_events_map.count(model_stack_copy.top()->get_name())!=0){
        event_size *=
        model_stack_copy.top()->size();
        }
        model_stack_copy.pop();
        }
        delete related_events_map_p;
        ...
        // This gives the pointer for the beginning of the next event(thus it will
        // be inserted in the map at the next iteration)
        index += event_size;
        model_stack.push(model_queue.front());
        model_queue.pop();
    }
...
void Model_marginals::normalize(
    unordered_map<Rec_Event_name, list<pair<shared_ptr<const Rec_Event>, int>>> inverse_offset_map,
    unordered_map<Rec_Event_name, int> index_map, queue<shared_ptr<Rec_Event>> model_queue) {
    while (!model_queue.empty()) {
        shared_ptr<Rec_Event> current_event_point = model_queue.front();
        list<pair<shared_ptr<const Rec_Event>, int>> related_events;
        if (inverse_offset_map.count(current_event_point->get_name()) == 0) {
            related_events = list<pair<shared_ptr<const Rec_Event>, int>>();  // TODO change this to
                                                                              // prevent memory leak
        } else {
...
void Model_marginals::iterate_normalize(
    shared_ptr<const Rec_Event> current_event_point,
    list<pair<shared_ptr<const Rec_Event>, int>> &related_events, int index, int current_offset) {
    if (related_events.empty()) {
        /*double sum_marginals = 0;
        for(int iter =0 ; iter != current_event_point->size() ; iter++){
                                        sum_marginals+= this->marginal_array_p[index +
        current_offset + iter];
        }
        if(sum_marginals!=0){
                                        for(int iter =0 ; iter != current_event_point->size() ;
        iter++){ this->marginal_array_p[index + current_offset + iter] /= sum_marginals;
                                        }
        }*/
        ...
        for (size_t iter = 0; iter != processed_event_size; ++iter) {
            ...
            iterate_normalize(current_event_point, related_events_copy, index,
                              new_offset);  // TODO might be something fishy here
        }
    }
}

void Model_marginals::copy_fixed_events_marginals(
    const Model_marginals &source_marginals, const Model_Parms &parms,
...
/*
 * This method initializes the marginal array with uniform probability for each
 * event.
 *
 */
void Model_marginals::uniform_initialize(const Model_Parms &parms) {
    ...
    unordered_map<Rec_Event_name, list<pair<shared_ptr<const Rec_Event>, int>>> inverse_offset_map =
        get_inverse_offset_map(parms, model_queue);
 ...
 *
 * //TODO recode this in order to be able to set several realizations probas at
 * the same time
 * //FIXME if the supplied new value is 1.0 there will be a zero division issue
 * (this could be fixed by fixing all others to 0 instead of trying to set the
 * supplied one to 1)
 */
void Model_marginals::set_realization_proba(string realization_name,
                                            shared_ptr<const Rec_Event> event_ptr, double new_value,
                                            const Model_Parms &model_parms) {
    if ((new_value < 0) or (new_value > 1)) {
        throw runtime_error("Invalid new probability value \"" + to_string(new_value) +
                            "\" in Model_marginals::set_realization_proba()");
    }
    ...
    if (event_ptr->get_realizations_map().count(realization_name) <= 0) {
        throw runtime_error("Unknown realization \"" + realization_name + "\" for event " +
                            event_ptr->get_name() + " in Model_marginals::set_realization_proba");
    }
    ...
    // Get the summed probabilities for all othe realizations for every
    // conditioning
    ...
    auto event_view =
        this->get_event_view(event_index, marginal_event_size / event_size, event_size);
    for (size_t i = 0; i != marginal_event_size / event_size; ++i) {
        event_view(i, real_index) = summed_probas[i];
    }
...
void Model_marginals::write2txt_iteration(
    list<pair<shared_ptr<const Rec_Event>, int>>::const_iterator iter,
    const list<pair<shared_ptr<const Rec_Event>, int>>::const_iterator iter_end, int index,
    ofstream &outfile, shared_ptr<Rec_Event> current_event_p, list<string> &header) {
    if (iter != iter_end) {
        for (int i = 0; i != (*iter).first->size(); ++i) {
            list<string> header_copy = header;
            header_copy.push_back(string("[") + (*iter).first->get_nickname() + string(",") +
                                  to_string(i) + string("]"));
            int new_index = index + i * (*iter).second;
            list<pair<shared_ptr<const Rec_Event>, int>>::const_iterator iter_copy = iter;
            ++iter_copy;
            write2txt_iteration(iter_copy, iter_end, new_index, outfile, current_event_p,
                                header_copy);
        }
    } else {
        outfile << "#";
...
void Model_marginals::txt2marginals(string filename, const Model_Parms &parms) {
    ...
    if (size_counter != current_marginals_size) {
        throw runtime_error("Marginals contained in file \"" + filename +
                            "\" and supplied Model_Parms do not match in size. "
                            "Make sure the Bayesian Network structure/event "
                            "realizations  and the marginals are coherent.");
    }
    ...
    while (getline(infile, line_str)) {
        if (line_str[0] == '%') {
            ...
            while (semicolon_index != string::npos) {
                size_t next_comma_index = line_str.find(",", (semicolon_index + 1));
                this->marginal_array_smart_p[index] = stod(line_str.substr(
                    (semicolon_index + 1), (next_comma_index - semicolon_index - 1)));
                semicolon_index = next_comma_index;
                ++index;
            }
        }
    ...
    // Make sure marginals are normalized (deals with problem of float precision
...
/**
 * A utility function to swap the order of the events on a marginal array (used
 * to marginalize and invert edges) This is used to further align the marginals
 * and combine them
 */
void swap_events_order(
    const Rec_Event_name event_1, const Rec_Event_name event_2,
    pair<list<pair<Rec_Event_name, size_t>>, shared_ptr<long double>> &swapped_marginals) {
    ...
    for (list<pair<Rec_Event_name, size_t>>::iterator iter = swapped_marginals.first.begin();
         iter != swapped_marginals.first.end(); ++iter) {
    ...
    if (not(event_1_found and event_2_found)) {
        throw runtime_error(event_1 + " and " + event_2 +
                            " not found on the array, in swap_events_order()");
    }

    // In case event 1 and 2 were the same events, no change
    if (event_1_position == event_2_position) {
        return;
    }

    // Then swap neighbors with the first event until it reaches the correct
    // position
    ...
    while (event_1_position != event_2_position) {
        ...
        // swap_neighboring_events_order(event_1 ,
        // (event_1_iterator+increment_factor)->first , swapped_marginals);
        swap_neighboring_events_order(event_1, next_event_iter->first, swapped_marginals);
        event_1_position += increment_factor;
    }
    // Adjust event_2_position since during the last move it has been swapped with
    // event_1
    ...
    while (event_2_position != initial_event_1_position) {
        ...
        // swap_neighboring_events_order(event_1 ,
        // (event_2_iterator+increment_factor)->first , swapped_marginals);
...
void swap_neighboring_events_order(
    ...
    for (list<pair<Rec_Event_name, size_t>>::iterator iter = swapped_marginals.first.begin();
        ...
        current_offset *= iter->second;  // Need to get the total array size, thus
    ...
    if (event_1_found) {
        ...
        if (event_2_iterator->first != event_2) {
            throw runtime_error(event_1 + " and " + event_2 +
                                " are not neighbors, in swap_neighboring_events_order()");
        }
        ...
        throw runtime_error(event_1 + " and " + event_2 +
                            " not found on the array, in swap_neighboring_events_order()");
    }

    shared_ptr<long double> new_array_ptr(
        new long double[current_offset]);  // Current offset is the total size of
                                           // the array after looping over all
                                           // events

    for (size_t i = 0; i != current_offset; ++i) {  // Current offset has been used to compute the
                                                    // total size of the array
        if (event_1_found) {
            // event 1 used to have
        }
        size_t small_offset = min(event_1_new_offset, event_2_new_offset);
        size_t big_offset = max(event_1_new_offset * event_1_iterator->second,
                                event_2_new_offset * event_2_iterator->second);
        /*
         * Now invert the marginals
         * (i/former_offset)%size corresponds to the realization index
         * i%small offset are all lower dependencies that need to be copied in the
         * same order (lower dimensions) big offset denotes the offset of stuff
         * depending on the two events (higher dimensions)
         *
         */
    ...
    // Instead i'll insert the two elements in the swapped order after the 2
    // elements in the right order I will then delete the two unswapped elements
    /*	if(event_1_found){
                                    //swapped_marginals.first.insert(event_2_iterator+1,event_1_iterator,event_2_iterator+1);
                                                                    ...
                                                                    list<pair<Rec_Event_name,size_t>>::iterator
 tmp_iter = event_2_iterator;
                                                                    ++tmp_iter;
 ...
 * Note that the implementation is probably not efficient but at least remains
 * simple Can align marginals with higher number of dimensions than the
 * reference
 */
void align_marginal_array(
    const list<pair<Rec_Event_name, size_t>> &reference_marginals_order,
    pair<list<pair<Rec_Event_name, size_t>>, shared_ptr<long double>> &aligned_marginals) {
    ////////////////////////////////////////////////////
    ...
    if (reference_marginals_order.size() > aligned_marginals.first.size()) {
        throw runtime_error(
            "Aligned marginals have less dimensions than the "
            "reference marginals in align_marginal_array()");
    }
    list<pair<Rec_Event_name, size_t>>::const_iterator reference_iterator =
        reference_marginals_order.cbegin();
    size_t counter = 0;
