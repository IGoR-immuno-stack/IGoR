...
Rec_Event::Rec_Event(Gene_class gene, Seq_side side)
    : priority(0), event_class(gene), event_side(side),
      name("Undefined_event_name"), len_min(INT16_MAX), len_max(INT16_MIN),
      type(Undefined_t), event_index(INT16_MIN), updated(false), fixed(false),
      current_realizations_index_vec(vector<int>()),
      scenario_downstream_upper_bound_proba(-1), event_upper_bound_proba(-1),
      scenario_upper_bound_proba(-1), current_realization_index(nullptr) {
...
Rec_Event::Rec_Event(list<Event_realization> realizations_list){
        for(list<Event_realization>::const_iterator iter =
realizations_list.begin() ; iter!=realizations_list.end() ; iter++){
                this->add_realization((*iter));
        }
}


Rec_Event::Rec_Event(list<Event_realization> realization_list, int new_priority
) : Rec_Event(realization_list) { this->priority = new_priority;
}
*/
...
void Rec_Event::update_event_name() {
  this->name = string() + this->type + string("_") + this->event_class +
               string("_") + this->event_side + string("_prio") +
               to_string(priority) + string("_size") + to_string(this->size());
}
...
int Rec_Event::size() const { return event_realizations.size(); }

void Rec_Event::set_event_identifier(size_t identifier) {
  this->event_index = identifier;
}

int Rec_Event::get_event_identifier() const { return event_index; }
...
void Rec_Event::iterate_wrap_up(
  ...
  /*			if(seq_offsets.count(make_pair(J_gene_seq,
     Three_prime))!=0){
                                          ...
                                          int
     offset3=seq_offsets.at(make_pair(J_gene_seq, Three_prime)); int
     offset5=seq_offsets.at(make_pair(J_gene_seq, Five_prime)); if(offset5==0){
                                                  cout<<"problem"<<endl;
                                          }
                                          if(offset3==0){
  ...
  if (next_event_ptr_arr.get()[this->event_index]) { // Tests whether the next
    ...
    next_event_ptr_arr.get()[this->event_index]->iterate(
        scenario_proba, downstream_proba_map, sequence, int_sequence, index_map,
        offset_map, next_event_ptr_arr, updated_marginal_array_p,
    ...
    long double scenario_error_w_proba =
        error_rate_p->compare_sequences_error_prob(
            scenario_proba, sequence, constructed_sequences, seq_offsets,
            events_map, mismatches_lists, seq_max_prob_scenario,
    ...
    /*for(forward_list<int*>::const_iterator iter = write_index_list.begin() ;
    iter!=write_index_list.end() ; iter++){ updated_marginal_array_p[*(*iter)]
    += scenario_error_w_proba; if(((*iter)==2865) &
    (!constructed_sequences.at(VD_ins_seq).empty())){ cout<<"error"<<endl;
            }
            if(((*iter)==2896) &
    (!constructed_sequences.at(DJ_ins_seq).empty())){ cout<<"error"<<endl;
            }
    }*/

    if (scenario_error_w_proba >=
        seq_max_prob_scenario * proba_threshold_factor) {
      if (scenario_error_w_proba > seq_max_prob_scenario) {
        seq_max_prob_scenario = scenario_error_w_proba;
      }

      for (std::map<size_t, std::shared_ptr<Counter>>::iterator iter =
               counters_list.begin();
           iter != counters_list.end(); ++iter) {
        (*iter).second->count_scenario(
            scenario_error_w_proba, scenario_proba, sequence,
            constructed_sequences, seq_offsets, events_map, mismatches_lists);
      }
...
void Rec_Event::initialize_event(
  ...
  // Need to call Rec_Event::initialize_event() to apply these common actions
  // when the method is overloaded
  ...
  downstream_proba_map.get_all_current_memory_layer(
...
void Rec_Event::ind_normalize(Marginal_array_p &marginal_array_p,
...
void Rec_Event::set_crude_upper_bound_proba(
...
/**
 * Does nothing since in general events will not need to perform any operation
 * on the marginal probabilities
 */
void Rec_Event::update_event_internal_probas(
 ...
 * The point is to compute the upper bound probability (given the model) of the
 * scenario for each event This allows to discard scenarios with too low
 * probability at early stages
 */
void Rec_Event::initialize_crude_scenario_proba_bound(
  ...
  if (!this->is_updated()) {
    ...
    throw logic_error("Updated events should overload "
                      "Rec_event::initialize_scenario_proba_bound()");
  }
}
...
double *Rec_Event::get_updated_ptr() {
  throw logic_error(
      "Updated events should overload Rec_event::get_updated_ptr()");
}

/*
 * Updates the value of scenario_upper_bound_proba according to the error
 * weighted scenario and the upper bound of downstream scenarios
 */
void Rec_Event::compute_crude_upper_bound_scenario_proba(
    double &tmp_err_w_proba) {
  scenario_upper_bound_proba =
      tmp_err_w_proba * scenario_downstream_upper_bound_proba;
  for (forward_list<double *>::const_iterator iter =
           updated_proba_bounds_list.begin();
       iter != updated_proba_bounds_list.end(); ++iter) {
    scenario_upper_bound_proba *= (*(*iter));
  }
}

void Rec_Event::iterate_initialize_Len_proba(
  ...
  int seq_len = 0;
  this->iterate_initialize_Len_proba(
 ...
 * Fills up the length-max_proba_bound for a given junction , and links the call
 * to iterate_initialize_len_proba for two events
 *
