    ...
    int receptor_len)
{
    ...
    // The motif must fit within the receptor, starting at frame_offset. Without
    // this check the per-codon writes below run past the end of every field.
    if (frame_offset < 0) {
        throw invalid_argument(
            "motif_to_journaled_query: frame_offset must be non-negative, got " +
            to_string(frame_offset));
    }
    const long required = static_cast<long>(frame_offset) +
                          3L * static_cast<long>(masks.size());
    if (required > static_cast<long>(receptor_len)) {
        throw invalid_argument(
            "motif_to_journaled_query: motif of " + to_string(masks.size()) +
            " codons at frame_offset " + to_string(frame_offset) +
            " requires " + to_string(required) +
            " nt but receptor_len is " + to_string(receptor_len));
    }
