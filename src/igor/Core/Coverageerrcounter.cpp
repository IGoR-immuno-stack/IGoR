...
void Coverage_err_counter::initialize_counter(const Model_Parms &parms, const Model_marginals &marginals)
{
    ...
    if (count_on_v) {
        ...
        try {
            shared_ptr<Rec_Event> v_gene_event_base_p;
            if (!EventUtils::try_get_event(events_map, GeneChoice_t, V_gene_seq, Undefined_side, v_gene_event_base_p)) {
                throw runtime_error("Coverage_err_counter: V gene choice event is missing in events map");
            }
            v_gene_event_p = dynamic_pointer_cast<Gene_choice>(v_gene_event_base_p);
            if (!v_gene_event_p) {
                throw runtime_error("Coverage_err_counter: V gene choice event has unexpected concrete type");
            }
            vgene_offset_p = &v_gene_event_p->alignment_offset_p;
            vgene_real_index_p = &v_gene_event_p->current_realization_index;
