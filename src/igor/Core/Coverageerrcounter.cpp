...
Coverage_err_counter::~Coverage_err_counter()
{
    ...
    if (count_on_v) {
        this->deallocate_coverage_and_errors_arrays(n_v_real, v_realizations, v_gene_nucleotide_coverage_p,
                                                    v_gene_per_nucleotide_error_p, v_gene_nucleotide_coverage_seq_p,
                                                    v_gene_per_nucleotide_error_seq_p);
    }
    if (count_on_d) {
        this->deallocate_coverage_and_errors_arrays(n_d_real, d_realizations, d_gene_nucleotide_coverage_p,
                                                    d_gene_per_nucleotide_error_p, d_gene_nucleotide_coverage_seq_p,
                                                    d_gene_per_nucleotide_error_seq_p);
    }
    if (count_on_j) {
        this->deallocate_coverage_and_errors_arrays(n_j_real, j_realizations, j_gene_nucleotide_coverage_p,
...
void Coverage_err_counter::count_sequence(double seq_likelihood, const Model_marginals &single_seq_marginals,
                                          const Model_Parms &single_seq_model_parms)
{
    //Normalize by the sequence likelihood and clean counter if not all seq are dumped
    if (seq_likelihood != 0) {
        if (count_on_v) {
            this->normalize_and_add_cov_and_err(seq_likelihood, n_v_real, v_gene_nucleotide_coverage_p,
                                                v_gene_per_nucleotide_error_p, v_gene_nucleotide_coverage_seq_p,
                                                v_gene_per_nucleotide_error_seq_p);
        }
        if (count_on_d) {
            this->normalize_and_add_cov_and_err(seq_likelihood, n_d_real, d_gene_nucleotide_coverage_p,
                                                d_gene_per_nucleotide_error_p, d_gene_nucleotide_coverage_seq_p,
                                                d_gene_per_nucleotide_error_seq_p);
        }
        if (count_on_j) {
            this->normalize_and_add_cov_and_err(seq_likelihood, n_j_real, j_gene_nucleotide_coverage_p,
                                                j_gene_per_nucleotide_error_p, j_gene_nucleotide_coverage_seq_p,
                                                j_gene_per_nucleotide_error_seq_p);
        }
    }
...
void Coverage_err_counter::add_checked(shared_ptr<Counter> counter)
{
    ...
    if (count_on_v) {
        this->normalize_and_add_cov_and_err(identity, n_v_real, this->v_gene_nucleotide_coverage_p,
                                            this->v_gene_per_nucleotide_error_p, other->v_gene_nucleotide_coverage_p,
    ...
    if (count_on_d) {
        this->normalize_and_add_cov_and_err(identity, n_d_real, this->d_gene_nucleotide_coverage_p,
                                            this->d_gene_per_nucleotide_error_p, other->d_gene_nucleotide_coverage_p,
    ...
    if (count_on_j) {
        this->normalize_and_add_cov_and_err(identity, n_j_real, this->j_gene_nucleotide_coverage_p,
                                            this->j_gene_per_nucleotide_error_p, other->j_gene_nucleotide_coverage_p,
...
void Coverage_err_counter::dump_sequence_data(int seq_index, int iteration_n)
{
    if (dump_individual_seqs) {
        if (count_on_v) {
            this->dump_cov_and_err_arrays(iteration_n, seq_index, output_cov_err_v_file_ptr, n_v_real,
                                          v_gene_nucleotide_coverage_seq_p, v_gene_per_nucleotide_error_seq_p);
        }

        if (count_on_d) {
            this->dump_cov_and_err_arrays(iteration_n, seq_index, output_cov_err_d_file_ptr, n_d_real,
                                          d_gene_nucleotide_coverage_seq_p, d_gene_per_nucleotide_error_seq_p);
        }

        if (count_on_j) {
            this->dump_cov_and_err_arrays(iteration_n, seq_index, output_cov_err_j_file_ptr, n_j_real,
                                          j_gene_nucleotide_coverage_seq_p, j_gene_per_nucleotide_error_seq_p);
        }
    }
}
    ...
    if (not dump_individual_seqs) {
        if (count_on_v) {
            this->dump_cov_and_err_arrays(iteration_n, -1, output_cov_err_v_file_ptr, n_v_real,
                                          v_gene_nucleotide_coverage_p, v_gene_per_nucleotide_error_p);
        }

        if (count_on_d) {
            this->dump_cov_and_err_arrays(iteration_n, -1, output_cov_err_d_file_ptr, n_d_real,
                                          d_gene_nucleotide_coverage_p, d_gene_per_nucleotide_error_p);
        }

        if (count_on_j) {
            this->dump_cov_and_err_arrays(iteration_n, -1, output_cov_err_j_file_ptr, n_j_real,
                                          j_gene_nucleotide_coverage_p, j_gene_per_nucleotide_error_p);
        }
    }
}
...
void Coverage_err_counter::allocate_coverage_and_errors_arrays(
        ...
        pair<size_t, double *> *&gene_nucleotide_coverage_p, pair<size_t, double *> *&gene_per_nucleotide_error_p,
...
void Coverage_err_counter::dump_cov_and_err_arrays(int iteration_n, int seq_index, shared_ptr<ofstream> outfile_ptr,
...
void Coverage_err_counter::normalize_and_add_cov_and_err(double &normalizing_cst, size_t n_real,
                                                         pair<size_t, double *> *target_coverage_array_p,
                                                         pair<size_t, double *> *target_error_array_p,
                                                         pair<size_t, double *> *base_coverage_array_p,
 ...
 */
void Coverage_err_counter::recurs_coverage_count(double scenario_seq_joint_proba, size_t N, size_t begin_bound,
                                               ...
                                               size_t end_bound, size_t gene_len)
{
    for (size_t j = begin_bound; j != end_bound; ++j) {
        ...
        if (N < record_Npoint_occurence - 1) {
            this->recurs_errors_count(scenario_seq_joint_proba, mismatch_list, gene_offset_p, N + 1, j, end_bound,
                                      gene_len);
        } else {
            size_t adress = 0;
            for (size_t a = 0; a != record_Npoint_occurence; ++a) {
...
void Coverage_err_counter::symmetrize_counter_array(double *counter_array, size_t N, size_t begin_bound,
                                                    size_t gene_len)
{
...
void Coverage_err_counter::symmetrize_counter_array_recurs(size_t adress, size_t N, size_t *position_array,
