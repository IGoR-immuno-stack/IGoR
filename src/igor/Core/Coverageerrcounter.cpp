...
Coverage_err_counter::Coverage_err_counter(Gene_class count_on, bool dump_all_seq,
                                           bool last_iter_only)
...
Coverage_err_counter::Coverage_err_counter(string path, Gene_class count_on, size_t Npoint_count,
                                           bool dump_all_seq, bool last_iter_only)
    : Counter(path, last_iter_only),
      count_on(count_on),
      dump_individual_seqs(dump_all_seq),
      record_Npoint_occurence(Npoint_count),
      positions(NULL),
      n_v_real(0),
      n_d_real(0),
      n_j_real(0),
      v_gene_nucleotide_coverage_p(NULL),
      v_gene_per_nucleotide_error_p(NULL),
      d_gene_nucleotide_coverage_p(NULL),
      d_gene_per_nucleotide_error_p(NULL),
      j_gene_nucleotide_coverage_p(NULL),
      j_gene_per_nucleotide_error_p(NULL),
      v_gene_nucleotide_coverage_seq_p(NULL),
      ...
      j_gene_per_nucleotide_error_seq_p(NULL),
      vgene_offset_p(NULL),
      dgene_offset_p(NULL),
      jgene_offset_p(NULL),
                                              ...
                                              const Model_marginals &marginals)
{
    if (not fstreams_created) {
        ...
        if (count_on_v) {
            ...
            if (dump_individual_seqs) {
                (*this->output_cov_err_v_file_ptr.get())
                        << "iteration_n;seq_index;gene_index;coverage;errors" << endl;
            } else {
                (*this->output_cov_err_v_file_ptr.get())
                        << "iteration_n;gene_index;coverage;errors" << endl;
            }
        }
        ...
        if (count_on_d) {
            ...
            //Create the header
            if (dump_individual_seqs) {
                (*this->output_cov_err_d_file_ptr.get())
                        << "iteration_n;seq_index;gene_index;coverage;errors" << endl;
            } else {
                (*this->output_cov_err_d_file_ptr.get())
                        << "iteration_n;gene_index;coverage;errors" << endl;
        ...
        if (count_on_j) {
            ...
            //Create the header
            if (dump_individual_seqs) {
                (*this->output_cov_err_j_file_ptr.get())
                        << "iteration_n;seq_index;gene_index;coverage;errors" << endl;
            } else {
                (*this->output_cov_err_j_file_ptr.get())
                        << "iteration_n;gene_index;coverage;errors" << endl;
    ...
    if (count_on_v) {
        ...
        try {
            ...
            this->allocate_coverage_and_errors_arrays(
                    n_v_real, v_realizations, v_gene_nucleotide_coverage_p,
                    v_gene_per_nucleotide_error_p, v_gene_nucleotide_coverage_seq_p,
                    v_gene_per_nucleotide_error_seq_p);

        } catch (exception &except) {
            cout << "Exception caught during initialization of Hypermutation global error rate"
                 << endl;
        ...
        if (events_map.count(
                    tuple<Event_type, Gene_class, Seq_side>(Deletion_t, V_gene, Three_prime))
            != 0) {
            shared_ptr<const Deletion> v_3_del_event_p = dynamic_pointer_cast<Deletion>(
                    events_map.at(tuple<Event_type, Gene_class, Seq_side>(Deletion_t, V_gene,
                                                                          Three_prime)));
    ...
    if (count_on_d) {
        ...
        if (events_map.count(
                    tuple<Event_type, Gene_class, Seq_side>(Deletion_t, D_gene, Five_prime))
            != 0) {
            shared_ptr<const Deletion> d_5_del_event_p = dynamic_pointer_cast<Deletion>(
                    events_map.at(tuple<Event_type, Gene_class, Seq_side>(Deletion_t, D_gene,
                                                                          Five_prime)));
        ...
        if (events_map.count(
                    tuple<Event_type, Gene_class, Seq_side>(Deletion_t, D_gene, Three_prime))
            != 0) {
    ...
    if (count_on_j) {
        ...
        if (events_map.count(
                    tuple<Event_type, Gene_class, Seq_side>(Deletion_t, J_gene, Five_prime))
            != 0) {
            shared_ptr<const Deletion> j_5_del_event_p = dynamic_pointer_cast<Deletion>(
                    events_map.at(tuple<Event_type, Gene_class, Seq_side>(Deletion_t, J_gene,
                                                                          Five_prime)));
...
void Coverage_err_counter::count_scenario(
        long double scenario_seq_joint_proba, double scenario_probability,
        ...
        Mismatch_vectors_map &mismatches_lists)
{
    if (count_on_v) {
        ...
        this->recurs_errors_count(scenario_seq_joint_proba, v_mismatch_list, vgene_offset_p, 0, 0,
                                  tmp_len_util, tmp_corr_len);
    ...
    if (count_on_j) {
        ...
        //Get the length of the gene and a pointer to the right array to write on
        tmp_corr_len =
                j_gene_nucleotide_coverage_seq_p[**jgene_real_index_p].first; //Length of the J gene
        ...
        this->recurs_coverage_count(scenario_seq_joint_proba, 0, max(0, (*j_5_del_value_p)),
        ...
        this->recurs_errors_count(scenario_seq_joint_proba, j_mismatch_list, jgene_offset_p, 0,
                                  tmp_len_util, j_mismatch_list.size(), tmp_corr_len);
...
void Coverage_err_counter::count_sequence(double seq_likelihood,
                                          const Model_marginals &single_seq_marginals,
                                          const Model_Parms &single_seq_model_parms)
{
    //Normalize by the sequence likelihood and clean counter if not all seq are dumped
    if (seq_likelihood != 0) {
        if (count_on_v) {
            this->normalize_and_add_cov_and_err(
                    seq_likelihood, n_v_real, v_gene_nucleotide_coverage_p,
                    v_gene_per_nucleotide_error_p, v_gene_nucleotide_coverage_seq_p,
                    v_gene_per_nucleotide_error_seq_p);
        }
        if (count_on_d) {
            this->normalize_and_add_cov_and_err(
                    seq_likelihood, n_d_real, d_gene_nucleotide_coverage_p,
                    d_gene_per_nucleotide_error_p, d_gene_nucleotide_coverage_seq_p,
                    d_gene_per_nucleotide_error_seq_p);
        }
        if (count_on_j) {
            this->normalize_and_add_cov_and_err(
                    seq_likelihood, n_j_real, j_gene_nucleotide_coverage_p,
                    j_gene_per_nucleotide_error_p, j_gene_nucleotide_coverage_seq_p,
                    j_gene_per_nucleotide_error_seq_p);
        }
    }
...
void Coverage_err_counter::add_checked(shared_ptr<Counter> counter)
{
    ...
    double identity = 1.0;
    if (count_on_v) {
        this->normalize_and_add_cov_and_err(identity, n_v_real, this->v_gene_nucleotide_coverage_p,
                                            this->v_gene_per_nucleotide_error_p,
                                            other->v_gene_nucleotide_coverage_p,
                                            other->v_gene_per_nucleotide_error_p);
    }
    if (count_on_d) {
        this->normalize_and_add_cov_and_err(identity, n_d_real, this->d_gene_nucleotide_coverage_p,
                                            this->d_gene_per_nucleotide_error_p,
                                            other->d_gene_nucleotide_coverage_p,
                                            other->d_gene_per_nucleotide_error_p);
    }
    if (count_on_j) {
        this->normalize_and_add_cov_and_err(identity, n_j_real, this->j_gene_nucleotide_coverage_p,
                                            this->j_gene_per_nucleotide_error_p,
                                            other->j_gene_nucleotide_coverage_p,
                                            other->j_gene_per_nucleotide_error_p);
    }
}
...
void Coverage_err_counter::dump_sequence_data(int seq_index, int iteration_n)
{
    if (dump_individual_seqs) {
        if (count_on_v) {
            this->dump_cov_and_err_arrays(iteration_n, seq_index, output_cov_err_v_file_ptr,
                                          n_v_real, v_gene_nucleotide_coverage_seq_p,
                                          v_gene_per_nucleotide_error_seq_p);
        }

        if (count_on_d) {
            this->dump_cov_and_err_arrays(iteration_n, seq_index, output_cov_err_d_file_ptr,
                                          n_d_real, d_gene_nucleotide_coverage_seq_p,
                                          d_gene_per_nucleotide_error_seq_p);
        }

        if (count_on_j) {
            this->dump_cov_and_err_arrays(iteration_n, seq_index, output_cov_err_j_file_ptr,
                                          n_j_real, j_gene_nucleotide_coverage_seq_p,
                                          j_gene_per_nucleotide_error_seq_p);
        }
    }
}
...
void Coverage_err_counter::dump_data_summary(int iteration_n)
{
    if (not dump_individual_seqs) {
        if (count_on_v) {
            this->dump_cov_and_err_arrays(iteration_n, -1, output_cov_err_v_file_ptr, n_v_real,
                                          v_gene_nucleotide_coverage_p,
                                          v_gene_per_nucleotide_error_p);
        }

        if (count_on_d) {
            this->dump_cov_and_err_arrays(iteration_n, -1, output_cov_err_d_file_ptr, n_d_real,
                                          d_gene_nucleotide_coverage_p,
                                          d_gene_per_nucleotide_error_p);
        }

        if (count_on_j) {
            this->dump_cov_and_err_arrays(iteration_n, -1, output_cov_err_j_file_ptr, n_j_real,
                                          j_gene_nucleotide_coverage_p,
                                          j_gene_per_nucleotide_error_p);
        }
    }
}
...
{
    shared_ptr<Coverage_err_counter> counter_copy_ptr(new Coverage_err_counter(
            path_to_file, count_on, record_Npoint_occurence, dump_individual_seqs, last_iter_only));
...
void Coverage_err_counter::allocate_coverage_and_errors_arrays(
        ...
        pair<size_t, double *> *&gene_per_nucleotide_error_seq_p)
{
    ...
    for (map<string, Event_realization>::const_iterator iter = realizations.begin();
        ...
        gene_nucleotide_coverage_p[(*iter).second.index] =
                pair<size_t, double *>((*iter).second.value_str_int.size(), new double[tmp]);
        gene_per_nucleotide_error_p[(*iter).second.index] =
                pair<size_t, double *>((*iter).second.value_str_int.size(), new double[tmp]);

        //Initialize sequence counters
        gene_nucleotide_coverage_seq_p[(*iter).second.index] =
                pair<size_t, double *>((*iter).second.value_str_int.size(), new double[tmp]);
        gene_per_nucleotide_error_seq_p[(*iter).second.index] =
                pair<size_t, double *>((*iter).second.value_str_int.size(), new double[tmp]);
...
void Coverage_err_counter::dump_cov_and_err_arrays(int iteration_n, int seq_index,
                                                   shared_ptr<ofstream> outfile_ptr, size_t n_real,
                                                   pair<size_t, double *> *coverage_array_p,
...
void Coverage_err_counter::normalize_and_add_cov_and_err(
        double &normalizing_cst, size_t n_real, pair<size_t, double *> *target_coverage_array_p,
        pair<size_t, double *> *target_error_array_p, pair<size_t, double *> *base_coverage_array_p,
        pair<size_t, double *> *base_error_array_p)
{
    for (i = 0; i != n_real; ++i) {
 ...
 */
void Coverage_err_counter::recurs_coverage_count(double scenario_seq_joint_proba, size_t N,
                                                 size_t begin_bound, size_t end_bound,
                                                 size_t gene_len)
{
 ...
 * \param gene_len : Considered gene length
 */
void Coverage_err_counter::recurs_errors_count(double scenario_seq_joint_proba,
                                               vector<int> &mismatch_list,
                                               const int **gene_offset_p, size_t N,
                                               size_t begin_bound, size_t end_bound,
                                               size_t gene_len)
{
    for (size_t j = begin_bound; j != end_bound; ++j) {
        ...
        if (N < record_Npoint_occurence - 1) {
            this->recurs_errors_count(scenario_seq_joint_proba, mismatch_list, gene_offset_p, N + 1,
                                      j, end_bound, gene_len);
        } else {
            size_t adress = 0;
            for (size_t a = 0; a != record_Npoint_occurence; ++a) {
                adress += positions[a] * pow(gene_len, a);
...
void Coverage_err_counter::symmetrize_counter_array(double *counter_array, size_t N,
                                                    size_t begin_bound, size_t gene_len)
{
...
void Coverage_err_counter::symmetrize_counter_array_recurs(size_t adress, size_t N,
                                                           size_t *position_array,
                                                           double *counter_array, size_t gene_len)
{
    for (size_t j = 0; j != record_Npoint_occurence; ++j) {
        ...
        if (is_valid) {
            position_array[N] = j;
            if (N < record_Npoint_occurence - 1) {
                symmetrize_counter_array_recurs(adress, N + 1, position_array, counter_array,
                                                gene_len);
            } else {
                size_t new_adress = 0;
                for (size_t a = 0; a != record_Npoint_occurence; ++a) {
