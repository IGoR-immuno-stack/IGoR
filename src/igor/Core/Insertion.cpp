...
Insertion::Insertion(Gene_class genes, pair<int, int> ins_range)
    : Insertion(genes) { // FIXME nonsense new
...
Insertion::Insertion(Gene_class gene)
    : Rec_Event(gene, Undefined_side), proba_contribution(-1),
      previous_index(-1), insertions(-1), new_scenario_proba(-1),
      base_index(-1), new_index(-1), realization_index(-1),
...
shared_ptr<Rec_Event> Insertion::copy() {
  ...
  shared_ptr<Insertion> new_insertion_p = shared_ptr<Insertion>(
      new Insertion(this->event_class, this->event_realizations));
...
bool Insertion::add_realization(int insertion_number) {
  this->Rec_Event::add_realization(
      Event_realization(to_string(insertion_number), insertion_number, "",
                        Int_Str(), this->size()));
...
void Insertion::iterate(
  ...
  case VD_genes: {
    ...
    int d_offset = chosen_genes.at(D_gene).second.offset; //will crash if no D
    has been chosen int d_del = deletion_map[make_pair(D_gene,Five_prime)];
    ...
    //ie the number of nucleotides in the read between the end of V and
    beginning of D

    int insertions =  (d_offset + d_del) - (v_offset + v_seq_size -1)
    -1;t(pair<Seq_type,Seq_side>(V_gene_seq,Three_prime)) -1
    */

    // if(!(*constructed_sequences.at(D_gene_seq)).empty()){
    // insertions =
    // seq_offsets.at(pair<Seq_type,Seq_side>(D_gene_seq,Five_prime)) -
    // seq_offsets.at(pair<Seq_type,Seq_side>(V_gene_seq,Three_prime)) -1;
    ...
    // TODO Think about including in-dels in the insertion process(thus create a
    // real loop on the number of insertion)

    proba_contribution = (*this).iterate_common(
        proba_contribution, insertions, base_index, base_index_map, offset_map,
        model_parameters_point);
    if (proba_contribution != 0) {
      inserted_str.assign(insertions, -1);
      new_index =
          base_index + this->event_realizations.at(to_string(insertions))
                           .index; // FIXME this should not exist
      constructed_sequences[VD_ins_seq] = &inserted_str;
      downstream_proba_map.set_value(
          VD_ins_seq, junction_length_best_proba_map.at(insertions),
  ...
  case DJ_genes: {
    ...
    //Get number of deletions (if no deletion event before, default initialized
    to 0)
    //need the number of 3' deletions in case the sequence has already been
    shortened int d_del_5 = deletion_map[make_pair(D_gene,Five_prime)]; int
    d_del_3 = deletion_map[make_pair(D_gene,Three_prime)];
    ...
    int insertions = (j_offset + j_del) - (d_offset + d_del_3  + d_seq.size() -
    1 - d_del_5) -1;
    */
    // if(!(*constructed_sequences.at(D_gene_seq)).empty()){
    // insertions =
    // seq_offsets.at(pair<Seq_type,Seq_side>(J_gene_seq,Five_prime)) -
    // seq_offsets.at(pair<Seq_type,Seq_side>(D_gene_seq,Three_prime)) -1;
    ...
    proba_contribution =
        iterate_common(proba_contribution, insertions, base_index,
                       base_index_map, offset_map, model_parameters_point);

    if (proba_contribution != 0) {
      inserted_str.assign(insertions, -1);
      new_index =
          base_index + this->event_realizations.at(to_string(insertions)).index;
      constructed_sequences[DJ_ins_seq] = &inserted_str;
      downstream_proba_map.set_value(
          DJ_ins_seq, junction_length_best_proba_map.at(insertions),
          memory_layer_proba_map_junction);
    }
  ...
  case VJ_genes: {
     ...
     * //Get the offset of V alignment
            int v_offset = chosen_genes.at(V_gene).second.offset;
            int original_v_seq_size =
    chosen_genes.at(V_gene).first.value_str.size();
    ...
    //TODO check if default initializing to 0 is better and then change the
    values int v_del = deletion_map[make_pair(V_gene,Three_prime)];
    */
    ...
    int j_del = deletion_map[make_pair(J_gene,Five_prime)];
    */
    // TODO declare insertion before and call iterate common after, clean code
    // get rid of code duplication int insertions = (j_offset + j_del) -
    // (v_offset  + (original_v_seq_size ) - v_del )-1; int insertions =
    // sequence.size() - (v_seq.size() + j_seq.size());
    ...
    if (proba_contribution != 0) {
      ...
      new_index =
          base_index +
          realization_index; // this->event_realizations.at(to_string(insertions)).index;
      constructed_sequences[VJ_ins_seq] = &inserted_str;
      downstream_proba_map.set_value(
          VJ_ins_seq, junction_length_best_proba_map.at(insertions),
          memory_layer_proba_map_junction);
    }
  ...
  default:
    throw invalid_argument(std::string("Unknown gene_class for Insertion: ") +
                           this->event_class);
    break;
  }
  ...
  if (proba_contribution != 0) {
    ...
    if (scenario_upper_bound_proba >=
      ...
      Rec_Event::iterate_wrap_up(
          ...
          updated_marginals_point, model_parameters_point, allowed_realizations,
          constructed_sequences, seq_offsets, error_rate_p, counters_list,
          events_map, safety_set, mismatches_lists, seq_max_prob_scenario,
...
inline double Insertion::iterate_common(
    double scenario_proba, int insertions, int base_index,
    Index_map &base_index_map,
    const unordered_map<Rec_Event_name,
                        vector<pair<shared_ptr<const Rec_Event>, int>>>
        &offset_map,
  ...
  if (this->ordered_realization_map.count(insertions) > 0) {
    ...
    realization_index =
        INT32_MAX; // make sure the index called at the end is in the range
    ...
    if (prob_count >= rand) {
      ...
      case VD_genes:
        constructed_sequences[VD_ins_seq] =
            string((*iter).second.value_int, 'I');
        break;
      case DJ_genes:
...
void Insertion::write2txt(ofstream &outfile) {
  outfile << "#Insertion;" << event_class << ";" << event_side << ";"
...
void Insertion::set_crude_upper_bound_proba(
    size_t base_index, size_t event_size, Marginal_array_p &marginal_array_p) {
  ...
  for (unordered_map<string, Event_realization>::const_iterator iter =
    ...
    while ((j * numb_realizations + real_index) < event_size) {
      if (marginal_array_p[base_index + ((j * numb_realizations +
                                          real_index))] > max_proba) {
        max_proba = marginal_array_p[base_index +
                                     ((j * numb_realizations + real_index))];
      }
      ++j;
    }
...
void Insertion::initialize_crude_scenario_proba_bound(
  ...
  case VD_genes:
    if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
            Dinuclmarkov_t, VD_genes, Undefined_side))) {
      dinuc_event_p = events_map.at(tuple<Event_type, Gene_class, Seq_side>(
          Dinuclmarkov_t, VD_genes, Undefined_side));
    } else if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
                   Dinuclmarkov_t, VDJ_genes, Undefined_side))) {
      dinuc_event_p = events_map.at(tuple<Event_type, Gene_class, Seq_side>(
          Dinuclmarkov_t, VDJ_genes, Undefined_side));
    }
    break;
  ...
  case VJ_genes:
    if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
            Dinuclmarkov_t, VJ_genes, Undefined_side))) {
      dinuc_event_p = events_map.at(tuple<Event_type, Gene_class, Seq_side>(
          Dinuclmarkov_t, VJ_genes, Undefined_side));
    }
    break;
  ...
  case DJ_genes:
    if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
            Dinuclmarkov_t, DJ_genes, Undefined_side))) {
      dinuc_event_p = events_map.at(tuple<Event_type, Gene_class, Seq_side>(
          Dinuclmarkov_t, DJ_genes, Undefined_side));
    } else if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
                   Dinuclmarkov_t, VDJ_genes, Undefined_side))) {
...
void Insertion::initialize_Len_proba_bound(
