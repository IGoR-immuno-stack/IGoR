...
Errors_counter::Errors_counter(size_t n_scenarios)
    : Counter(),
      n_scenarios_counted(n_scenarios),
      output_scenarios(n_scenarios_counted > 0),
      scenario_n_mismatches(0),
      scenario_n_genomic(0),
      sequence_average_error_freq(0),
      sequence_average_n_genomic(0),
      sequence_average_n_mismatches(0)
{
}

Errors_counter::Errors_counter(size_t n_scenarios, bool is_last_iter_only)
    : Errors_counter(n_scenarios)
{
    this->last_iter_only = is_last_iter_only;
}

Errors_counter::Errors_counter(size_t n_scenarios, string path)
    : Counter(path),
      n_scenarios_counted(n_scenarios),
      output_scenarios(n_scenarios_counted > 0),
      scenario_n_mismatches(0),
 ...
 */
Errors_counter::Errors_counter(size_t n_scenarios, string path, bool is_last_iter_only)
    : Errors_counter(n_scenarios, path)
{
    this->last_iter_only = is_last_iter_only;
}
...
void Errors_counter::count_scenario(long double scenario_seq_joint_proba,
                                    ...
                                    Mismatch_vectors_map &mismatches_lists)
{
    ...
    if (this->output_scenarios) {
        if (this->best_scenarios_vec.size() < this->n_scenarios_counted) {
            ...
            if (this->best_scenarios_vec.empty()) {
                this->best_scenarios_vec.emplace_back(scenario_seq_joint_proba,
                                                      this->scenario_n_genomic,
                                                      this->scenario_n_mismatches);
            } else {
                vector<tuple<double, size_t, size_t>>::iterator jter =
                        this->best_scenarios_vec.begin();
                while ((scenario_seq_joint_proba > get<0>(*jter))
                       and (jter != this->best_scenarios_vec.end())) {
                    ++jter;
                }
    ...
    this->sequence_average_error_freq += scenario_seq_joint_proba
            * ((double)this->scenario_n_mismatches / (double)this->scenario_n_genomic);
...
void Errors_counter::count_sequence(double seq_likelihood,
                                    const Model_marginals &single_seq_marginals,
                                    const Model_Parms &single_seq_model_parms)
{
    for (vector<tuple<double, size_t, size_t>>::iterator iter = this->best_scenarios_vec.begin();
...
void Errors_counter::initialize_counter(const Model_Parms &parms, const Model_marginals &marginals)
{
    if (not fstreams_created) {
        ...
        if (this->n_scenarios_counted > 0) {
            ...
            this->output_scenario_errors_file_ptr->open(path_to_file
                                                        + "scenarios_background_and_errors.csv");
            //Create the header
            (*this->output_scenario_errors_file_ptr.get())
                    << "seq_index;scenario_rank;scenario_proba_cond_seq;n_genomic_nt;n_mismatches"
                    << endl;
        }

        //Create the average mutation rate scenario file
        output_sequence_averaged_errors_file_ptr = shared_ptr<ofstream>(new ofstream);
        this->output_sequence_averaged_errors_file_ptr->open(path_to_file
                                                             + "sequence_mutation_frequency.csv");
        //Create the header
...
void Errors_counter::dump_sequence_data(int seq_index, int iteration_n)
{
    ...
    if (this->output_scenarios) {
        size_t counter = 1;
        for (vector<tuple<double, size_t, size_t>>::reverse_iterator iter =
                     this->best_scenarios_vec.rbegin();
             iter != this->best_scenarios_vec.rend(); ++iter) {
            (*this->output_scenario_errors_file_ptr.get())
                    << seq_index << ";" << counter << ";" << get<0>(*iter) << ";" << get<1>(*iter)
                    << ";" << get<2>(*iter) << endl;
            ++counter;
        }
    }
    ...
    (*this->output_sequence_averaged_errors_file_ptr.get())
            << seq_index << ";" << this->sequence_average_n_genomic << ";"
            << this->sequence_average_n_mismatches << ";" << this->sequence_average_error_freq
            << endl;
...
shared_ptr<Counter> Errors_counter::copy() const
{
    ...
    if (this->fstreams_created) {
        ...
        counter_copy_ptr->output_sequence_averaged_errors_file_ptr =
                this->output_sequence_averaged_errors_file_ptr;
    } else {
        throw runtime_error("Counters should not be copied before stream initalization in "
                            "Errors_counter::copy()");
    }
