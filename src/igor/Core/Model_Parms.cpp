...
Model_Parms::Model_Parms(const Model_Parms &other)
{
    ...
    for (map<Rec_Event_name, Adjacency_list>::const_iterator iter = other.edges.begin();
        ...
        for (list<shared_ptr<Rec_Event>>::const_iterator jiter = (*iter).second.children.begin();
            ...
            for (list<shared_ptr<Rec_Event>>::const_iterator kiter = this->events.begin(); kiter != this->events.end();
                 ++kiter) {
                if ((**kiter) == (**jiter)) {
    ...
    if (this->has_edge(ev1_point, ev2_point)) {
        ...
        try {
            ...
            cerr << "Exception caught trying to invert an edge between " + ev1_point->get_name() + " and "
                            + ev2_point->get_name()
                            + " in Model_Parms::invert_edge , this is most likely creating a cycle, throwing exception "
                              "now..."
                 << endl;
            throw e;
        }
...
queue<shared_ptr<Rec_Event>> Model_Parms::get_model_queue() const
{
    ...
    map<Rec_Event_name, shared_ptr<Rec_Event>> *processed_events_point =
            new map<Rec_Event_name, shared_ptr<Rec_Event>>;
