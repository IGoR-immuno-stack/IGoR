...
public:
    Insertion();
    Insertion(int);
    Insertion(int, std::pair<int, int>); // bounds for insertion lengths
    Insertion(int, std::unordered_map<std::string, Event_realization> &);
    Insertion(int, Seq_side);
    Insertion(int, Seq_side, std::unordered_map<std::string, Event_realization> &);
    ...
    void initialize_crude_scenario_proba_bound(
            ...
            const std::unordered_map<std::tuple<Event_type, int, Seq_side>, std::shared_ptr<Rec_Event>> &) override;
