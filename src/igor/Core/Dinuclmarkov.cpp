        ...
        double &seq_max_prob_scenario, double &proba_threshold_factor)
{
    ...
    if (event_class == VD_genes || event_class == VDJ_genes) {
        ...
        // vd_seq.size());
        data_seq_substr =
                int_sequence.substr(seq_offsets.at(V_gene_seq, Five_prime) + previous_seq_size,
    ...
    if (event_class == VJ_genes) {
        ...
        data_seq_substr = int_sequence.substr(
                seq_offsets.at(V_gene_seq, Five_prime) + previous_seq_size, vj_seq.size());
    ...
    if (scenario_upper_bound_proba >= (seq_max_prob_scenario * proba_threshold_factor)) {
        iterate_wrap_up(new_scenario_proba, downstream_proba_map, sequence, int_sequence,
...
bool Dinucl_markov::has_effect_on(int seq_type) const
{
        ...
        Seq_type_str_p_map &constructed_sequences, int &seq_len /*=0*/) const
{
    ...
    Rec_Event::iterate_initialize_Len_proba_wrap_up(
            ...
            model_parameters_point, base_index_map, constructed_sequences, seq_len /*=0*/);
}
