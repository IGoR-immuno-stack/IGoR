...
shared_ptr<Rec_Event> Dinucl_markov::copy() {
  ...
  shared_ptr<Dinucl_markov> new_dinucl_markov_p =
      shared_ptr<Dinucl_markov>(new Dinucl_markov(this->event_class));
...
void Dinucl_markov::iterate(
  ...
  if (event_class == VD_genes || event_class == VDJ_genes) {
    ...
    // data_seq_substr =
    // sequence.substr(seq_offsets.at(pair<Seq_type,Seq_side>(V_gene_seq,Five_prime))
    // + previous_seq_size , vd_seq.size()); data_seq_substr =
    // sequence.substr(seq_offsets.at(v_5_pair) + previous_seq_size ,
    // vd_seq.size());//TODO check this data_seq_substr =
    // int_sequence.substr(seq_offsets.at(v_5_pair) + previous_seq_size ,
    // vd_seq.size());
    data_seq_substr = int_sequence.substr(
        seq_offsets.at(V_gene_seq, Five_prime) + previous_seq_size,
        vd_seq.size()); // FIXME use precomputed vd seq size

    previous_nt_str = previous_seq.back();
    iterate_common(vd_realizations_indices, previous_nt_str, vd_seq,
  ...
  if (event_class == DJ_genes || event_class == VDJ_genes) {
    correct_class = 1;
    /*		//TODO check the side taken into account for the first
       nucleotide string d_seq = constructed_sequences.at(D_gene_seq); size_t
       d_seq_size = d_seq.size();

                    size_t char_index =
       seq_offsets.at(pair<Seq_type,Seq_side>(V_gene_seq,Five_prime)) +
       constructed_sequences.at(V_gene_seq).size() +
       constructed_sequences.at(VD_ins_seq).size() + d_seq_size ;//TODO rewrite
       this and once deletion mechanism has been rewritten string
       data_seq_substr = sequence.substr(char_index ,
       constructed_sequences.at(DJ_ins_seq).size()); new_scenario_proba =
       iterate_common(new_scenario_proba , prev_seq.substr(prev_seq_size-1,1) ,
       data_seq_substr , constructed_sequences.at(DJ_ins_seq) , base_index ,
       write_index_list , model_parameters_point);
                    */
    ...
    // -dj_seq.size(); size_t char_index = seq_offsets.at(j_5_pair)
    // -dj_seq.size();
    ...
    iterate_common(dj_realizations_indices, previous_nt_str, dj_seq,
                   model_parameters_point);
  ...
  if (event_class == VJ_genes) {
    ...
    previous_seq_size = previous_seq.size();
    // data_seq_substr = sequence.substr(seq_offsets.at(v_5_pair) +
    // previous_seq_size , vj_seq.size()); data_seq_substr =
    // int_sequence.substr(seq_offsets.at(v_5_pair) + previous_seq_size ,
    // vj_seq.size());
    data_seq_substr = int_sequence.substr(
        seq_offsets.at(V_gene_seq, Five_prime) + previous_seq_size,
    ...
    previous_nt_str = previous_seq.back();
    iterate_common(vj_realizations_indices, previous_nt_str, vj_seq,
                   model_parameters_point);
  ...
  if (!correct_class) {
    throw invalid_argument(
        std::string("Unknown gene class for DincuclMarkov model: ") +
        this->event_class);
  }
...
queue<int> Dinucl_markov::draw_random_realization(
  ...
  if (event_class == VD_genes || event_class == VDJ_genes) {
    ...
    queue<int> tmp = this->draw_random_common(
        v_seq, vd_ins_seq, model_marginals_p, index, distribution, generator);
    while (!tmp.empty()) {
  ...
  if (event_class == DJ_genes || event_class == VDJ_genes) {
    ...
    queue<int> tmp = this->draw_random_common(
        j_seq, dj_ins_seq, model_marginals_p, index, distribution, generator);
    while (!tmp.empty()) {
      realization_queue.push(tmp.front());
      tmp.pop();
    }
  ...
  if (event_class == VJ_genes) {
    ...
    queue<int> tmp = this->draw_random_common(
        v_seq, vj_ins_seq, model_marginals_p, index, distribution, generator);
    while (!tmp.empty()) {
      realization_queue.push(tmp.front());
      tmp.pop();
    }
...
queue<int> Dinucl_markov::draw_random_common(
    const string &previous_seq, string &inserted_seq,
  ...
  if (!inserted_seq.empty()) {
    ...
    if (inserted_seq[0] == 'I') {
      ...
      try{
              offset =
      event_realizations.at(previous_seq.substr(previous_seq.size()-1,1)).index*event_realizations.size();
      }
      catch(exception& except){
              cout<<"exception caught in DinucMarkov draw random common, key
      used: "<<previous_seq.substr(previous_seq.size()-1,1); throw except;
      }
      */
      int prev_nt =
          nt2int(previous_seq.substr(previous_seq.size() - 1, 1)).at(0);
      for (unordered_map<string, Event_realization>::const_iterator iter =
               event_realizations.begin();
           iter != event_realizations.end(); ++iter) {
        // prob_count += model_marginals_p[index + offset +
        // (*iter).second.index];
    ...
    for (size_t i = 1; i != inserted_seq.size(); ++i) {
      if (inserted_seq[i] == 'I') {
        ...
        try{
                offset =
        event_realizations.at(inserted_seq.substr(i-1,1)).index*event_realizations.size();
        }
        catch(exception& except){
                cout<<"exception caught, key used:
        "<<inserted_seq.substr(i-1,1)<<",ins seq: "<<inserted_seq<<",i =
        "<<i<<", previous rand: "<<rand<<", previous prob_count:
        "<<prob_count<<endl; throw except;
        }
        */
...
void Dinucl_markov::write2txt(ofstream &outfile) {
  outfile << "#DinucMarkov;" << event_class << ";" << event_side << ";"
          << priority << ";" << nickname << endl;
  for (unordered_map<string, Event_realization>::const_iterator iter =
           event_realizations.begin();
       iter != event_realizations.end(); ++iter) {
    outfile << "%" << (*iter).second.value_str << ";" << (*iter).second.index
            << endl;
  }
}

void Dinucl_markov::iterate_common(
    int *indices_array, int &previous_assigned_nt, Int_Str &ins_seq,
    const Marginal_array_p &model_parameters_point) {
  ...
  if (!ins_seq.empty()) {
    if (ins_seq.at(0) == -1) {
      ...
      // sec_nt_index =
      // event_realizations.at(data_seq_substr.substr(0,1)).index;
      ...
      // For this Dinucl_Markov model the values on the marginal array
      // represents the conditional probability of a couple of nucleotides (N2 |
      // N1)
      if ((first_nt_index < 4) & (sec_nt_index < 4)) {
        ...
        proba_contribution *= model_parameters_point
            [realization_final_index]; /// compute_nt_freq(base_index+offset ,
                                       /// model_parameters_point);
        indices_array[0] = realization_final_index;
      } else {
        // If an ambiguous nucleotide is present we take the average probability
        // over possible underlying nts
    ...
    for (size_t i = 1; i != ins_seq.size(); ++i) {
      if (ins_seq.at(i) == -1) {
        ...
        // first_nt_index =
        // event_realizations.at(data_seq_substr.substr(i-1,1)).index;
        // sec_nt_index =
        // event_realizations.at(data_seq_substr.substr(i,1)).index;
        ...
        // For this Dinucl_Markov model the values on the marginal array
        // represents the joint probability of a couple of nucleotides (N1 , N2)
        if ((first_nt_index < 4) & (sec_nt_index < 4)) {
          offset = first_nt_index * event_realizations.size();
          realization_final_index = base_index + offset + sec_nt_index;
          proba_contribution *= model_parameters_point
              [base_index + offset +
               sec_nt_index]; /// compute_nt_freq(base_index+offset
                              /// ,
                              /// model_parameters_point);
          indices_array[i] = realization_final_index;
        } else {
          // If an ambiguous nucleotide is present we take the average
          // probability over possible underlying nts
          proba_contribution *=
              dinuc_proba_matrix(first_nt_index, sec_nt_index);
          indices_array[i] = -1;
        }
 ...
 */
double
Dinucl_markov::compute_nt_freq(int index,
                               const Marginal_array_p &model_marginals) const {
  double nucl_freq = 0;
...
void Dinucl_markov::ind_normalize(Marginal_array_p &marginal_array_p,
  ...
  for (size_t i = 0; i != numb_realizations; ++i) {
    ...
    if (sum_marginals != 0) {
      for (size_t j = 0; j != numb_realizations; ++j) {
        marginal_array_p[base_index + i * numb_realizations + j] /=
            sum_marginals;
      }
    }
  }
...
/**
 * \bug Will only count realizations of unambiguous nucleotides (realization
 * indices>=0 since they are set to -1 in iterate_common)
 */
void Dinucl_markov::add_to_marginals(
    long double scenario_proba, Marginal_array_p &updated_marginals) const {
  if (viterbi_run) {
...
/**
 * Update probability values contained in a matrix where coordinates >=4
 * indicates ambiguous nucleotides We simply take the average probability over
 * the different possible nucleotides.
 */
void Dinucl_markov::update_event_internal_probas(
  ...
  Int_nt const all_nt_vals[] = {int_A, int_C, int_G, int_T, int_R,
                                int_Y, int_K, int_M, int_S, int_W,
                                int_B, int_D, int_H, int_V, int_N};
      ...
      for (Int_nt prev_nt : previous_list) {
        for (Int_nt next_nt : next_list) {
          this->dinuc_proba_matrix(i, j) +=
              marginal_array[event_index + prev_nt * event_realizations.size() +
                             next_nt];
        }
      }
      // By taking the average we assume all nucleotides underlying the
      // ambiguous one are  equally probable
      this->dinuc_proba_matrix(i, j) /=
          (double)(previous_list.size() * next_list.size());
    }
  }
}

double *Dinucl_markov::get_updated_ptr() { return updated_upper_bound_proba; }
...
bool Dinucl_markov::has_effect_on(Seq_type seq_type) const {
  ...
  case VDJ_genes:
    if (seq_type == VJ_ins_seq or seq_type == VD_ins_seq or
        seq_type == DJ_ins_seq) {
      return true;
    } else
      return false;
  ...
  if (event_class == VD_genes or event_class == VDJ_genes) {
    ...
    if (this->has_effect_on(considered_junction)) {
      if (constructed_sequences.exist(VD_ins_seq)) {
        scenario_proba *= pow(this->get_upper_bound_proba(),
                              constructed_sequences.at(VD_ins_seq)->size());
      }
  ...
  if (event_class == DJ_genes or event_class == VDJ_genes) {
    ...
    if (this->has_effect_on(considered_junction)) {
      if (constructed_sequences.exist(DJ_ins_seq)) {
        scenario_proba *= pow(this->get_upper_bound_proba(),
                              constructed_sequences.at(DJ_ins_seq)->size());
      }
      // Otherwise the proba contribution is 1
    }
  }
  if (event_class == VJ_genes) {
    ...
    if (this->has_effect_on(considered_junction)) {
      if (constructed_sequences.exist(VJ_ins_seq)) {
        scenario_proba *= pow(this->get_upper_bound_proba(),
                              constructed_sequences.at(VJ_ins_seq)->size());
      }
      // Otherwise the proba contribution is 1
    }
  }
...
void Dinucl_markov::initialize_Len_proba_bound(
    queue<shared_ptr<Rec_Event>> &model_queue,
    const Marginal_array_p &model_parameters_point, Index_map &base_index_map) {
  // Do nothing
  /*
   * For now let's assume nothing can happen to the junction once the
   * dinucleotide has been chosen
   * =>no errors
   * =>no in/dels
   */
}
