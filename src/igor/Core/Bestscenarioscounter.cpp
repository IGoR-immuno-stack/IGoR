...
Best_scenarios_counter::Best_scenarios_counter(size_t n_scenarios)
    : Counter(), n_scenarios_counted(n_scenarios)
{
}
        ...
        Mismatch_vectors_map &mismatches_lists)
{
    ...
    if (this->best_scenarios_vec.size() < this->n_scenarios_counted) {
        for (forward_list<shared_ptr<const Rec_Event>>::const_iterator iter =
                     this->event_fw_list.begin();
             iter != this->event_fw_list.end(); ++iter) {
            this->single_scenario_realizations_queue.push(
                    (*iter)->get_current_realizations_index_vec());
        }
        ...
        if (mismatches_lists.exist(V_gene_seq)) {
            ...
            single_scenario_mismatches_list.insert(single_scenario_mismatches_list.end(),
                                                   v_mismatch_list.begin(), v_mismatch_list.end());
        }
        if (mismatches_lists.exist(D_gene_seq)) {
            const vector<int> &d_mismatch_list = *mismatches_lists[D_gene_seq];
            single_scenario_mismatches_list.insert(single_scenario_mismatches_list.end(),
                                                   d_mismatch_list.begin(), d_mismatch_list.end());
        }
        if (mismatches_lists.exist(J_gene_seq)) {
            const vector<int> &j_mismatch_list = *mismatches_lists.at(J_gene_seq);
            single_scenario_mismatches_list.insert(single_scenario_mismatches_list.end(),
                                                   j_mismatch_list.begin(), j_mismatch_list.end());
        }
        ...
        if (this->best_scenarios_vec.empty()) {
            ...
            vector<tuple<double, queue<vector<int>>, list<int>>>::iterator jter =
                    this->best_scenarios_vec.begin();
            while ((scenario_seq_joint_proba > get<0>(*jter))
                   and (jter != this->best_scenarios_vec.end())) {
                ++jter;
            }
            this->best_scenarios_vec.emplace(
                    jter, scenario_seq_joint_proba,
                    const_cast<queue<vector<int>> &>(this->single_scenario_realizations_queue),
                    single_scenario_mismatches_list);
        }
    } else {
        if (scenario_seq_joint_proba > get<0>(this->best_scenarios_vec[0])) {
            ...
            if (mismatches_lists.exist(V_gene_seq)) {
                const vector<int> &v_mismatch_list = *mismatches_lists.at(V_gene_seq);
                single_scenario_mismatches_list.insert(single_scenario_mismatches_list.end(),
                                                       v_mismatch_list.begin(),
                                                       v_mismatch_list.end());
            }
            if (mismatches_lists.exist(D_gene_seq)) {
                const vector<int> &d_mismatch_list = *mismatches_lists[D_gene_seq];
                single_scenario_mismatches_list.insert(single_scenario_mismatches_list.end(),
                                                       d_mismatch_list.begin(),
                                                       d_mismatch_list.end());
            }
            if (mismatches_lists.exist(J_gene_seq)) {
                const vector<int> &j_mismatch_list = *mismatches_lists.at(J_gene_seq);
                single_scenario_mismatches_list.insert(single_scenario_mismatches_list.end(),
                                                       j_mismatch_list.begin(),
                                                       j_mismatch_list.end());
            }
            ...
            this->best_scenarios_vec.emplace(jter, scenario_seq_joint_proba,
                                             this->single_scenario_realizations_queue,
                                             single_scenario_mismatches_list);
...
void Best_scenarios_counter::count_sequence(double seq_likelihood,
                                            const Model_marginals &single_seq_marginals,
                                            const Model_Parms &single_seq_model_parms)
{
    for (vector<tuple<double, queue<vector<int>>, list<int>>>::iterator iter =
                 this->best_scenarios_vec.begin();
...
void Best_scenarios_counter::initialize_counter(const Model_Parms &parms,
...
void Best_scenarios_counter::dump_sequence_data(int seq_index, int iteration_n)
{
    ...
    for (vector<tuple<double, queue<vector<int>>, list<int>>>::reverse_iterator iter =
        ...
        while (not scenario_queue.empty()) {
            ...
            for (vector<int>::const_iterator jter = real_vec.begin(); jter != real_vec.end();
                 ++jter) {
        ...
        --util_iter;
        for (list<int>::const_iterator kter = mismatches_list.begin();
             kter != mismatches_list.end(); ++kter) {
...
{
    shared_ptr<Best_scenarios_counter> counter_copy_ptr(
            new Best_scenarios_counter(this->n_scenarios_counted));
