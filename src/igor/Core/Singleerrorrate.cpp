...
Single_error_rate::Single_error_rate(double error_rate)
    : Error_rate(), model_rate(error_rate), normalized_counter(0), seq_weighted_er(0)
{
        ...
        Mismatch_vectors_map &mismatches_lists, double &seq_max_prob_scenario, const double &proba_threshold_factor)
{
    ...
    if (mismatches_lists.exists(D_gene_seq)) {
        vector<size_t> &d_mismatch_list = *mismatches_lists.get(D_gene_seq);
        number_errors += d_mismatch_list.size();
        genomic_nucl += d_gene_seq.size();
    }
