 ...
 * \version 1.0
 *
 *	Depending on the RecEvent type to which it belongs, the
 *Event_realization must supply either a string (both std::string and IntStr) or
 *an integer value. Integers values are e.g the number of deletions or
 *insertions of Insertion or Deletion RecEvent String values are e.g realization
 *of a GeneChoice Rec_Event, and stands for the gene sequence.
 *
 */
struct Event_realization {
  std::string name;
  int value_int; // union? template? inheritance and reference? just use a
                 // virtual class containing two types of events:str and int
  ...
  Event_realization(std::string real_name, int val_int, std::string val_str,
                    Int_Str val_str_int, int index_val)
      : name(real_name), value_int(val_int), value_str(val_str),
        value_str_int(val_str_int), index(index_val) {}
};
 ...
 * This is a purely abstract class and cannot be instanciated as is, only
 * classes deriving from it and implementing the purely abstract methods can be.
 *
 * Rec_Events contain the different Event_realization associated to it in a
 * hashmap.
 *
 * The RecEvents design is key to the way IGoR explore all possible scenarios
 * (through the iterate method) and generate sequences (through the
...
public:
  ...
  Rec_Event(Gene_class, Seq_side,
            std::unordered_map<std::string, Event_realization> &);
  virtual ~Rec_Event();
   ...
   * \author Q.Marcou
   * \version 1.0
   * \param [in,out] scenario_proba Probability of the currently explored
   *(incomplete) scenario
   ...
   * \param [in,out] base_index_map Dynamic map recording where probabilities
   *should be read on the marginals.
   * \param [in] offset_map Tells the event by how much indices from the
   *children events should be modified
   * \param [in] next_event_ptr_arr Indicates the next event to call iterate on
   * \param [in] updated_marginals_point Summary marginals on which complete
   *scenario posteriors are recorded
   * \param [in] model_parameters_point Current recombination probability
   *distribution
   * \param [in] allowed_realizations The set of genomic templates alignment
   * \param [in,out] constructed_sequences Map containing the (incomplete)
   *scenario's resulting sequence
   * \param [in,out] seq_offsets Map containing the 3' and 5' offsets of each
   *scenario sequence piece
   * \param [in] error_rate_p Pointer to the error model object
   * \param [in] counters_list The list of Counter to be counted
   * \param [in] events_map A map containing all events contained in the
   *Model_parms, accessible through their type, gene class and side.
   * \param [in,out] safety_set A map indicating whether checks on offsets
   *overlap should be performed
   * \param [in,out] mismatches_lists A map containing the (incomplete) scenario
   *mismatches
   * \param [in] seq_max_prob_scenario Most likely scenario's probability for
   *the considered sequence
   * \param [in] proba_threshold_factor Threshold on probability ratio between
   *most likely scenario and explored scenario
   *
   *
   * \return void
   *
   *	The iterate method is the heart of IGoR's scenario exploration.
   *Model_Parms define an order in which the RecEvent should be processed. Upon
   *call of iterate all EventRealization of the RecEvent are assessed and for
   *each possible realization the iterate method is called recursively for the
   *next event. Inside the iterate method a filtering on too improbable
   *realizations is performed (tree prunning) using the downstream_proba_map.
   *
   *  The index map is used to read off the probability of the current RecEvent
   *EventRealization at the correct location given the events parent's
   *realizations. It is further modified to take into account the current
   *event's realization when its children realization probabilities will be
   *read.
   *
   */
  virtual void
  iterate(double &, Downstream_scenario_proba_bound_map &, const std::string &,
          const Int_Str &, Index_map &,
          const std::unordered_map<
              Rec_Event_name,
              std::vector<std::pair<std::shared_ptr<const Rec_Event>, int>>> &,
          std::shared_ptr<Next_event_ptr> &, Marginal_array_p &,
          const Marginal_array_p &,
          const std::unordered_map<Gene_class, std::vector<Alignment_data>> &,
          Seq_type_str_p_map &, Seq_offsets_map &,
          std::shared_ptr<Error_rate> &,
  ...
  const std::unordered_map<std::string, Event_realization>
  get_realizations_map() const {
  ...
  virtual void initialize_event(
      ...
      Safety_bool_map &, std::shared_ptr<Error_rate>, Mismatch_vectors_map &,
      Seq_offsets_map &, Index_map &);
  ...
  virtual void
  update_event_internal_probas(const Marginal_array_p &,
  ...
  void set_event_marginal_size(size_t ev_size) {
    this->event_marginal_size = ev_size;
  };
  ...
  virtual void initialize_Len_proba_bound(
      std::queue<std::shared_ptr<Rec_Event>> &model_queue,
...
protected:
  ...
  double scenario_upper_bound_proba; // Used at runtime to store the upper bound
                                     // probability of the whole scenario
  ...
  // Rec_Event*,int>>>& , std::queue<Rec_Event*>  , Marginal_array_p&  , const
  ...
  void iterate_wrap_up(
      ...
      const std::string &sequence, const Int_Str &int_sequence,
      Index_map &index_map,
      const std::unordered_map<
          Rec_Event_name,
          std::vector<std::pair<std::shared_ptr<const Rec_Event>, int>>>
