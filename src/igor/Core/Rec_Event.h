...
public:
    ...
    iterate(double &, Downstream_scenario_proba_bound_map &, const std::string &, const Int_Str &, Index_map &,
            ...
            const std::map<Gene_class, std::vector<Alignment_data>> &, Seq_type_str_p_map &,
    ...
    virtual std::queue<int> draw_random_realization(
            const Marginal_array_p &, std::map<Rec_Event_name, int> &,
            const std::map<Rec_Event_name, std::vector<std::pair<std::shared_ptr<const Rec_Event>, int>>> &,
            std::map<Seq_type, std::string> &, std::mt19937_64 &) const = 0;
...
protected:
    ...
    //inline void iterate_wrap_up(double& , double& , const std::string& , const std::string& , Index_map& , const std::map<Rec_Event_name,std::vector<std::pair<const Rec_Event*,int>>>& , std::queue<Rec_Event*>  , Marginal_array_p&  , const Marginal_array_p& , const std::map<Gene_class , std::vector<Alignment_data>>& , Seq_type_str_p_map& , Seq_offsets_map& ,std::shared_ptr<Error_rate>&,const std::map<std::tuple<Event_type,Gene_class,Seq_side>,const Rec_Event*>&  , Safety_bool_map& , Mismatch_vectors_map& , double& , double&);
    void iterate_wrap_up(
            ...
            const Marginal_array_p &model_parameters_point,
            const std::map<Gene_class, std::vector<Alignment_data>> &allowed_realizations,
