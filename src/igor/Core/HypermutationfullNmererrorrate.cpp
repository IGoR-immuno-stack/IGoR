...
Hypermutation_full_Nmer_errorrate::Hypermutation_full_Nmer_errorrate(
        size_t nmer_width, Gene_class learn, Gene_class apply, double starting_flat_value,
      ...
      output_Nmer_stat_stream(new ofstream)
{
    ...
    if (fmod(nmer_width, 2) == 0) {
        throw runtime_error("Cannot instanciate hypermutation full Nmer error rate with an even "
                            "size Nmer(need to be symmetric) in "
                            "Hypermutation_full_Nmer_errorrate(size_t nmer_width , Gene_class "
                            "learn , Gene_class apply , double starting_flat_value)");
    }
    ...
    if ((starting_flat_value < 0) or (starting_flat_value > 1)) {
        throw invalid_argument(
                "The starting flat value for the hypermutation probability must lie "
                "between 0 and 1, passed value is "
                + to_string(starting_flat_value)
                + " in Hypermutation_full_Nmer_errorrate(size_t nmer_width , Gene_class "
                  "learn , Gene_class apply , double starting_flat_value)");
    }
...
Hypermutation_full_Nmer_errorrate::Hypermutation_full_Nmer_errorrate(
        ...
        vector<double> init_Nmer_mutations_probas, size_t n_observed_thresh /*=0*/)
    ...
    if (init_Nmer_mutations_probas.size() == pow(4, mutation_Nmer_size)) {
        for (i = 0; i != init_Nmer_mutations_probas.size(); ++i) {
            if ((init_Nmer_mutations_probas[i] >= 0) and (init_Nmer_mutations_probas[i] <= 1)) {
                ...
                throw invalid_argument(
                        "The starting values for the hypermutation probabilities must lie "
                        "between 0 and 1, passed value is "
                        + to_string(init_Nmer_mutations_probas[i]) + "for Nmer index "
                        + to_string(i)
                        + " in Hypermutation_full_Nmer_errorrate(size_t nmer_width , "
                          "Gene_class learn , Gene_class apply , vector<double> "
                          "init_Nmer_mutations_probas)");
            }
        }
    } else {
        throw runtime_error("Size of Nmer mutation probabilities vector does not "
                            "match the expected size in "
                            "Hypermutation_full_Nmer_errorrate(size_t,Gene_class,"
                            "Gene_class,double,std::vector<double>)");
    }
}
...
Hypermutation_full_Nmer_errorrate::Hypermutation_full_Nmer_errorrate(
    ...
    : Hypermutation_full_Nmer_errorrate(nmer_width, learn, apply, starting_flat_value,
                                        n_observed_thresh)
{
...
Hypermutation_full_Nmer_errorrate::Hypermutation_full_Nmer_errorrate(
        size_t nmer_width, Gene_class learn, Gene_class apply,
        vector<double> init_Nmer_mutations_probas, string filename, size_t n_observed_thresh /*=0*/)
    : Hypermutation_full_Nmer_errorrate(nmer_width, learn, apply, init_Nmer_mutations_probas,
                                        n_observed_thresh)
{
    this->set_output_Nmer_stream(filename);
}
...
shared_ptr<Error_rate> Hypermutation_full_Nmer_errorrate::copy() const
{
    ...
    shared_ptr<Hypermutation_full_Nmer_errorrate> copy_err_r =
            shared_ptr<Hypermutation_full_Nmer_errorrate>(new Hypermutation_full_Nmer_errorrate(
                    this->mutation_Nmer_size, this->learn_on, this->apply_to, 0,
                    this->n_observed_Nmer_threshold));
...
Hypermutation_full_Nmer_errorrate::operator+=(Hypermutation_full_Nmer_errorrate err_r)
{
    ...
    if ((this->learn_on == err_r.learn_on) & (this->apply_to == err_r.apply_to)
        & (this->mutation_Nmer_size == err_r.mutation_Nmer_size) & identical_mut_probas) {
        ...
        throw runtime_error("Hypermutation models cannot be added in "
                            "Hypermutation_full_Nmer_errorrate::operator +=()");
    }
}
...
const double &Hypermutation_full_Nmer_errorrate::get_err_rate_upper_bound(size_t n_errors,
                                                                          size_t n_error_free)
{
    ...
    size_t array_size = pow(4,mutation_Nmer_size);
    for(size_t ii=0 ; ii != array_size ; ++ii){
            if(max_mut_proba<this->Nmer_mutation_proba[ii])
    max_mut_proba=this->Nmer_mutation_proba[ii];
            if(min_mut_proba>this->Nmer_mutation_proba[ii])
    min_mut_proba=this->Nmer_mutation_proba[ii];
    }*/
        ...
        double median_mut_proba = (probas_vector[probas_vector.size() / 2 - 1]
                                   + probas_vector[probas_vector.size() / 2])
                / 2.0;
        ...
        for (size_t i = 0; i != new_bound_mat.get_n_rows(); ++i) {
            for (size_t j = 0; j != new_bound_mat.get_n_cols(); ++j) {
                if (i < this->max_err and j < this->max_noerr) {
                    ...
                    new_bound_mat(i, j) =
                            pow(median_mut_proba / 3.0, i) * pow((1 - median_mut_proba), j);
                }
            }
        }
...
void Hypermutation_full_Nmer_errorrate::build_upper_bound_matrix(size_t m, size_t n)
{
  ...
  min_mut_proba=this->Nmer_mutation_proba[ii];
    ...
    double median_mut_proba =
            (probas_vector[probas_vector.size() / 2 - 1] + probas_vector[probas_vector.size() / 2])
            / 2.0;
    ...
    for (size_t i = 0; i != new_bound_mat.get_n_rows(); ++i) {
        for (size_t j = 0; j != new_bound_mat.get_n_cols(); ++j) {
            if (i < this->max_err and j < this->max_noerr) {
                ...
                // new_bound_mat(i,j) =
                // pow(max_mut_proba/3.0,i)*pow((1-min_mut_proba),j);
                new_bound_mat(i, j) =
                        pow(median_mut_proba / 3.0, i) * pow((1 - median_mut_proba), j);
            }
        }
...
queue<int> Hypermutation_full_Nmer_errorrate::generate_errors(string &generated_seq,
...
void Hypermutation_full_Nmer_errorrate::update()
{
    ...
    // This simply boils down to equating the model mutation probabilities to the
    // posterior mutation frequencies
    ...
    for (size_t ii = 0; ii != array_size; ++ii) {
        ...
        // Note that if an Nmer is not observed much the mutation probability might
        // artificially go to 0 because of undersampling, remain cautious when
        // interpreting such values
        if (Nmer_N_bg[ii] >= n_observed_Nmer_threshold) {
            Nmer_mutation_proba[ii] = Nmer_N_SHM[ii] / Nmer_N_bg[ii];
            trusted_probas_vector.push_back(
                    this->Nmer_mutation_proba[ii]); // Compute the median mutability value
            // over trustworthy Nmers
    ...
    if ((trusted_probas_vector.size() % 2) == 0) {
        median_mut_proba = (trusted_probas_vector[trusted_probas_vector.size() / 2 - 1]
                            + trusted_probas_vector[trusted_probas_vector.size() / 2])
                / 2.0;
    } else {
        ...
        const unordered_map<tuple<Event_type, int, Seq_side>, shared_ptr<Rec_Event>> &events_map)
{
    ...
    if (events_map.count(tuple<Event_type, int, Seq_side>(GeneChoice_t, V_gene, Undefined_side))
        > 0) {
        v_gene = true;
    } else {
        v_gene = false;
    }
    if (events_map.count(tuple<Event_type, int, Seq_side>(GeneChoice_t, D_gene, Undefined_side))
        > 0) {
        d_gene = true;
    } else {
        d_gene = false;
    }
    if (events_map.count(tuple<Event_type, int, Seq_side>(GeneChoice_t, J_gene, Undefined_side))
        > 0) {
        j_gene = true;
    } else {
        j_gene = false;
    }
    if (events_map.count(tuple<Event_type, int, Seq_side>(Insertion_t, VJ_genes, Undefined_side))
        > 0) {
        vj_ins = true;
    } else {
        vj_ins = false;
    }
    if (events_map.count(tuple<Event_type, int, Seq_side>(Insertion_t, VD_genes, Undefined_side))
        > 0) {
        vd_ins = true;
    } else {
        vd_ins = false;
    }
    if (events_map.count(tuple<Event_type, int, Seq_side>(Insertion_t, DJ_genes, Undefined_side))
        > 0) {
        dj_ins = true;
    } else {
        dj_ins = false;
...
{
    outfile << "#HypermutationfullNmererrorrate;" << this->mutation_Nmer_size << ";"
...
void Hypermutation_full_Nmer_errorrate::introduce_uniform_transversion(
