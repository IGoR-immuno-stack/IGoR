 ...
 * size. A different mutation probability is recorded for each context of size
 * N, leading to 4^N parameters. This model is inspired from the S5F mutability
 * model. The identity of the resulting nucleotide after mutation is assumed to
...
public:
    ...
    Hypermutation_full_Nmer_errorrate(size_t, Gene_class, Gene_class, std::vector<double>,
                                      std::string, size_t = 0);
    // Hypermutation_full_Nmer_errorrate(size_t,Gene_class,Gene_class, ??);
