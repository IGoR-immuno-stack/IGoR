 ...
 *
 *      Please note that although this class inherits Rec_event properties, it
 is not currently designed to be parent
 ...
 * \version 1.0
 *
 * Models a Markov chain dictating the identity of inserted nucleotides in the
 * inserted region. We assume a low error frequency and almost flat dinucleotide
 * model regime such that we use an euristic to extract the most likely
 * realization. This choice has been made because the full handling through a
 * forward algorithm would not be able to cope with e.g context dependent
 * errors.
 *
...
public:
    // Constructors
    Dinucl_markov(Gene_class); // TODO should be scalable on one side easily (mono
    // di tri quadri nucl)
    // Destructor
...
private:
    double *updated_upper_bound_proba; // This points to a double modified by the
    // Insertion event given the number of
    // insertion
    ...
    // std::pair<Seq_type,Seq_side> v_5_pair = std::make_pair
    ...
    inline std::queue<int> draw_random_common(const std::string &, std::string &,
                                              const Marginal_array_p &, int,
