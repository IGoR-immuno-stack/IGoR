...
/*Hypermutation_global_errorrate::Hypermutation_global_errorrate(size_t
   ...
   n_d_real(0) , v_gene_nucleotide_coverage_p(NULL) ,
   v_gene_per_nucleotide_error_p(NULL),d_gene_nucleotide_coverage_p(NULL) ,
   d_gene_per_nucleotide_error_p(NULL),j_gene_nucleotide_coverage_p(NULL) ,
   j_gene_per_nucleotide_error_p(NULL), v_gene_nucleotide_coverage_seq_p(NULL) ,
   v_gene_per_nucleotide_error_seq_p(NULL) ,
   d_gene_nucleotide_coverage_seq_p(NULL) ,
   d_gene_per_nucleotide_error_seq_p(NULL) ,
   j_gene_nucleotide_coverage_seq_p(NULL) ,
   j_gene_per_nucleotide_error_seq_p(NULL) , dj_ins(true) , vd_ins(true) ,
   vj_ins(true) , v_gene(true) , d_gene(true) , j_gene(true) ,
                vgene_offset_p(NULL) , dgene_offset_p(NULL) ,
   jgene_offset_p(NULL) , vgene_real_index_p(NULL) , dgene_real_index_p(NULL) ,
   jgene_real_index_p(NULL), v_3_del_value_p(NULL) , d_5_del_value_p(NULL) ,
   d_3_del_value_p(NULL) , j_5_del_value_p(NULL), i(-1) , j(-1) ,
   v_3_del_value_corr(INT16_MAX) , d_5_del_value_corr(INT16_MAX) ,
   d_3_del_value_corr(INT16_MAX) , j_5_del_value_corr(INT16_MAX) ,
   tmp_cov_p(NULL) , tmp_err_p(NULL) , tmp_corr_len(-1) , tmp_len_util(-1) ,
   scenario_new_proba(-1) , largest_nuc_adress(-1), tmp_int_nt(-1) ,
   Nmer_index(-1){*/

Hypermutation_global_errorrate::Hypermutation_global_errorrate(
    size_t nmer_width, Gene_class learn, Gene_class apply,
    double starting_flat_value)
    : Error_rate(), mutation_Nmer_size(nmer_width), learn_on(learn),
      apply_to(apply),
      ei_nucleotide_contributions((new double[4 * nmer_width])),
      mu(starting_flat_value), n_v_real(0), n_j_real(0), n_d_real(0),
      v_sequences(NULL), j_sequences(NULL), dj_ins(true), vd_ins(true),
      vj_ins(true), v_gene(true), d_gene(true), j_gene(true),
      vgene_offset_p(NULL), dgene_offset_p(NULL), jgene_offset_p(NULL),
      vgene_real_index_p(NULL), dgene_real_index_p(NULL),
      jgene_real_index_p(NULL), v_3_del_value_p(NULL), d_5_del_value_p(NULL),
      d_3_del_value_p(NULL), j_5_del_value_p(NULL), i(-1), j(-1),
      v_3_del_value_corr(INT16_MAX), d_5_del_value_corr(INT16_MAX),
      d_3_del_value_corr(INT16_MAX), j_5_del_value_corr(INT16_MAX),
      tmp_cov_p(NULL), tmp_err_p(NULL), tmp_corr_len(-1), tmp_len_util(-1),
      scenario_new_proba(-1), largest_nuc_adress(-1), tmp_int_nt(-1),
      Nmer_index(-1), output_Nmer_stat_stream(new ofstream) {

  build_upper_bound_matrix(1, 1);

  if (fmod(nmer_width, 2) == 0) {
    throw runtime_error("Cannot instanciate hypermutation globale error rate "
                        "with an even size Nmer(need to be symmetric)");
  }
  ...
  // Initialize booleans
  if (apply_to == V_gene | apply_to == VJ_genes | apply_to == VD_genes |
      apply_to == VDJ_genes) {
    apply_to_v = true;
  } else
    apply_to_v = false;

  if (apply_to == D_gene | apply_to == DJ_genes | apply_to == VD_genes |
      apply_to == VDJ_genes) {
    apply_to_d = true;
  } else
    apply_to_d = false;

  if (apply_to == J_gene | apply_to == VJ_genes | apply_to == DJ_genes |
      apply_to == VDJ_genes) {
    apply_to_j = true;
  } else
    apply_to_j = false;

  if (learn_on == V_gene | learn_on == VJ_genes | learn_on == VD_genes |
      learn_on == VDJ_genes) {
    learn_on_v = true;
  } else
    learn_on_v = false;

  if (learn_on == D_gene | learn_on == DJ_genes | learn_on == VD_genes |
      learn_on == VDJ_genes) {
    learn_on_d = true;
  } else
    learn_on_d = false;

  if (learn_on == J_gene | learn_on == VJ_genes | learn_on == DJ_genes |
      learn_on == VDJ_genes) {
    learn_on_j = true;
  } else
    learn_on_j = false;
...
Hypermutation_global_errorrate::Hypermutation_global_errorrate(
    size_t nmer_width, Gene_class learn, Gene_class apply,
    double starting_flat_value, vector<double> ei_contributions)
    : Hypermutation_global_errorrate(nmer_width, learn, apply,
                                     starting_flat_value) {
  if (ei_contributions.size() == (4 * mutation_Nmer_size)) {
    ...
    throw runtime_error(
        "Size of ei contribution vector does not match the expected size in "
...
Hypermutation_global_errorrate::Hypermutation_global_errorrate(
    ...
    string filename)
    : Hypermutation_global_errorrate(nmer_width, learn, apply,
                                     starting_flat_value, ei_contributions) {
  this->set_output_Nmer_stream(filename);
}
...
Hypermutation_global_errorrate::~Hypermutation_global_errorrate() {
  ...
  if (learn_on_v) {
    ...
    // delete [] v_gene_nucleotide_coverage_p; //FIXME find out why free()
    // exception delete [] v_gene_nucleotide_coverage_seq_p; delete []
    // v_gene_per_nucleotide_error_p; delete []
...
shared_ptr<Error_rate> Hypermutation_global_errorrate::copy() const {
  ...
  shared_ptr<Hypermutation_global_errorrate> copy_err_r =
      shared_ptr<Hypermutation_global_errorrate>(
          new Hypermutation_global_errorrate(this->mutation_Nmer_size,
                                             this->learn_on, this->apply_to,
                                             this->mu));
  ...
  for (int ii = 0; ii != mutation_Nmer_size * 4; ++ii) {
    copy_err_r->ei_nucleotide_contributions[ii] =
        this->ei_nucleotide_contributions[ii];
  }
...
Hypermutation_global_errorrate &Hypermutation_global_errorrate::operator+=(
    Hypermutation_global_errorrate err_r) {

  // FIXME sequential ifs throwing more meaningful exception
  if ((this->learn_on == err_r.learn_on) & (this->apply_to == err_r.apply_to) &
    ...
    throw runtime_error("Hypermutation models cannot be added in "
                        "Hypermutation_global_errorrate::operator +=()");
  }
}

Error_rate *Hypermutation_global_errorrate::add_checked(Error_rate *err_r) {
  return &(this->operator+=(
      *(dynamic_cast<Hypermutation_global_errorrate *>(err_r))));
}

const double &
Hypermutation_global_errorrate::get_err_rate_upper_bound(size_t n_errors,
                                                         size_t n_error_free) {
  /*	double max_proba = 0;
  ...
  if (n_errors > this->max_err || n_error_free > this->max_noerr) {
    ...
    // read_len^2
    Matrix<double> new_bound_mat(max(this->max_err, n_errors + 10),
    ...
    for (size_t i = 0; i != new_bound_mat.get_n_rows(); ++i) {
      for (size_t j = 0; j != new_bound_mat.get_n_cols(); ++j) {
        if (i < this->max_err and j < this->max_noerr) {
          ...
          new_bound_mat(i, j) =
              pow(this->mu / (3 * (1 + mu)), i) * pow(1 - mu / ((1 + mu)), j);
        }
      }
    }
    this->upper_bound_proba_mat = new_bound_mat;
    this->max_err =
        new_bound_mat.get_n_rows() - 1; //-1 since 0 errors is at index 0
...
void Hypermutation_global_errorrate::build_upper_bound_matrix(size_t m,
                                                              size_t n) {
...
double Hypermutation_global_errorrate::compare_sequences_error_prob(
    ...
    const unordered_map<tuple<Event_type, Gene_class, Seq_side>,
                        shared_ptr<Rec_Event>> &events_map,
  ...
  if (mismatches_lists.exist(D_gene_seq)) {
    vec_ptr_util =
        mismatches_lists[D_gene_seq]; // Should not have to check this and
                                      // default initialization should be
                                      // sufficient
  } else {
  ...
  if (tmp_len_util >= mutation_Nmer_size) {
    ...
    if (current_mismatch ==
        d_mismatch_list.end()) { // no else if in case d_mismatch list is empty
      current_mismatch = j_mismatch_list.begin();
    }

    // TODO Need to get the previous V nucleotides and last J ones

    // Get the adress of the first Nmer(disregarding the error penalty on the
    // first nucleotides)
    Nmer_index = 0;
    ...
    // FIXME maybe should iterate the other way around, what happens for
    // errors/context of first nucleotides?
    while ((current_mismatch != j_mismatch_list.end()) &&
      ...
      if (current_mismatch ==
          d_mismatch_list
              .end()) { // no else if in case d_mismatch list is empty
        current_mismatch = j_mismatch_list.begin();
      }

      // Takes care of the fact that current_mismatch is never incremented if
      // there's a mutation at 0 for instance this needs a true correct fix
    }
    ...
    // Removing the contribution of the first and adding the contribution of the
    // new last

    for (i = (mutation_Nmer_size - 1) / 2 + 1;
         i != scenario_resulting_sequence.size() - (mutation_Nmer_size - 1) / 2;
         ++i) {

      // Remove the previous first nucleotide of the Nmer and it's contribution
      // to the index
      ...
      tmp_int_nt = scenario_resulting_sequence.at(
          i + (mutation_Nmer_size - 1) / 2); // Assume a symetric Nmer
      ...
      if ((current_mismatch != j_mismatch_list.end()) &&
        ...
        if (d_gene) {
          if ((i <= seq_offsets.at(V_gene_seq, Three_prime)) or
              ((i >= seq_offsets.at(D_gene_seq, Five_prime)) and
               ((i <= seq_offsets.at(D_gene_seq, Three_prime)))) or
              (i >= seq_offsets.at(J_gene_seq, Five_prime))) {
            scenario_new_proba *= (1 - Nmer_mutation_proba[Nmer_index]);
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
    // Hard fix test for taking (mutation_Nmer_size-1)/2 last J nucleotides into
    // account //FIXME This is assuming that J is last nucleotide of the
    // sequence (does agree with the rest of the code until now)
    for (i = scenario_resulting_sequence.size() - (mutation_Nmer_size - 1) / 2;
         i != scenario_resulting_sequence.size(); ++i) {
      Nmer_index -= current_Nmer.front() * adressing_vector[0];
      current_Nmer.pop();
      // Shift the index
      Nmer_index *= 4;
      // Add the contribution of the new nucleotide
      tmp_int_nt = j_sequences[**jgene_real_index_p].at(
          i - seq_offsets.at(J_gene_seq, Five_prime) +
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////
    // Hard fix test for taking (mutation_Nmer_size-1)/2 first V nucleotides
    // into account //FIXME This is assuming that V is the first nucleotide of
    // the sequence (does agree with the rest of the code until now)
    if (v_gene) {
      Nmer_index = 0;
      while (!current_Nmer.empty()) {
        current_Nmer.pop();
      ...
      for (i = 0; i != mutation_Nmer_size; ++i) {
        if (i < (mutation_Nmer_size - 1) / 2) {
          ...
          tmp_int_nt =
              scenario_resulting_sequence.at(i - (mutation_Nmer_size - 1) / 2);
        }
        current_Nmer.push(tmp_int_nt);
        Nmer_index += adressing_vector[i] * tmp_int_nt;
      }
      ...
      for (i = 1; i != (mutation_Nmer_size - 1) / 2; ++i) {
        ...
        // Add the contribution of the new nucleotide
        tmp_int_nt =
            scenario_resulting_sequence[i + (mutation_Nmer_size - 1) / 2];
        Nmer_index += tmp_int_nt;
        current_Nmer.push(tmp_int_nt);
  ...
  // If viterbi learning clean seq counters in order to count only this new most
  // likely scenario
  if (viterbi_run) {
    this->clean_seq_counters();
  }

  // Record the number of times each Nmer is seen and the number of times it is
  // mutated

  if (learn_on_v) {

    /*
     * If at least (mutation_Nmer_size+1)/2 V nucleotides remaining (1 visible
     * to count the error, (mutation_Nmer_size-1)/2 on the 3' side for the
     * context assessment
     * => this is most likely ALWAYS the case for V since it is ~300bp long
     * Length of the visible part of V: seq_offsets.at(V_gene_seq,Three_prime) -
     * seq_offsets.at(V_gene_seq,Five_prime) +1 Length of the non visible part
     * of V: V_gene_size - #visible - #deleted (could also use vgene offset) =>
     * this is what is done for now until I find a good reason why V should not
     * be assumed to be seen all the way 5' There must be at least one V
     * nucleotide visible (this is ensured by the alignments)
     *
     * The situation is simpler for V than J since all non visible nucleotides
     * are included at the beginning
     */
    if (v_sequences[**vgene_real_index_p].size() - *v_3_del_value_p >=
      ...
      tmp_len_util = seq_offsets.at(J_gene_seq, Five_prime) -
                     (mutation_Nmer_size - 1) /
                         2; // Start using the information of the (N-1)/2
                            // inserted (or D) nucleotides before the J
      ...
      // Check if there is an error on the first nucleotide and record Nmer
      // statistics
      if ((current_mismatch != v_mismatch_list.end()) &&
          ((*current_mismatch) == 0)) {
       ...
       */
      for (i = (mutation_Nmer_size - 1) / 2 + 1;
           i != (seq_offsets.at(V_gene_seq, Three_prime) +
                 (mutation_Nmer_size - 1) / 2 + 1);
           ++i) {
        // Remove the previous first nucleotide of the Nmer and it's
        ...
        // Check if there is an error on the central nucleotide and record Nmer
        // statistics
        if ((current_mismatch != v_mismatch_list.end()) &&
      ...
      // Do nothing: there is not enough nucleotides on the right to compute an
      // Nmer
    }
    ...
    /*	THIS IS AN OLD VERSION COMMENTED OUT, WILL BE CLEANED SOON
                    ...
                    if(seq_offsets.at(V_gene_seq,Three_prime) >=
       (mutation_Nmer_size-1)){
                            //TODO Need to get the previous V nucleotides and
       last J ones

                            //Get the address of the first Nmer(disregarding the
       error penalty on the first nucleotides) Nmer_index = 0;
                            while(!current_Nmer.empty()){
                                    current_Nmer.pop();
                            }
                            ...
                            for(i=0 ; i!=mutation_Nmer_size ; ++i){
                                    tmp_int_nt =
       scenario_resulting_sequence.at(i); current_Nmer.push(tmp_int_nt);
                                    Nmer_index+=adressing_vector[i]*tmp_int_nt;
                            }

                            //FIXME maybe should iterate the other way around,
       what happens for errors/context of first nucleotides?
                            while((current_mismatch!=v_mismatch_list.end())
                                            &&
       (*current_mismatch)<(mutation_Nmer_size-1)/2){
                                    ++current_mismatch;
                                    //Takes care of the fact that
       current_mismatch is never incremented if there's a mutation at 0 for
       instance
       ...
       what happens for errors/context of first nucleotides?
                            ...
                            //Check if there's an error and apply the cost
       accordingly

                            if( (current_mismatch!=v_mismatch_list.end())
                                    &&
       ((*current_mismatch)==(mutation_Nmer_size-1)/2) ){
                                    one_seq_Nmer_N_SHM[Nmer_index] +=
       scenario_new_proba; one_seq_Nmer_N_bg[Nmer_index] += scenario_new_proba;
                                    ++current_mismatch;
                            }
                            else{
                                    one_seq_Nmer_N_bg[Nmer_index] +=
       scenario_new_proba;
                            }
                            ...
                            if(seq_offsets.at(V_gene_seq,Three_prime) >=
       (mutation_Nmer_size)){
                                    //Look at all Nmers in the
       scenario_resulting_sequence by sliding window
                                    //Removing the contribution of the first and
       adding the contribution of the new last

                                    for( i = (mutation_Nmer_size-1)/2 +1 ;
       i<seq_offsets.at(V_gene_seq,Three_prime)-(mutation_Nmer_size-1)/2 +1  ;
       ++i){
                                            //FIXME
       seq_offsets.at(V_gene_seq,Three_prime)-(mutation_Nmer_size-1)/2 +1 ??
                                            //Remove the previous first
       nucleotide of the Nmer and it's contribution to the index
                                            Nmer_index-=current_Nmer.front()*adressing_vector[0];
                                            current_Nmer.pop();
                                            //Shift the index
                                            Nmer_index*=4;
                                            //Add the contribution of the new
       nucleotide tmp_int_nt =
       scenario_resulting_sequence.at(i+(mutation_Nmer_size-1)/2);//Assume a
       symetric Nmer Nmer_index+=tmp_int_nt; current_Nmer.push(tmp_int_nt);

                                            //Apply the error cost
                                            if(
       (current_mismatch!=v_mismatch_list.end())
                                                            &&
       ((*current_mismatch)==i)){ one_seq_Nmer_N_SHM[Nmer_index] +=
       scenario_new_proba; one_seq_Nmer_N_bg[Nmer_index] += scenario_new_proba;
                                                    ++current_mismatch;
                                            }
                                            else{
  ...
  if (learn_on_d) {
    ...
    if ((seq_offsets.at(D_gene_seq, Five_prime) <=
         seq_offsets.at(D_gene_seq,
                        Three_prime)) // Makes sure there is at least one D
                                      // nucleotide(not fully deleted)
        and
        (seq_offsets.at(D_gene_seq, Five_prime) - (mutation_Nmer_size - 1) / 2 >
         0) // Makes sure there are enough nucleotides on the left
        and
        (seq_offsets.at(D_gene_seq, Three_prime) +
             (mutation_Nmer_size - 1) / 2 <
         scenario_resulting_sequence
             .size())) { // Makes sure there are enough nucleotides on the right
      current_mismatch = d_mismatch_list.begin();
      ...
      // tmp_corr_len = seq_offsets.at(J_gene_seq,Three_prime) -
      // seq_offsets.at(J_gene_seq,Five_prime)+(mutation_Nmer_size-1)/2;
      ...
      for (i = 0; i != mutation_Nmer_size; ++i) {
        // assume there is no error in the rest of the context => read the
        // scenario resulting sequence
        tmp_int_nt = scenario_resulting_sequence.at(i + tmp_len_util);
       ...
       */
      for (i = (seq_offsets.at(D_gene_seq, Five_prime) +
                (mutation_Nmer_size - 1) / 2 + 1);
           i != (seq_offsets.at(D_gene_seq, Three_prime) +
                 (mutation_Nmer_size - 1) / 2 + 1);
           ++i) {
        // Remove the previous first nucleotide of the Nmer and it's
        // contribution to the index
  ...
  if (learn_on_j) {
    ...
    /*
     * If at least (mutation_Nmer_size+1)/2 J nucleotides remaining (1 visible
     * to count the error, (mutation_Nmer_size-1)/2 on the 3' side for the
     * context assessment Length of the visible part of J:
     * seq_offsets.at(J_gene_seq,Three_prime) -
     * seq_offsets.at(J_gene_seq,Five_prime) +1 Length of the non visible part
     * of J: J_gene_size - #visible - #deleted There must be at least one J
     * nucleotide visible (this is ensured by the alignments)
     */

    if (j_sequences[**jgene_real_index_p].size() - *j_5_del_value_p >=
        (mutation_Nmer_size + 1) / 2) {
      current_mismatch = j_mismatch_list.begin();
      ...
      is_visible_nt = true;
      tmp_corr_len = seq_offsets.at(J_gene_seq, Three_prime) -
                     seq_offsets.at(J_gene_seq, Five_prime) +
                     (mutation_Nmer_size - 1) / 2;
      tmp_len_util = seq_offsets.at(J_gene_seq, Five_prime) -
                     (mutation_Nmer_size - 1) /
                         2; // Start using the information of the (N-1)/2
      ...
      for (i = 0; i != mutation_Nmer_size; ++i) {
        if (is_visible_nt) {
          // For visible nucleotides assume there is no error in the rest of the
          // context => read the scenario resulting sequence
          tmp_int_nt = scenario_resulting_sequence.at(i + tmp_len_util);
          if (i == tmp_corr_len) {
            is_visible_nt = false; // All 3' nucleotides are not visible
            tmp_corr_len =
                (mutation_Nmer_size - 1) / 2 -
                *j_5_del_value_p; // Correct offset to read the j sequence =>
                                  // Should read the J sequence at position i -
      ...
      /*
       * i stands for the position of the last nucleotide of the window
       * Need to stop when i== #insertions considered + #visible J considered +
       * #invisible J considered i.e i == (N-1)/2 + (J3'_offset - J5'_offset +1)
       * + (N-1)/2 => i!= N + (J3'_offset - J5'_offset +1)
       */
      for (i = mutation_Nmer_size;
           i !=
           mutation_Nmer_size + (seq_offsets.at(J_gene_seq, Three_prime) -
                                 seq_offsets.at(J_gene_seq, Five_prime) + 1);
           ++i) {
        // Remove the previous first nucleotide of the Nmer and it's
        // contribution to the index
        Nmer_index -= current_Nmer.front() * adressing_vector[0];
        ...
        if (is_visible_nt) {
          ...
          if (i == tmp_corr_len) {
            ...
            tmp_corr_len =
                (mutation_Nmer_size - 1) / 2 -
                *j_5_del_value_p; // Correct offset to read the j sequence =>
                                  // #insertion_considered - #deletions +
                                  // #visible_J_nucs
          }
        } else {
          tmp_int_nt = j_sequences[**jgene_real_index_p].at(i - tmp_corr_len);
        ...
        if ((current_mismatch != j_mismatch_list.end()) &&
            ((*current_mismatch) ==
             i + tmp_len_util - (mutation_Nmer_size - 1) / 2)) {
          one_seq_Nmer_N_SHM[Nmer_index] += scenario_new_proba;
          one_seq_Nmer_N_bg[Nmer_index] += scenario_new_proba;
          ++current_mismatch;
        } else {
    ...
    /*	THIS IS AN OLD VERSION COMMENTED OUT, WILL BE CLEANED SOON
     * if( (seq_offsets.at(J_gene_seq,Three_prime) -
     seq_offsets.at(J_gene_seq,Five_prime) +1 )>=mutation_Nmer_size ){
                            //There are enough J nucleotides on the read to
     start counting at the first J position

                                    current_mismatch = j_mismatch_list.begin();

                                    //TODO Need to get the previous V
     nucleotides and last J ones

                                    //Get the address of the first
     Nmer(disregarding the error penalty on the first nucleotides) Nmer_index =
     0; while(!current_Nmer.empty()){ current_Nmer.pop();
                                    }

                                    for(i=seq_offsets.at(J_gene_seq,Five_prime)-1
     ; i!=seq_offsets.at(J_gene_seq,Five_prime)-1 + mutation_Nmer_size ; ++i){
                                            tmp_int_nt =
     scenario_resulting_sequence.at(i); current_Nmer.push(tmp_int_nt);
                                    ...
                                    //FIXME maybe should iterate the other way
     around, what happens for errors/context of first nucleotides?
                                    while((current_mismatch!=j_mismatch_list.end())
                                                    &&
     (*current_mismatch)<(mutation_Nmer_size-1)/2){
                                            ++current_mismatch;
     ...
     around, what happens for errors/context of first nucleotides?
                                    ...
                                    //Check if there's an error and apply the
     cost accordingly

                                    if(
     (current_mismatch!=j_mismatch_list.end())
                                            &&
     ((*current_mismatch)==seq_offsets.at(J_gene_seq,Five_prime)) ){
                                            one_seq_Nmer_N_SHM[Nmer_index] +=
     scenario_new_proba; one_seq_Nmer_N_bg[Nmer_index] += scenario_new_proba;
     ...
     adding the contribution of the new last
                                    if((seq_offsets.at(J_gene_seq,Three_prime) -
     seq_offsets.at(J_gene_seq,Five_prime) )>=mutation_Nmer_size){

                                            for( i =
     seq_offsets.at(J_gene_seq,Five_prime) +1 ;
     i<seq_offsets.at(J_gene_seq,Three_prime)-(mutation_Nmer_size-1)/2 +1  ;
     ++i){
                                                    //FIXME
     seq_offsets.at(V_gene_seq,Three_prime)-(mutation_Nmer_size-1)/2 +1 ??
                                                    //Remove the previous first
     ...
     += scenario_new_proba;
                            ...
                            //There is not enough J nucleotides on the read,
     however using the "unseen" 3' J nucleotides at least one position can be
     counted
                    }
                    else{
                            //There is not enough nucleotides on the 3' side to
     be able to count this position
                    }*/
  }

  this->seq_likelihood += scenario_new_proba;
  this->seq_probability += scenario_probability;
  this->seq_mean_error_number +=
      (v_mismatch_list.size() + d_mismatch_list.size() +
       j_mismatch_list.size()) *
      scenario_new_proba;
...
queue<int>
Hypermutation_global_errorrate::generate_errors(string &generated_seq,
  ...
  if (rand_err < error_proba) {
    ...
    introduce_uniform_transversion(generated_seq[(mutation_Nmer_size - 1) / 2],
                                   generator, distribution);
  }

  for (i = (mutation_Nmer_size + 1) / 2;
       i != int_generated_seq.size() - (mutation_Nmer_size - 1) / 2; ++i) {
    // Remove the previous first nucleotide of the Nmer and it's contribution to
    // the index
    Nmer_index -= current_Nmer.front() * adressing_vector[0];
    current_Nmer.pop();
    // Shift the index
    Nmer_index *= 4;
    // Add the contribution of the new nucleotide
    tmp_int_nt = int_generated_seq.at(
        i + (mutation_Nmer_size - 1) / 2); // Assume a symmetrically sized Nmer
    Nmer_index += tmp_int_nt;
    current_Nmer.push(tmp_int_nt);
...
uint64_t Hypermutation_global_errorrate::generate_random_contributions(
    double ei_contribution_range) {
  ...
  uniform_real_distribution<double> distribution(-ei_contribution_range,
                                                 ei_contribution_range);
...
void Hypermutation_global_errorrate::update() {
  ...
  // If an output stream is provided outputs Nmer statistics in a file (mostly
  // for debugging)
  if (output_Nmer_stat) {
    for (int zzz = 0; zzz != pow(4, mutation_Nmer_size); ++zzz) {
      (*output_Nmer_stat_stream)
          << zzz << "," << Nmer_N_bg[zzz] << "," << Nmer_N_SHM[zzz] << endl;
    }
  }
  ...
  /*
   * Find the maximum likelihood parameters by using newton's method on the
   * derivative The constraint on the sum of e_j at one position being zero is
   * absorbed by setting e_j(\pi_k) = -sum(e_j(\pi_i)) for i!=k At each step we
   * solve H\deltaX=-J (where H is the hessian and J the jacobian of the
   * "modified error-likelihood" function)
   */
  ...
  while (x_norm > 1e-5 && counter < 50000) {
    ...
    gsl_vector_view J =
        gsl_vector_view_array(J_data, 3 * mutation_Nmer_size + 1);

    // Construct the 3N+1 square Hessian matrix
    double H_data[(3 * mutation_Nmer_size + 1) *
                  (3 * mutation_Nmer_size +
                   1)]; // Are gsl matrices row or column first?
    for (i = 0;
         i != (3 * mutation_Nmer_size + 1) * (3 * mutation_Nmer_size + 1);
         ++i) {
      H_data[i] = 0;
    }
    gsl_matrix_view H = gsl_matrix_view_array(
        H_data, 3 * mutation_Nmer_size + 1, 3 * mutation_Nmer_size + 1);

    /*		for(int yyy = 0 ; yyy !=(3*mutation_Nmer_size)+1 ; ++yyy){
                            for(int zzz = 0 ; zzz !=(3*mutation_Nmer_size)+1 ;
       ++zzz){ cout<<gsl_matrix_get(&H.matrix , zzz , yyy)<<";";
                            }
                            cout<<endl;
                    }
                    cout<<endl;*/
    ...
    while (j != max_address) {
      ...
      double current_Nmer_unorm_score =
          compute_Nmer_unorm_score(base_4_address, ei_nucleotide_contributions);
      if (current_Nmer_P_bg != 0) {
        for (i = 0; i != mutation_Nmer_size; ++i) {
          if (base_4_address[i] == 3) {
            ...
            for (size_t jj = 0; jj != 3; ++jj) {
              ...
              J_data[i * 3 + jj] -=
                  current_Nmer_P_SHM - current_Nmer_P_bg *
                                           (mu * current_Nmer_unorm_score) /
                                           (1 + mu * current_Nmer_unorm_score);

              // Add contribution to d²Q/dei²
              *(gsl_matrix_ptr(&H.matrix, i * 3 + jj, i * 3 + jj)) +=
                  -current_Nmer_P_bg * (mu * current_Nmer_unorm_score) /
                  pow(1 + mu * current_Nmer_unorm_score, 2);

              // Add contributions to the two other constrained nucleotides
              for (size_t jjj = (jj + 1); jjj != 3; ++jjj) {
                *(gsl_matrix_ptr(&H.matrix, i * 3 + jj, i * 3 + jjj)) +=
                    -current_Nmer_P_bg * (mu * current_Nmer_unorm_score) /
                    pow(1 + mu * current_Nmer_unorm_score, 2);
              }
              ...
              for (size_t ii = (i + 1); ii != mutation_Nmer_size; ++ii) {
                // Since the Hessian is symmetric no need to go over everything
                // twice(only fill the lower triangular matrix)
                if (base_4_address[ii] == 3) {
                  for (size_t jjj = 0; jjj != 3; ++jjj) {
                    *(gsl_matrix_ptr(&H.matrix, i * 3 + jj, ii * 3 + jjj)) +=
                        -current_Nmer_P_bg * (mu * current_Nmer_unorm_score) /
                        pow(1 + mu * current_Nmer_unorm_score, 2);
                  }
                } else {
                  *(gsl_matrix_ptr(&H.matrix, i * 3 + jj,
                                   ii * 3 + base_4_address[ii])) -=
                      -current_Nmer_P_bg * (mu * current_Nmer_unorm_score) /
                      pow(1 + mu * current_Nmer_unorm_score, 2);
                }
              }
              ...
              *(gsl_matrix_ptr(&H.matrix, i * 3 + jj,
                               3 * mutation_Nmer_size)) -=
                  -current_Nmer_P_bg * (current_Nmer_unorm_score) /
                  pow(1 + mu * current_Nmer_unorm_score, 2);
            }
          } else {
            // Add contribution to dQ/dei
            J_data[i * 3 + base_4_address[i]] +=
                current_Nmer_P_SHM - current_Nmer_P_bg *
                                         (mu * current_Nmer_unorm_score) /
                                         (1 + mu * current_Nmer_unorm_score);

            // Add contribution to d²Q/dei²
            *(gsl_matrix_ptr(&H.matrix, i * 3 + base_4_address[i],
                             i * 3 + base_4_address[i])) +=
                -current_Nmer_P_bg * (mu * current_Nmer_unorm_score) /
                pow(1 + mu * current_Nmer_unorm_score, 2);
            ...
            for (size_t ii = (i + 1); ii != mutation_Nmer_size; ++ii) {
              ...
              // if same site(ii==i) the only remaining term is the one from the
              // constrained contribution(4th nucleotide), taken care of in the
              // upper part
            }
            // Add contribution to d²Q/dRdei
            *(gsl_matrix_ptr(&H.matrix, i * 3 + base_4_address[i],
        ...
        J_data[(3 * mutation_Nmer_size)] +=
            current_Nmer_P_SHM / mu -
            (current_Nmer_P_bg * current_Nmer_unorm_score /
             (1 + mu * current_Nmer_unorm_score));
        H_data[(3 * mutation_Nmer_size + 1) * (3 * mutation_Nmer_size + 1) -
               1] += current_Nmer_P_bg * pow(current_Nmer_unorm_score, 2) /
                         (1 + mu * current_Nmer_unorm_score) -
                     current_Nmer_P_SHM / pow(mu, 2);
        ...
        for (int ii = 0; ii != 3 * mutation_Nmer_size + 1; ++ii) {
          for (int jj = ii + 1; jj != 3 * mutation_Nmer_size + 1; ++jj) {
            *(gsl_matrix_ptr(&H.matrix, jj, ii)) =
                *(gsl_matrix_ptr(&H.matrix, ii, jj));
          }
      ...
      error_model_likelihood +=
          current_Nmer_P_SHM *
              (log(mu) + log(current_Nmer_unorm_score) - log(3)) -
          current_Nmer_P_bg * log(1 + mu * current_Nmer_unorm_score);
    }

    // cout<<"current hypermutation model likelihood:
    // "<<error_model_likelihood<<endl;
    ...
    /*		gsl_matrix *V =
       gsl_matrix_alloc(3*mutation_Nmer_size+1,3*mutation_Nmer_size+1);
                    gsl_vector *S = gsl_vector_alloc(3*mutation_Nmer_size+1);
                    gsl_vector *work_vect =
       gsl_vector_alloc(3*mutation_Nmer_size+1); gsl_linalg_SV_decomp(&H.matrix
       , V , S , work_vect);
    ...
    while (new_error_model_likelihood - error_model_likelihood <
           alpha * c * m) {
      alpha *= tau;
      new_error_model_likelihood = compute_new_model_likelihood(alpha, x);
      // cout<<"alpha: "<<alpha<<",
      // ML:"<<new_error_model_likelihood-error_model_likelihood<<";"<<new_error_model_likelihood<<";"<<error_model_likelihood<<endl;
      // std::cin.ignore();
    }
    // cout<<alpha<<endl;
    ...
    for (i = 0; i != mutation_Nmer_size; ++i) {
      ...
      for (j = 0; j != 3; ++j) {
        ...
        ei_nucleotide_contributions[i * 4 + j] +=
            alpha * gsl_vector_get(x, (i * 3 + j));

        // Compute the contribution of the constrained nucleotide
        ei_nucleotide_contributions[i * 4 + 3] -=
            alpha * gsl_vector_get(x, (i * 3 + j));
      }
    }
...
double Hypermutation_global_errorrate::compute_new_model_likelihood(
    double alpha, gsl_vector *update_vect_p) {
  double new_R =
      mu + alpha * gsl_vector_get(update_vect_p, (3 * mutation_Nmer_size));
  ...
  for (a = 0; a != mutation_Nmer_size; ++a) {
    ...
    for (b = 0; b != 3; ++b) {
      // Update the contribution of the nucleotide
      new_ei_nucleotide_contributions[a * 4 + b] +=
          alpha * gsl_vector_get(update_vect_p, (a * 3 + b));

      // Compute the contribution of the constrained nucleotide
      new_ei_nucleotide_contributions[a * 4 + 3] -=
          alpha * gsl_vector_get(update_vect_p, (a * 3 + b));
    }
  }
  // Compute the values for the Jacobian and Hessian entries
  ...
  while (b != max_address) {
    ...
    error_model_likelihood +=
        current_Nmer_P_SHM *
            (log(new_R) + log(current_Nmer_unorm_score) - log(3)) -
...
void Hypermutation_global_errorrate::increment_base_10_and_4(
    int &base_10_counter, int *base_4_address) {
  // Update the base 10 and 4 addresses
  ++base_10_counter; // base 10
  bool bool_mod_4N =
      fmod(base_10_counter, 4) == 0; // FIXME define a function for this
  if (bool_mod_4N) {
    ...
    while (bool_mod_4N) {
      ...
      if (position == 0)
        break;
      bool_mod_4N =
          fmod(base_10_counter, adressing_vector.at(position - 1)) == 0;
...
void Hypermutation_global_errorrate::initialize(
    const unordered_map<tuple<Event_type, Gene_class, Seq_side>,
                        shared_ptr<Rec_Event>> &events_map) {
  // FIXME look for previous initialization to avoid memory leak

  // Initialize booleans for constructed sequences
  if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
          GeneChoice_t, V_gene, Undefined_side)) > 0) {
    v_gene = true;
  } else {
    v_gene = false;
  }
  if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
          GeneChoice_t, D_gene, Undefined_side)) > 0) {
    d_gene = true;
  } else {
    d_gene = false;
  }
  if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
          GeneChoice_t, J_gene, Undefined_side)) > 0) {
    j_gene = true;
  } else {
    j_gene = false;
  }
  if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
          Insertion_t, VJ_genes, Undefined_side)) > 0) {
    vj_ins = true;
  } else {
    vj_ins = false;
  }
  if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
          Insertion_t, VD_genes, Undefined_side)) > 0) {
    vd_ins = true;
  } else {
    vd_ins = false;
  }
  if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
          Insertion_t, DJ_genes, Undefined_side)) > 0) {
    dj_ins = true;
  } else {
    dj_ins = false;
  }
  ...
  if (v_gene) {
    v_gene_event_p = dynamic_pointer_cast<Gene_choice>(
        events_map.at(tuple<Event_type, Gene_class, Seq_side>(
            GeneChoice_t, V_gene, Undefined_side)));
    ...
    if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
            Deletion_t, V_gene, Three_prime)) != 0) {
      shared_ptr<const Deletion> v_3_del_event_p =
          dynamic_pointer_cast<Deletion>(
              events_map.at(tuple<Event_type, Gene_class, Seq_side>(
                  Deletion_t, V_gene, Three_prime)));
    ...
    if (learn_on == V_gene | learn_on == VJ_genes | learn_on == VD_genes |
        learn_on == VDJ_genes) {
      cout << "Exception caught during initialization of Hypermutation global "
              "error rate"
           << endl;
      cout << "Exception caught trying to initialize V gene pointers" << endl;
      cout << endl << "throwing exception now..." << endl;
      throw runtime_error(
          "Cannot learn on V gene without V choice in the model!");
    }
  }
  ...
  if (d_gene) {
    d_gene_event_p = dynamic_pointer_cast<Gene_choice>(
        events_map.at(tuple<Event_type, Gene_class, Seq_side>(
            GeneChoice_t, D_gene, Undefined_side)));
    ...
    if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
            Deletion_t, D_gene, Five_prime)) != 0) {
      shared_ptr<const Deletion> d_5_del_event_p =
          dynamic_pointer_cast<Deletion>(
              events_map.at(tuple<Event_type, Gene_class, Seq_side>(
                  Deletion_t, D_gene, Five_prime)));
    ...
    if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
            Deletion_t, D_gene, Three_prime)) != 0) {
      shared_ptr<const Deletion> d_3_del_event_p =
          dynamic_pointer_cast<Deletion>(
              events_map.at(tuple<Event_type, Gene_class, Seq_side>(
                  Deletion_t, D_gene, Three_prime)));
    ...
    if (learn_on == D_gene | learn_on == DJ_genes | learn_on == VD_genes |
      ...
      cout << endl << "throwing exception now..." << endl;
      throw runtime_error(
          "Cannot learn on D gene without D choice in the model!");
    }
  }
  ...
  if (j_gene) {
    j_gene_event_p = dynamic_pointer_cast<Gene_choice>(
        events_map.at(tuple<Event_type, Gene_class, Seq_side>(
            GeneChoice_t, J_gene, Undefined_side)));
    ...
    if (events_map.count(tuple<Event_type, Gene_class, Seq_side>(
            Deletion_t, J_gene, Five_prime)) != 0) {
      shared_ptr<const Deletion> j_5_del_event_p =
          dynamic_pointer_cast<Deletion>(
              events_map.at(tuple<Event_type, Gene_class, Seq_side>(
                  Deletion_t, J_gene, Five_prime)));
      j_5_del_value_p = &(j_5_del_event_p->deletion_value);
    } else {
      j_5_del_value_p = &no_del_buffer;
    }
  } else {
    if (learn_on == J_gene | learn_on == DJ_genes | learn_on == VJ_genes |
        learn_on == VDJ_genes) {
      cout << "Exception caught during initialization of Hypermutation global "
              "error rate"
           << endl;
      ...
      cout << endl << "throwing exception now..." << endl;
      throw runtime_error(
          "Cannot learn on J gene without J choice in the model!");
    }
  }
...
void Hypermutation_global_errorrate::write2txt(ofstream &outfile) {
  outfile << "#Hypermutationglobalerrorrate;" << this->mutation_Nmer_size << ";"
          << this->learn_on << ";" << this->apply_to << endl;
  outfile << mu << endl;
...
void Hypermutation_global_errorrate::update_Nmers_proba(int current_pos,
                                                        int current_index,
  ...
  for (int ii = 0; ii != 4; ++ii) {
    ...
    double new_score =
        current_score * exp(ei_nucleotide_contributions[current_pos * 4 + ii]);
    if (current_pos != mutation_Nmer_size - 1) {
      update_Nmers_proba(current_pos + 1, new_index, new_score);
    } else {
      this->Nmer_mutation_proba[new_index] =
          new_score * mu / (1 + new_score * mu);
...
nucleotide_error = v_gene_per_nucleotide_error_p[(*real_iter).second.index];
                        ...
                        for(j=0 ; j!=mutation_Nmer_size ; j++){
                                tmp_int_nt =
(*real_iter).second.value_str_int.at(j); current_Nmer.push(tmp_int_nt);
                                Nmer_index+=adressing_vector[j]*tmp_int_nt;

                                if(nucleotide_coverage.second[j]<min_coverage){
                                        min_coverage =
nucleotide_coverage.second[j];
                                }
                        }

                        Nmer_P_BG[Nmer_index] += min_coverage; //The coverage of
the Nmer is only as high as the lowest covered nt Nmer_P_SHM[Nmer_index] +=
nucleotide_error.second[(mutation_Nmer_size-1)/2];

                        for(i=(mutation_Nmer_size-1)/2 +1 ;
i!=(*real_iter).second.value_str_int.size() - (mutation_Nmer_size-1)/2; ++i){

                                //Remove the previous first nucleotide of the
Nmer and it's contribution to the index
                                Nmer_index-=current_Nmer.front()*adressing_vector[0];
                                current_Nmer.pop();
                                //Shift the index
                                Nmer_index*=4;
                                ...
                                tmp_int_nt =
(*real_iter).second.value_str_int.at(i+(mutation_Nmer_size-1)/2);//Assume a
symetric Nmer
                                ...
                                double min_coverage = INT16_MAX;
                                for(j=-(mutation_Nmer_size-1)/2 ; j!=
(mutation_Nmer_size-1)/2 +1 ; j++){
                                        if(nucleotide_coverage.second[j+i]<min_coverage){
                                                min_coverage =
nucleotide_coverage.second[j+i];
                                        }
                                }

                                Nmer_P_BG[Nmer_index] += min_coverage; //The
coverage of the Nmer is only as high as the lowest covered nt
                                Nmer_P_SHM[Nmer_index] +=
nucleotide_error.second[i];
        ...
        //TODO remove code duplication this should be only one function taking
...
nucleotide_error = j_gene_per_nucleotide_error_p[(*real_iter).second.index];
...
double Hypermutation_global_errorrate::compute_Nmer_unorm_score(
    int *base_4_address, double *ei_nucleotide_contributions_p) {
  double unorm_score = 1;
  for (int ii = 0; ii != mutation_Nmer_size; ++ii) {
    unorm_score *=
        exp(ei_nucleotide_contributions_p[4 * ii + base_4_address[ii]]);
  }
  // cout<<"unorm score b4
  // address:"<<base_4_address[0]<<base_4_address[1]<<base_4_address[2]<<";"<<unorm_score<<endl;
  return unorm_score;
}

void Hypermutation_global_errorrate::introduce_uniform_transversion(
    char &nt, std::mt19937_64 &generator,
