      ...
      output_Nmer_stat_stream(new ofstream)
{
    ...
    if (fmod(nmer_width, 2) == 0) {
        throw runtime_error(
                "Cannot instanciate hypermutation globale error rate with an even size Nmer(need to be symmetric)");
    }
                ...
                if (is_visible_nt) {
                    ...
                    if (i == tmp_corr_len) {
                        ...
                        tmp_corr_len = (mutation_Nmer_size - 1) / 2
                                - *j_5_del_value_p; //Correct offset to read the j sequence => Should read the J sequence at position i - #insertions + #deletions
                    }
                } else {
            ...
            for (i = mutation_Nmer_size; i
                ...
                if (is_visible_nt) {
                    ...
                    if (i == tmp_corr_len) {
                        is_visible_nt = false; //All 3' nucleotides are not visible
                        tmp_corr_len = (mutation_Nmer_size - 1) / 2
                                - *j_5_del_value_p; //Correct offset to read the j sequence => #insertion_considered - #deletions + #visible_J_nucs
                    }
                } else {
                    tmp_int_nt = j_sequences[**jgene_real_index_p].at(i - tmp_corr_len);
                }
    ...
    while (x_norm > 1e-5 && counter < 50000) {
        ...
        while (j != max_address) {
            ...
            double current_Nmer_unorm_score =
                    compute_Nmer_unorm_score(base_4_address.data(), ei_nucleotide_contributions);
        ...
        const Events_map &events_map)
{
    ...
    if (events_map.count(make_tuple(GeneChoice_t, string("V_gene_seq"), Undefined_side)) > 0) {
        v_gene = true;
    } else {
        v_gene = false;
    }
    ...
    if (events_map.count(make_tuple(GeneChoice_t, string("J_gene_seq"), Undefined_side)) > 0) {
        j_gene = true;
    } else {
        j_gene = false;
    }
    if (events_map.count(make_tuple(Insertion_t, string("VJ_ins_seq"), Undefined_side)) > 0) {
        vj_ins = true;
    } else {
        vj_ins = false;
    }
    if (events_map.count(make_tuple(Insertion_t, string("VD_ins_seq"), Undefined_side)) > 0) {
        vd_ins = true;
    } else {
        vd_ins = false;
    }
    if (events_map.count(make_tuple(Insertion_t, string("DJ_ins_seq"), Undefined_side)) > 0) {
        dj_ins = true;
    } else {
        dj_ins = false;
    }
