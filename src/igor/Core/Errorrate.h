...
public:
    ...
    /**
     * \brief Zero the cross-sequence accumulators, keeping the model parameters.
     *
     * Counterpart of add_checked(): allows the accumulated contribution of a group
     * of sequences to be drained into a separate accumulator so that the summation
     * order over sequences can be fixed independently of the thread schedule.
     * Per-sequence scratch values are left to clean_seq_counters().
     */
    virtual void clear_accumulators() = 0;
