...
Gene_choice::Gene_choice(Gene_class gene)
    : Rec_Event(gene, Undefined_side), vd_check(true), vj_check(true),
      dj_check(true), d_5_min_offset(INT16_MAX), d_5_max_offset(INT16_MAX),
      j_5_min_offset(INT16_MAX), j_5_max_offset(INT16_MAX), v_5_off(INT16_MAX),
      v_3_off(INT16_MAX), d_offset(INT16_MAX), j_offset(INT16_MAX),
      v_offset(INT16_MAX), v_3_min_offset(INT16_MAX), v_3_max_offset(INT16_MAX),
      d_3_off(INT16_MAX), d_5_off(INT16_MAX), d_3_min_offset(INT16_MAX),
      d_3_max_offset(INT16_MAX), j_5_off(INT16_MAX), no_d_align(true),
      d_size(INT16_MAX), d_full_3_offset(INT16_MAX), base_index(-1),
      new_scenario_proba(-1), new_tmp_err_w_proba(-1), proba_contribution(-1),
      new_index(-1), alignment_offset_p(NULL), memory_layer_cs(-1),
      memory_layer_mismatches(-1), memory_layer_safety_1(-1),
      memory_layer_safety_2(-1), memory_layer_off_threep(-1),
      memory_layer_off_fivep(-1), memory_layer_offset_check1(-1),
      memory_layer_offset_check2(-1), v_chosen(false), v_choice_exist(true),
      d_chosen(false), d_choice_exist(false), j_chosen(false),
      j_choice_exist(true), d_5_max_del(INT16_MIN), d_5_min_del(INT16_MAX),
      d_5_real_max_del(INT16_MIN), j_5_max_del(INT16_MIN),
      j_5_min_del(INT16_MIN), v_3_max_del(INT16_MIN), v_3_min_del(INT16_MAX),
      d_3_max_del(INT16_MIN), d_3_min_del(INT16_MAX) {
  this->type = Event_type::GeneChoice_t;
  this->update_event_name();
}
...
Gene_choice::Gene_choice(Gene_class gene,
                         vector<pair<string, string>> genomic_sequences)
    : Gene_choice(gene) {
  this->type = Event_type::GeneChoice_t;
  for (vector<pair<string, string>>::const_iterator seq_it =
           genomic_sequences.begin();
...
shared_ptr<Rec_Event> Gene_choice::copy() {
  shared_ptr<Gene_choice> new_gene_choice_p = shared_ptr<Gene_choice>(
      new Gene_choice(this->event_class, this->event_realizations));
...
bool Gene_choice::add_realization(string gene_name, string gene_sequence) {
  ...
  this->Rec_Event::add_realization(*(new Event_realization(
      gene_name, INT16_MAX, gene_sequence, nt2int(gene_sequence),
      this->event_realizations.size()))); // FIXME nonsense new
  this->update_event_name();
  return 1;
}

void Gene_choice::set_genomic_templates(
    const vector<pair<string, string>> &genomic_templates) {
  // First remove previous realizations
  this->event_realizations.clear();
  for (vector<pair<string, string>>::const_iterator iter =
           genomic_templates.begin();
...
void Gene_choice::iterate(
    ...
    Marginal_array_p &updated_marginals_pointer,
  ...
  switch (this->event_class) {
    ...
    // TODO take into account in-dels and construct them in the constructed
    // sequences
    // TODO withdraw the assignment body to iterate  so that other same kind of
    // functions can be constructed such as one to iterate one time and generate
    // ""counters""
  ...
  case V_gene: {
    ...
    for (vector<Alignment_data>::const_iterator iter =
             allowed_realizations.at(V_gene).begin();
         iter != allowed_realizations.at(V_gene).end(); ++iter) {
      ...
      if ((*iter).offset >= 0) {
        ...
        // If the offset is negative then the whole V-gene is not visible in the
        // sequence thus only the aligned part of the gene is used. gene_seq =
        // this->event_realizations.at((*iter).gene_name).value_str.substr(
        // -(*iter).offset ) ; Use integer sequence (allow indexing on
        // nucleotide identity)
        gene_seq = this->event_realizations.at((*iter).gene_name)
                       .value_str_int.substr(-(*iter).offset);
        v_5_off = 0;
      }
      ...
      if (vd_check) {
        ...
        if ((v_3_off + v_3_max_del) >= (d_5_max_offset)) {
          // Even with maximum number of deletions on each side the V and D
        ...
        if ((v_3_off + v_3_min_del) < (d_5_min_offset)) {
          ...
          // In the deletion range => some number of deletion won't be allowed
          // and will be discarded in the deletion process
          safety_set.set_value(Event_safety::VD_safe, false,
                               memory_layer_safety_1);
        }
      }
      ...
      if (vj_check) {
        ...
        if ((v_3_off + v_3_max_del) >= (j_5_max_offset)) {
          // Even with maximum number of deletions on each side the V and J
          // overlap => bad alignments
          continue;
        }
      ...
      current_realizations_index_vec[0] =
          this->event_realizations.at((*iter).gene_name).index;
      ...
      // State pointers
      current_realization_index =
          &this->event_realizations.at((*iter).gene_name).index;
      alignment_offset_p = &(*iter).offset;

      proba_contribution = iterate_common(
          proba_contribution, current_realizations_index_vec[0], base_index,
      ...
      // Set seq offsets
      seq_offsets.set_value(V_gene_seq, Five_prime, v_5_off,
                            memory_layer_off_fivep);
      seq_offsets.set_value(V_gene_seq, Three_prime, v_3_off,
                            memory_layer_off_threep);

      // Set the the V mismatch list using the mismatch list computed during the
      // alignment
      mismatches_lists.set_value(V_gene_seq, &(*iter).mismatches,
                                 memory_layer_mismatches);

      // Update downstream proba map and compute the downstream proba bound for
      // this event
      ...
      if (d_chosen) {
        ...
        downstream_proba_map.set_value(
            VD_ins_seq, vd_length_best_proba_map.at(d_offset - v_3_off - 1),
        ...
        downstream_proba_map.set_value(
            VJ_ins_seq, vj_length_best_proba_map.at(j_offset - v_3_off - 1),
            memory_layer_proba_map_junction);
      }

      // Count the number of mismatches that will not go away whatever the
      // number off deletions
      endogeneous_mismatches = 0;
      mism_iter = iter->mismatches.begin();
      while ((mism_iter != iter->mismatches.end()) and
             ((*mism_iter) <= v_3_off + v_3_max_del)) {
        // Count one mismatch
      ...
      Rec_Event::iterate_wrap_up(
          ...
          updated_marginals_pointer, model_parameters_pointer,
          allowed_realizations, constructed_sequences, seq_offsets,
          error_rate_p, counters_list, events_map, safety_set, mismatches_lists,
          seq_max_prob_scenario, proba_threshold_factor);
    }
  } break;
  ...
  case D_gene: {
    ...
    if (j_chosen) {
      ...
      // Useful in case of no D, however for speed purpose it might be better to
      // process J choice first
    ...
    for (vector<Alignment_data>::const_iterator iter =
             allowed_realizations.at(D_gene).begin();
         iter != allowed_realizations.at(D_gene).end(); ++iter) {
      ...
      // Assume that the whole D is in the sequence and add the D sequence to
      // the constructed sequences
      seq_offsets.set_value(D_gene_seq, Five_prime, (*iter).offset,
                            memory_layer_off_fivep);
      seq_offsets.set_value(D_gene_seq, Three_prime,
                            (*iter).offset + gene_seq.size() - 1,
                            memory_layer_off_threep);

      mismatches_lists.set_value(D_gene_seq, &(*iter).mismatches,
                                 memory_layer_mismatches);
      ...
      if (v_chosen and j_chosen) {
        if (vd_length_best_proba_map.count(d_5_off - v_offset - 1) <= 0 or
            dj_length_best_proba_map.count(j_offset - d_3_off - 1) <= 0) {
          continue; // This means no scenario can lead to a correct solution,
                    // would need to be changed for Error models with in/dels
        }
        downstream_proba_map.set_value(VJ_ins_seq, 1.0,
                                       memory_layer_proba_map_junction);
        downstream_proba_map.set_value(
            VD_ins_seq, vd_length_best_proba_map.at(d_5_off - v_offset - 1),
            memory_layer_proba_map_junction_d2);
        downstream_proba_map.set_value(
            DJ_ins_seq, dj_length_best_proba_map.at(j_offset - d_3_off - 1),
      ...
      // Count the number of mismatches that will not go away even with maximum
      // number of deletions
      endogeneous_mismatches = 0;
      if ((d_5_off - d_5_max_del) < (d_3_off + d_3_max_del)) {
        mism_iter = iter->mismatches.begin();
        while (mism_iter != iter->mismatches.end()) {
          if ((*mism_iter) >= (d_5_off - d_5_max_del) and
              (*mism_iter) <= (d_3_off + d_3_max_del)) {
            // Count one mismatch
            ++endogeneous_mismatches;
          }
          ++mism_iter;
    ...
    if (no_d_align) {
      ...
      // Pass the mismatch vector pointer to the memory map once (will be
      // updated in the next loop)
      mismatches_lists.set_value(D_gene_seq, &no_d_mismatches,
                                 memory_layer_mismatches);
      ...
      if (v_chosen and j_chosen) {
        ...
        if (vj_length_d_position_proba.count(vj_len) != 0) {
          ...
          for (vector<tuple<string, int, int, double>>::const_iterator
                   d_position_iter = d_positions_vector.begin();
               d_position_iter != d_positions_vector.end(); ++d_position_iter) {

            const Event_realization &d_real =
                this->event_realizations.at(get<0>(*d_position_iter));
            ...
            constructed_sequences.set_value(D_gene_seq, &gene_seq,
                                            memory_layer_cs);
            ...
            proba_contribution = 1;
            proba_contribution = iterate_common(
                proba_contribution, current_realizations_index_vec[0],
                base_index, base_index_map, offset_map,
                model_parameters_pointer);

            new_scenario_proba = scenario_proba * proba_contribution;

            // Update downstream proba map and compute the downstream proba
            // bound for this event
            scenario_upper_bound_proba = new_scenario_proba;

            // Get DJ or VJ junction upper bound proba
            /*							if(v_chosen and
               j_chosen){ if(vd_length_best_proba_map.count(d_5_off - v_offset
               -1)<=0 or dj_length_best_proba_map.count(j_offset -
               d_full_3_offset  -1)<=0){ continue; //This means no scenario can
               lead to a correct solution, would need to be changed for Error
               models with in/dels
                                                                            }*/
            downstream_proba_map.set_value(VJ_ins_seq, 1.0,
                                           memory_layer_proba_map_junction);
            ...
            /*							}
               ...
               if(v_chosen){ if(vd_length_best_proba_map.count(d_5_off -
               v_offset -1)<=0){ continue; //This means no scenario can lead to
               a correct solution, would need to be changed for Error models
               with in/dels
                                                                            }
               ...
               if(j_chosen){ if(dj_length_best_proba_map.count(j_offset -
               d_full_3_offset  -1)<=0){ continue; //This means no scenario can
               lead to a correct solution, would need to be changed for Error
               models with in/dels
                                                                            }
            ...
            // If even without taking the weight of errors into account not
            // good, then any lower one not good
            if (scenario_upper_bound_proba <
                (seq_max_prob_scenario * proba_threshold_factor)) {
              break;
            }
            ...
            if ((d_5_off - d_5_max_del) < (d_full_3_offset + d_3_max_del)) {
              ...
              while (mism_iter != no_d_mismatches.end()) {
                if ((*mism_iter) >= (d_5_off - d_5_max_del) and
                    (*mism_iter) <= (d_full_3_offset + d_3_max_del)) {
                  // Count one mismatch
                  ++endogeneous_mismatches;
                }
                ++mism_iter;
              ...
              downstream_proba_map.set_value(
                  ...
                  error_rate_p->get_err_rate_upper_bound(
                      endogeneous_mismatches, (d_full_3_offset + d_3_max_del) -
                                                  (d_5_off - d_5_max_del) -
                                                  endogeneous_mismatches),
                  memory_layer_proba_map_seq);
            ...
            // Assume that the whole D is in the sequence and add the D sequence
            // to the constructed sequences
            seq_offsets.set_value(D_gene_seq, Five_prime, d_5_off,
                                  memory_layer_off_fivep);
            seq_offsets.set_value(D_gene_seq, Three_prime, d_5_off + d_size - 1,
                                  memory_layer_off_threep);

            Rec_Event::iterate_wrap_up(
                new_scenario_proba, downstream_proba_map, sequence,
                int_sequence, base_index_map, offset_map, next_event_ptr_arr,
                updated_marginals_pointer, model_parameters_pointer,
                allowed_realizations, constructed_sequences, seq_offsets,
                error_rate_p, counters_list, events_map, safety_set,
                mismatches_lists, seq_max_prob_scenario,
                proba_threshold_factor);
          }
        }
      } else {
        for (unordered_map<string, Event_realization>::const_iterator
                 d_gene_iter = this->event_realizations.begin();
             d_gene_iter != this->event_realizations.end(); ++d_gene_iter) {

          // Starts the D one nucleotide after v_3_min offset(-1 if no V chosen)
          // given max deletions on the 5' of the D
          ...
          //  and that the maximum number of deletions might be greater than the
          //  D itself
          ...
          if (v_3_min_offset > 0) {
            ...
            d_5_off = 1 + d_5_real_max_del +
                      1; // Consider that V cannot be absent from the read, at
                         // least one nucleotide is present
          }
          ...
          while (d_3_min_offset < j_5_min_offset) {
            // Slides the D one nucleotide at a time towards 3', updating the
            // mismatch list,offsets

            // Get mismatches between D gene and sequence at the 5' most
            // position
            no_d_mismatches.clear();
            for (int i = 0; i != d_size; ++i) {
              if (((d_5_off + i) >= 0) & (d_5_off + i) < int_sequence.size()) {
            ...
            seq_offsets.set_value(D_gene_seq, Five_prime, d_5_off,
                                  memory_layer_off_fivep);
            seq_offsets.set_value(D_gene_seq, Three_prime, d_full_3_offset,
                                  memory_layer_off_threep);
            ...
            if (v_chosen and j_chosen) {
              if (vd_length_best_proba_map.count(d_5_off - v_offset - 1) <= 0 or
                  dj_length_best_proba_map.count(j_offset - d_full_3_offset -
                                                 1) <= 0) {
              ...
              if (dj_length_best_proba_map.count(j_offset - d_full_3_offset -
                                                 1) <= 0) {
                continue; // This means no scenario can lead to a correct
                          // solution, would need to be changed for Error models
                          // with in/dels
            ...
            if(gene_seq[d_size-1] != int_sequence[d_full_3_offset])
            no_d_mismacthes.push_back(d_full_3_offset);
*/
          }
        }
  ...
  case J_gene: {
    ...
    for (vector<Alignment_data>::const_iterator iter =
             allowed_realizations.at(J_gene).begin();
         iter != allowed_realizations.at(J_gene).end(); ++iter) {
      ...
      // Compute the number of nucleotides at the end of the sequence that are
      // not aligned with the J-gene and remove them gene_seq =
      // this->event_realizations.at((*iter).gene_name).value_str.substr(0,sequence.size()
      // - (*iter).offset);
      gene_seq = this->event_realizations.at((*iter).gene_name)
                     .value_str_int.substr(0, sequence.size() - (*iter).offset);

      constructed_sequences.set_value(J_gene_seq, &gene_seq, memory_layer_cs);

      seq_offsets.set_value(J_gene_seq, Five_prime, (*iter).offset,
                            memory_layer_off_fivep);
      seq_offsets.set_value(J_gene_seq, Three_prime,
                            (*iter).offset + gene_seq.size() - 1,
                            memory_layer_off_threep);

      // Mismatches list computed during alignment
      mismatches_lists.set_value(J_gene_seq, &(*iter).mismatches,
                                 memory_layer_mismatches);
      ...
      if (d_chosen) {
        ...
        downstream_proba_map.set_value(
            DJ_ins_seq, dj_length_best_proba_map.at(j_5_off - d_offset - 1),
            memory_layer_proba_map_junction);
      } else if (v_chosen) {
        ...
        downstream_proba_map.set_value(
            VJ_ins_seq, vj_length_best_proba_map.at(j_5_off - v_offset - 1),
            memory_layer_proba_map_junction);
      }
      ...
      while ((rev_mism_iter != iter->mismatches.rend()) and
             ((*rev_mism_iter) >= j_5_off - j_5_max_del)) {
        // Count one mismatch
        ++endogeneous_mismatches;
        ++rev_mism_iter;
      }
  ...
  default:
    throw invalid_argument(std::string("Unknown gene_class for GeneChoice: ") +
                           this->event_class);
    break;
  }
};
...
double Gene_choice::iterate_common(
    double scenario_proba, const int &gene_index, int base_index,
    Index_map &base_index_map,
    const unordered_map<Rec_Event_name,
                        vector<pair<shared_ptr<const Rec_Event>, int>>>
        &offset_map,
...
void Gene_choice::write2txt(ofstream &outfile) {
  outfile << "#GeneChoice;" << event_class << ";" << event_side << ";"
          << priority << ";" << nickname << endl;
  for (unordered_map<string, Event_realization>::const_iterator iter =
...
void Gene_choice::initialize_event(
  ...
  case V_gene:
    ...
    this->memory_layer_off_fivep =
        seq_offsets.get_current_memory_layer(V_gene_seq, Five_prime);
  ...
  case J_gene:
    ...
    this->memory_layer_off_threep =
        seq_offsets.get_current_memory_layer(J_gene_seq, Three_prime);
...
void Gene_choice::initialize_Len_proba_bound(
  ...
  case D_gene:
    ...
    if (v_chosen and j_chosen) {
      ...
      for (unordered_map<string, Event_realization>::const_iterator
        ...
        base_index = base_index_map.at(this->event_index, 0);
        for (size_t i = 0; i != this->event_marginal_size / this->size(); ++i) {
          if (model_parameters_point[base_index + d_gene_iter->second.index +
                                     i * this->size()] > d_gene_max_proba) {
            d_gene_max_proba =
                model_parameters_point[base_index + d_gene_iter->second.index +
                                       i * this->size()];
          }
        }
        // Loop over possible VD junction lengths
        for (map<int, double>::const_iterator vd_len_iter =
                 vd_length_best_proba_map.begin();
             vd_len_iter != vd_length_best_proba_map.end(); ++vd_len_iter) {
          // Loop over possible DJ junction lengths
          for (map<int, double>::const_iterator dj_len_iter =
                   dj_length_best_proba_map.begin();
               dj_len_iter != dj_length_best_proba_map.end(); ++dj_len_iter) {
            junction_len = d_gene_iter->second.value_str.size() +
                           vd_len_iter->first + dj_len_iter->first;

            if (vj_length_d_position_proba.count(junction_len) != 0) {
              vj_length_d_position_proba.at(junction_len)
                  .emplace_back(d_gene_iter->first, vd_len_iter->first,
                                dj_len_iter->first,
              ...
              vj_length_d_position_proba.emplace(
                  ...
                  make_tuple(
                      1, make_tuple(d_gene_iter->first, vd_len_iter->first,
                                    dj_len_iter->first,
                                    (d_gene_max_proba * vd_len_iter->second *
                                     dj_len_iter->second))));
            }
      ...
      // Now sort each vector in the map in decreasing order of probability
      // (according to the model)
      for (map<int, vector<tuple<string, int, int, double>>>::iterator
               d_position_map_iter = vj_length_d_position_proba.begin();
           d_position_map_iter != vj_length_d_position_proba.end();
           ++d_position_map_iter) {
        sort(d_position_map_iter->second.begin(),
             d_position_map_iter->second.end(), D_position_tuple);
      }
    }
