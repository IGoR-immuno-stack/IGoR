...
Pgen_counter::Pgen_counter(std::string path , bool output_Pgen_estimator_only , bool do_output_sequences): Counter(path) , output_sequences(do_output_sequences) , output_Pgen_estimator(output_Pgen_estimator_only) , sequence_Pgens_map(map<Int_Str,pair<double,double>>()) , scenario_resulting_sequence(Int_Str()) , read_likelihood(0)  , v_gene(false) , d_gene(false) , j_gene(false) , vd_ins(false) , dj_ins(false) , vj_ins(false){
...
void Pgen_counter::initialize_counter(const Model_Parms& parms , const Model_marginals& marginals){
	...
	const map<tuple<Event_type,Gene_class,Seq_side>, shared_ptr<Rec_Event>>& events_map = parms.get_events_map();
...
void Pgen_counter::dump_sequence_data(int seq_index , int iteration_n ){
	...
	for(map<Int_Str,pair<double,double>>::const_iterator iter = sequence_Pgens_map.begin() ; iter != sequence_Pgens_map.end() ; ++iter){
		if(output_Pgen_estimator){
