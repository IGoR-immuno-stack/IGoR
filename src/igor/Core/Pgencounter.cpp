...
Pgen_counter::Pgen_counter(std::string path, bool output_Pgen_estimator_only,
                           bool do_output_sequences)
    : Counter(path), output_sequences(do_output_sequences),
      output_Pgen_estimator(output_Pgen_estimator_only),
      sequence_Pgens_map(unordered_map<Int_Str, pair<double, long double>>()),
      scenario_resulting_sequence(Int_Str()), read_likelihood(0), v_gene(false),
      d_gene(false), j_gene(false), vd_ins(false), dj_ins(false),
      vj_ins(false) {
  if (output_Pgen_estimator and output_sequences) {
    throw invalid_argument(
        "Cannot set both \"output_Pgen_estimator_only\" and "
...
void Pgen_counter::initialize_counter(const Model_Parms &parms,
  ...
  if (not fstreams_created) {
    ...
    if (output_Pgen_estimator) {
      ...
      if (output_sequences) {
        (*output_pgen_file_ptr.get())
            << "seq_index;scen_sequence;Pgen;P_joint_read_seq" << endl;
      } else {
        (*output_pgen_file_ptr.get())
            << "seq_index;Pgen;P_seq_given_read" << endl;
      }
    }
...
void Pgen_counter::count_scenario(
  ...
  if (sequence_Pgens_map.count(scenario_resulting_sequence) > 0) {
    pair<double, long double> &Pgen_Pjoint_pair =
        sequence_Pgens_map[scenario_resulting_sequence];
...
void Pgen_counter::dump_sequence_data(int seq_index, int iteration_n) {
  ...
  for (unordered_map<Int_Str, pair<double, long double>>::const_iterator iter =
           sequence_Pgens_map.begin();
       iter != sequence_Pgens_map.end(); ++iter) {
    if (output_Pgen_estimator) {
      log_P_gen_estimate +=
          (*iter).second.second / read_likelihood * log((*iter).second.first);
    } else {
      if (output_sequences) {
        //(*output_pgen_file_ptr)<<seq_index<<";"<<(*iter).first<<";"<<(*iter).second.first<<";"<<(*iter).second.second/read_likelihood<<endl;
      } else {
        (*output_pgen_file_ptr.get())
            << seq_index << ";" << (*iter).second.first << ";"
  ...
  if (output_Pgen_estimator) {
    (*output_pgen_file_ptr.get())
        << seq_index << ";" << exp(log_P_gen_estimate) << endl;
  }
  // Reset counters
  read_likelihood = 0.0;
...
void Pgen_counter::add_checked(shared_ptr<Counter> counter) { return; }
...
shared_ptr<Counter> Pgen_counter::copy() const {
  ...
  if (this->fstreams_created) {
    ...
    throw runtime_error(
        "Counters should not be copied before stream initalization");
  }
