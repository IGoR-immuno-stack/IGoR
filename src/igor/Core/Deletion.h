...
public:
    // Constructor
    Deletion();
    Deletion(int, Seq_side);
    Deletion(int, Seq_side, std::pair<int, int>); // bounds for deletion lengths
    Deletion(int, Seq_side, std::unordered_map<std::string, Event_realization> &);
