 ...
 * \version 1.0
 *
 * The Deletion RecEvent models deletions of one genomic fragment on a given
 * side. Deletions can be either positive or negative (= Palindromic insertions)
 *
 * By construction the corresponding GeneChoice must have been explored first.
 */
class Deletion : public Rec_Event
{
    friend class Coverage_err_counter; // Grant friendship to access the current
    // number of deletion
    friend class Hypermutation_global_errorrate; // Grant friendship to access the
    // current number of deletion
...
private:
    ...
    // Pre create pairs to call seq_offsets (otherwise cost of creating a pair at
    // each call) std::pair<Seq_type,Seq_side> d_5_pair = std::make_pair
    // (D_gene_seq,Five_prime); std::pair<Seq_type,Seq_side> v_3_pair =
    // std::make_pair (V_gene_seq,Three_prime); std::pair<Seq_type,Seq_side>
    // j_5_pair = std::make_pair (J_gene_seq,Five_prime);
    // std::pair<Seq_type,Seq_side> d_3_pair = std::make_pair
    // (D_gene_seq,Three_prime);
    ...
    // Generic members for tandem D support
    int sequence_type_id;
    ...
    std::vector<std::pair<int, int>> active_upstream_junctions; // neighbor_type, junction_type
    std::vector<std::pair<int, int>> active_downstream_junctions; // neighbor_type, junction_type
    int neighbor_min_del;
    int neighbor_max_del;
    bool opposite_side_processed;

    // Downstream junction length proba bounds
    std::map<int, std::map<int, double>> junction_length_best_proba_maps;
    // Deprecated specific maps
