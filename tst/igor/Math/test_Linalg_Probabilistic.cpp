...
TEST_CASE("Linalg Probabilistic Operations", "[Math][Linalg][Probabilistic]") {
    ...
    SECTION("Dot Product - Basic") {
        auto v1 = test::make_sequential<double>({3}, 1.0);
        auto v2 = test::make_sequential<double>({3}, 4.0);
    ...
    SECTION("Matrix Multiplication - Basic 2x2") {
        ...
        // B = [[5, 6],
        //      [7, 8]]
        test::fill_2d(B, [](size_t i, size_t j) { return 5.0 + i * 2 + j; });
    ...
    SECTION("Matrix Multiplication - Non-Square") {
        ...
        test::fill_2d(A, [](size_t i, size_t j) { return i * 3 + j + 1; });
        test::fill_2d(B, [](size_t i, size_t j) { return i * 4 + j + 1; });
    ...
    SECTION("Matrix Multiplication - Identity") {
        Tensor<double> A({3, 3}), C({3, 3});

        // A = some matrix
        test::fill_2d(A, [](size_t i, size_t j) { return i * 3 + j + 1; });

        // I = identity
        auto I = test::make_identity(3);
    ...
    SECTION("Normalize - Basic") {
        auto t = test::make_sequential<double>({3}, 1.0);
        Tensor<double> out({3});
    ...
    SECTION("Normalize - 2D Tensor") {
        auto t = test::make_sequential<double>({2, 3}, 1.0);
        Tensor<double> out({2, 3});
    ...
    SECTION("Normalize - Zero Sum") {
        auto zeros = test::make_tensor<double>({3}, [](size_t) { return 0.0; });
        Tensor<double> out({3});
