/**
 * @file test_genetic_code.cpp
 * @brief Tests for the genetic code utilities and the JournaledQuery representation.
 *
 * Covers:
 * - GeneticCode.h: translate_codon, codon_index, codon_mask_for_aa,
 *   aa_to_iupac_codon, mask_to_iupac_codon, motif_char_to_mask,
 *   parse_aa_motif, translate_int_seq
 * - JournaledQuery.h: motif_to_journaled_query
 * - QuerySequenceContext: the optional patched/motif journal
 *
 * Extracted from feature/AA_PGEN. These are the parts of that branch that do not
 * live inside Rec_Event::iterate() and so are unaffected by the iterate refactor.
 */
...
TEST_CASE("Genetic code: translate_codon", "[genetic_code]") {
  // Standard codons
...
TEST_CASE("Genetic code: codon_index", "[genetic_code]") {
...
TEST_CASE("Genetic code: codon_mask_for_aa", "[genetic_code]") {
...
TEST_CASE("Genetic code: aa_to_iupac_codon", "[genetic_code]") {
  // Phe: TTY → TTY
...
TEST_CASE("Genetic code: mask_to_iupac_codon", "[genetic_code]") {
...
TEST_CASE("Genetic code: motif_char_to_mask", "[genetic_code]") {
  // Single AA
...
TEST_CASE("Genetic code: parse_aa_motif", "[genetic_code]") {
  // Simple AA sequence
...
TEST_CASE("Genetic code: translate_int_seq", "[genetic_code]") {
  // ATG TTA → ML
...
TEST_CASE("QuerySequenceContext: standard NT mode (no journaled_query)", "[query_context]") {
...
// ============================================================================
// Input validation: the motif must fit within the receptor
// ============================================================================
//
// Without the bounds check in motif_to_journaled_query, a motif that does not fit
// writes past the end of reference / iupac_union / iupac_intersection / empty_isect.
// The write is silent and corrupts the heap rather than failing, so these cases are
// asserted explicitly.

TEST_CASE("JournaledQuery: motif longer than the receptor is rejected", "[journaled_query]") {
    // 3 codons need 9 nt, receptor is 6
    REQUIRE_THROWS_AS(motif_to_journaled_query("MET", 0, 6), std::invalid_argument);
}

TEST_CASE("JournaledQuery: frame_offset pushing the motif past the end is rejected",
          "[journaled_query]") {
    // 1 codon at offset 4 needs 7 nt, receptor is 6
    REQUIRE_THROWS_AS(motif_to_journaled_query("M", 4, 6), std::invalid_argument);
}

TEST_CASE("JournaledQuery: negative frame_offset is rejected", "[journaled_query]") {
    REQUIRE_THROWS_AS(motif_to_journaled_query("M", -1, 6), std::invalid_argument);
}

TEST_CASE("JournaledQuery: motif exactly filling the receptor is accepted",
          "[journaled_query]") {
    REQUIRE_NOTHROW(motif_to_journaled_query("MW", 0, 6));
    // and the same motif one nt short is not
    REQUIRE_THROWS_AS(motif_to_journaled_query("MW", 0, 5), std::invalid_argument);
    // trailing slack is fine
    REQUIRE_NOTHROW(motif_to_journaled_query("MW", 0, 7));
}
