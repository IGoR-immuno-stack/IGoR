...
public:
    ...
    void
    iterate(double &, Downstream_scenario_proba_bound_map &, const string &, const Int_Str &,
            Index_map &,
            const unordered_map<Rec_Event_name, vector<pair<shared_ptr<const Rec_Event>, int>>> &,
            shared_ptr<Next_event_ptr> &, Marginal_array_p &, const Marginal_array_p &,
            const unordered_map<Gene_class, vector<Alignment_data>> &, Seq_type_str_p_map &,
            Seq_offsets_map &, shared_ptr<Error_rate> &, map<size_t, shared_ptr<Counter>> &,
    ...
    queue<int> draw_random_realization(
            ...
            const unordered_map<Rec_Event_name, vector<pair<shared_ptr<const Rec_Event>, int>>> &,
            unordered_map<int, string> &, mt19937_64 &) const override
    {
        return queue<int>();
    }
    ...
    void initialize_event(
            ...
            shared_ptr<Error_rate>, Mismatch_vectors_map &, Seq_offsets_map &, Index_map &) override
    {
    }
    ...
    void iterate_initialize_Len_proba(int, map<int, double> &, queue<shared_ptr<Rec_Event>> &,
                                      double &, const Marginal_array_p &, Index_map &,
                                      Seq_type_str_p_map &, int &) const override
    {
    }
    void initialize_Len_proba_bound(queue<shared_ptr<Rec_Event>> &, const Marginal_array_p &,
                                    Index_map &) override
    {
    }
};
...
TEST_CASE("EventUtils CheckGeneChoice", "[EventUtils]")
{
    ...
    SECTION("Event exists but not processed")
    {
        ...
        events_map[make_tuple(GeneChoice_t, (int)V_gene, Undefined_side)] = v_event;
...
TEST_CASE("EventUtils BuildScenarioSequence", "[EventUtils]")
{
    ...
    {
        Int_Str result =
                build_scenario_sequence(constructed_sequences, true, true, true, true, true, false);
        Int_Str expected = { 0, 1, 2, 3, 3, 0, 0, 1, 2 }; // V + VD + D + DJ + J
        REQUIRE(static_cast<std::vector<int>>(result) == static_cast<std::vector<int>>(expected));
    }

    SECTION("V-J with insertion")
    {
        Int_Str result = build_scenario_sequence(constructed_sequences, true, false, true, false,
                                                 false, true);
...
TEST_CASE("EventUtils GetInsertionLenMax", "[EventUtils]")
{
    ...
    {
    public:
        MockInsertionEvent(string name, int max_len) : MockEvent(name) { this->len_max = max_len; }
    };
    ...
    events_map[make_tuple(Insertion_t, (int)VD_genes, Undefined_side)] = vd_ins;
    events_map[make_tuple(Insertion_t, (int)DJ_genes, Undefined_side)] = dj_ins;
    events_map[make_tuple(Insertion_t, (int)VJ_genes, Undefined_side)] = vj_ins;
