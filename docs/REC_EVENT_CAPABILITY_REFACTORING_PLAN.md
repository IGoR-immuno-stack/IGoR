...
`has_effect_on()` and `iterate_initialize_Len_proba()` compare strings but are init-time. *(S4a replaced the former with `affects_length_of` / `affects_proba_of`; `Insertion`'s and `Deletion`'s still compare strings, `Dinucl_markov`'s now resolves the name once per query. Still init-time, so still not the decision that matters.)*
